// AUTO-GENERATED by scripts/bundle.js — do not edit. Source: engine/ + ai/ + data/cards.json
(function (global) {
'use strict';
// ===== engine/rng.js =====
// 确定性 RNG（mulberry32）：同 seed 同序列，支撑回放测试与联机同步
function makeRng(seed) {
  let a = seed >>> 0;
  const rng = () => {
    a |= 0; a = (a + 0x6D2B79F5) | 0;
    let t = Math.imul(a ^ (a >>> 15), 1 | a);
    t = (t + Math.imul(t ^ (t >>> 7), 61 | t)) ^ t;
    return ((t ^ (t >>> 14)) >>> 0) / 4294967296;
  };
  return rng;
}

function shuffle(arr, rng) {
  const a = arr.slice();
  for (let i = a.length - 1; i > 0; i--) {
    const j = Math.floor(rng() * (i + 1));
    [a[i], a[j]] = [a[j], a[i]];
  }
  return a;
}

// ===== engine/state.js =====
// 牌局状态结构与创建。纯数据，无方法——便于克隆/序列化/联机传输。


const START_HAND = 5;   // 起手抽 5
const DON_DECK_SIZE = 10;
const BOARD_LIMIT = 5;  // 角色区上限
const DON_CAP = 10;     // 全场 DON!! 上限

// leaderDef / cardDef 均为 data 层的静态定义（见 data/cards.json）
// 卡上 effect 为声明式对象：{ hook: 'onPlay'|'whenAttacking'|'onKO'|'trigger', op: {...} }
// fusions（F13 融合）：融合卡定义数组——不进卡组，由 state 携带配方供 fuse 动作消费；缺省空=融合不可用
function createGame({ leaderA, deckA, leaderB, deckB, seed = 1, fusions = [], rules = null }) {
  if (deckA.length !== 50 || deckB.length !== 50) throw new Error('deck must be 50 cards');
  const rng = makeRng(seed);
  const players = [
    mkPlayer(0, leaderA, deckA, rng),
    mkPlayer(1, leaderB, deckB, rng),
  ];

  return {
    seed,
    rng: null,             // rng 不可序列化；克隆时外部重建（见 cloneGame）
    turn: 1,
    active: 0,             // 先手固定 0 号（联机时由房间分配）
    firstTurn: true,       // 全局第一回合：先手 DON!! 阶段只 +1
    pending: null,         // 响应窗口 { kind:'block'|'counter', ... }
    onceMark: {},          // 每回合1次阀门记账：`${side}:${来源id}:${hook}:${技能名}` -> turn
    fusions,               // F13 融合配方（纯数据，快照/克隆随行；旧快照缺字段=旧局无融合）
    fuseUsed: [false, false], // F13 每回合限 1 次融合记账（endTurn 重置；旧快照缺字段时使用处兜底）
    rules,               // v2 可选规则集（{ deckoutByLp:bool }）：null=全默认旧行为；旧快照缺字段按 null 兜底
    winner: null,
    winReason: null,
    players,
    log: [],
  };
}

function mkPlayer(id, leaderDef, deckDefs, rng) {
  let deck = shuffle(deckDefs, rng);
  const hand = deck.slice(0, START_HAND);
  deck = deck.slice(START_HAND);
  return {
    id,
    leader: { ...leaderDef, rest: false, attackedTurn: 0, dons: 0, buffs: [] },
    lp: leaderDef.life * 2000, // LP 积分（游戏王式：原生命卡 ×2000 折算，LP≤0 判负）
    deck,                  // 牌组（顶在尾部 pop）
    hand,                  // 手牌
    donDeck: DON_DECK_SIZE,
    donArea: [],           // { id, rest, attached: null|{type:'leader'|'char', idx} }
    board: [],             // { ...cardDef, rest, playedTurn, dons, buffs }
    stage: null,           // { ...cardDef }
    trash: [],
  };
}

// 深克隆（rng 除外——由调用方按需重建并注入）
function cloneGame(state) {
  const s = JSON.parse(JSON.stringify(state));
  return s;
}

// ===== 查询辅助（纯函数） =====

function powerOfUnit(unit, pl, phase) {
  if (!unit) return 0;
  const buff = unit.buffs.reduce((n, b) => n + b.x, 0);
  const gearAtk = (unit.gears || []).reduce((n, g) => n + ((g.gear && g.gear.atk) || 0), 0);
  return unit.power + unit.dons * 1000 + buff + gearAtk + formationEdge(pl, unit.formation, phase);
}

function leaderPower(pl, phase) {
  const buff = pl.leader.buffs.reduce((n, b) => n + b.x, 0);
  return pl.leader.power + pl.leader.dons * 1000 + buff + formationEdge(pl, pl.leader.formation, phase);
}

// 阵型光环（design-system §4.1，标签制方案 C）：只在战斗结算相位生效（UI 基础显示不含光环，
// 真实对比体现在 dmg-calc 伤害算式浮字）。旧卡/旧快照无 formation 字段=无光环，天然兼容。
//   vanguard 突击：攻击相位，攻击者每多 1 名场上突击单位 +500
//   bulwark  铁壁：防守相位，防守方每多 1 名场上铁壁单位 +500
//   skirmish 游击：非光环——登场计数触发（见 phases.js summon 点）
function formationEdge(pl, formation, phase) {
  if (!pl || !formation) return 0;
  if (formation === 'vanguard' && phase === 'attack') {
    return Math.max(0, formationCount(pl, 'vanguard') - 1) * 500;
  }
  if (formation === 'bulwark' && phase === 'defense') {
    return Math.max(0, formationCount(pl, 'bulwark') - 1) * 500;
  }
  return 0;
}

function formationCount(pl, formation) {
  let n = pl.leader.formation === formation ? 1 : 0;
  for (const u of pl.board) if (u.formation === formation) n++;
  return n;
}

// 可用（未横置、未附着）DON!! 数量
function usableDons(pl) {
  return pl.donArea.filter((d) => !d.rest && !d.attached).length;
}

// 场上单位引用统一解析：{ side, type:'leader'|'char', idx }
function resolveUnit(state, ref) {
  const pl = state.players[ref.side];
  if (ref.type === 'leader') return pl.leader;
  return pl.board[ref.idx];
}

function logEvent(state, ev) {
  state.log.push(ev);
  return ev;
}

// ===== engine/keywords.js =====
// 词条与效果算子注册表（数据驱动：卡池 JSON 声明式挂载，引擎按表执行）
//
// 静态词条（布尔标记，引擎在规则点检查）：
//   rush          登场回合即可攻击
//   blocker       可横置顶包成为攻击目标（守备表示挡刀）
//   doubleAttack  直攻船长的 LP 伤害 ×2
//   banish        猛击：直攻船长时 LP 伤害额外 +2000（LP 积分制语义）
//
// 触发钩子（卡上 effect: { hook, op }；船长 v2 用 skills: [{hook, op, cond?, oncePerTurn?}]）：
//   onPlay         登场/打出时
//   whenAttacking  攻击宣告时（横置后、Block/Counter 窗口前）
//   onKO           被击倒进垃圾场时
//   trigger        作为 Life 被翻出时（banish 送达的不触发）
//   onSummon       己方角色登场时（船长技能用：娜美抽牌/索隆强化）
//   onAllyKO       己方角色被击沉时（船长技能用：山治回血）
//   onTurnStart    己方回合开始（DON 阶段后；船长技能用：明戈抽牌/香克斯加速）
//   onKill         己方击沉对方角色时（ctx.attacker=击沉发起者 ref；船长技能用：罗抽牌/凯多震伤）
//   whenAttacked   己方单位成为攻击目标时（防御方船长技能用：汉库珂魅惑）
//
// 效果声明（v2 船长分化）：
//   effect/skills 条目：{ hook, op, cond?, oncePerTurn? }
//   cond 条件（触发时校验，不满足=本次不触发）：
//     { lpMax }        我方 LP ≤ x（残血觉醒线）
//     { foeBoardMin }  对方场上角色数 ≥ x
//     { foeBoardMax }  对方场上角色数 ≤ x
//     { myBoardMin }   我方场上角色数 ≥ x
//     { handMin/handMax } 我方手牌数界
//   oncePerTurn: true —— 同一回合同一来源同钩子只触发一次（state.onceMark 记账）
//
// 算子 op（op 可为单对象或数组；数组=复合，discard 代价不可付时整条跳过）：
//   { k:'draw', n, minCost?, reqAttacker? }
//   { k:'powerSelf', x, until, minCost? }  x 为增量（如 +2000 写 2000）；minCost=仅对费用≥该值的单位生效
//   { k:'powerLeader', x, until }
//   { k:'gainDon', n }                 从 DON!! 牌库翻 n 张入费用区
//   { k:'koWeakest' }                  击倒敌方场上战力最低角色
//   { k:'restEnemy', target? }         横置敌方一个角色（last 默认/strongest/weakest）
//   { k:'healLP', x }                  LP 回复（上限=life×2000）
//   { k:'damageLP', x }                对方 LP 直伤（可终局）
//   { k:'buffAll', x, until }          我方全体角色战力+x
//   { k:'debuffFoeAll', x, until }     对方全体角色战力-x
//   { k:'discard', n }                 弃自己手牌 n 张（仅作复合代价段，弃最右）
//   { k:'search', n, faction?, type?, maxCost?, formation? }  从牌组找匹配卡入手牌（P1a archetype 引擎）
//   { k:'revive', maxCost? }           从墓场复活 ≤maxCost 角色回手牌（默认 3；仅 SS+ 卡可用，费率 −3K）

const HOOKS = ['onPlay', 'whenAttacking', 'onKO', 'trigger', 'onSummon', 'onAllyKO', 'onTurnStart', 'onKill', 'whenAttacked'];

function hasKeyword(unit, kw) {
  if (Array.isArray(unit.keywords) && unit.keywords.includes(kw)) return true;
  // 装备词条归并（甲胄给 blocker 等）：装备在则视为单位词条
  return Array.isArray(unit.gears) && unit.gears.some((g) => g.gear && Array.isArray(g.gear.gives) && g.gear.gives.includes(kw));
}

// 效果条目归一：船长 v2 skills 数组 / 通用 effects 数组 / 旧版单 effect
function effectsOf(cardOrUnit) {
  if (!cardOrUnit) return [];
  if (Array.isArray(cardOrUnit.skills)) return cardOrUnit.skills;
  if (Array.isArray(cardOrUnit.effects)) return cardOrUnit.effects;
  return cardOrUnit.effect ? [cardOrUnit.effect] : [];
}

// 统一执行入口：ctx = { state, side(操作方), self(效果来源单位或卡), rngNotNeeded }
function runEffect(state, cardOrUnit, hook, ctx = {}) {
  const side = ctx.side !== undefined ? ctx.side : state.active;
  const me = state.players[side];
  const foe = state.players[side === 0 ? 1 : 0];
  state.onceMark = state.onceMark || {}; // 旧回放兼容：缺字段时补挂

  for (const eff of effectsOf(cardOrUnit)) {
    if (!eff || eff.hook !== hook) continue;
    if (!condMet(state, eff.cond, side, me, foe)) continue;
    if (eff.oncePerTurn) {
      const key = `${side}:${cardOrUnit.id || 'anon'}:${hook}:${eff.name || ''}`;
      if (state.onceMark[key] === state.turn) continue;
      state.onceMark[key] = state.turn;
    }
    const ops = Array.isArray(eff.op) ? eff.op : [eff.op];
    // 原子性预检：discard 代价不可付 → 整条跳过（不留半完成状态）
    for (const op of ops) {
      if (op && op.k === 'discard' && me.hand.length < (op.n || 1)) return;
    }
    // 船长技能发动事件（F11 动效驱动源）：仅 skills 来源记，普通卡 effect 不记
    if (Array.isArray(cardOrUnit.skills) && eff.name) {
      logEvent(state, { t: 'skill', side, name: eff.name, awaken: /觉醒/.test(eff.name), src: cardOrUnit.id });
    }
    for (const op of ops) {
      execOp(state, cardOrUnit, eff, op, side, me, foe, ctx);
      if (state.winner !== null) return; // 效果可能直接终局
    }
  }
}

function condMet(state, cond, side, me, foe) {
  if (!cond) return true;
  if (cond.lpMax !== undefined && me.lp > cond.lpMax) return false;
  if (cond.foeBoardMin !== undefined && foe.board.length < cond.foeBoardMin) return false;
  if (cond.foeBoardMax !== undefined && foe.board.length > cond.foeBoardMax) return false;
  if (cond.myBoardMin !== undefined && me.board.length < cond.myBoardMin) return false;
  if (cond.handMin !== undefined && me.hand.length < cond.handMin) return false;
  if (cond.handMax !== undefined && me.hand.length > cond.handMax) return false;
  return true;
}

function execOp(state, cardOrUnit, eff, op, side, me, foe, ctx) {
  const enemySide = side === 0 ? 1 : 0;
  switch (op.k) {
    case 'draw': {
      const n = op.n || 1;
      // minCost 门槛（娜美：仅 3 费+ 登场才抽，防低费连抽滚雪球）
      if (op.minCost && ctx.self && ctx.self.cost != null && ctx.self.cost < op.minCost) break;
      // reqAttacker 门槛（罗 onKill：仅船长发起的击沉才抽，角色互斗吃掉不算）
      if (op.reqAttacker && (!ctx.attacker || ctx.attacker.type !== op.reqAttacker)) break;
      for (let i = 0; i < n; i++) {
        if (me.deck.length === 0) { declareDeckOut(state, side); return; }
        me.hand.push(me.deck.pop());
      }
      logEvent(state, { t: 'effectDraw', side, n, src: cardOrUnit.id });
      break;
    }
    case 'powerSelf': {
      // self 必须是场上单位；否则退化为无操作
      if (!ctx.self || !ctx.self.power) break;
      if (op.minCost && ctx.self.cost < op.minCost) break; // 费用门槛（索隆：只强化 5 费+）
      ctx.self.buffs.push({ x: op.x, until: op.until || 'turn', src: cardOrUnit.id });
      logEvent(state, { t: 'effectBuff', side, target: unitRefOf(state, side, ctx.self), x: op.x, src: cardOrUnit.id });
      break;
    }
    case 'powerLeader': {
      me.leader.buffs.push({ x: op.x, until: op.until || 'turn', src: cardOrUnit.id });
      logEvent(state, { t: 'effectBuff', side, target: { side, type: 'leader' }, x: op.x, src: cardOrUnit.id });
      break;
    }
    case 'gainDon': {
      const n = Math.min(op.n || 1, me.donDeck, DON_CAP - me.donArea.length);
      for (let i = 0; i < n; i++) {
        me.donArea.push({ id: me.donArea.length, rest: false, attached: null });
        me.donDeck--;
      }
      logEvent(state, { t: 'effectDon', side, n, src: cardOrUnit.id });
      break;
    }
    case 'koWeakest': {
      if (foe.board.length === 0) break;
      let mi = 0;
      for (let i = 1; i < foe.board.length; i++) {
        if (powerOfUnit(foe.board[i]) < powerOfUnit(foe.board[mi])) mi = i;
      }
      const [dead] = foe.board.splice(mi, 1);
      if (Array.isArray(dead.gears) && dead.gears.length) {
        foe.trash.push(...dead.gears);
        dead.gears = [];
      }
      foe.trash.push(dead);
      // 击沉回收 + 索引前移（v0.9.5，与 combat.koUnit/fuse 同款：附着贝里即时回费用区）
      for (const d of foe.donArea) {
        if (!d.attached || d.attached.type !== 'char') continue;
        if (d.attached.idx === mi) d.attached = null;
        else if (d.attached.idx > mi) d.attached.idx--;
      }
      logEvent(state, { t: 'ko', side: enemySide, idx: mi, cardId: dead.id, by: cardOrUnit.id });
      runEffect(state, dead, 'onKO', { side: enemySide, self: null });
      // 船长钩子补挂（v0.9.5）：效果击沉同享战斗击沉语义——阵亡方 onAllyKO（山治回血类）、
      // 击沉方 onKill（罗的 draw 带 reqAttacker:'leader' 门槛→效果击沉不抽=船长亲手击沉才抽，
      // 无门槛的 onKill 技能照常吃到）
      runEffect(state, foe.leader, 'onAllyKO', { side: enemySide, self: null });
      runEffect(state, me.leader, 'onKill', { side, self: null, attacker: null });
      break;
    }
    case 'restEnemy': {
      if (foe.board.length === 0) break;
      // target 选择：last 默认（最右）/ strongest / weakest
      let idx = foe.board.length - 1;
      if (op.target === 'strongest' || op.target === 'weakest') {
        idx = 0;
        for (let i = 1; i < foe.board.length; i++) {
          const p = powerOfUnit(foe.board[i]), cur = powerOfUnit(foe.board[idx]);
          if (op.target === 'strongest' ? p > cur : p < cur) idx = i;
        }
      }
      foe.board[idx].rest = true;
      logEvent(state, { t: 'rest', side: enemySide, idx, src: cardOrUnit.id });
      break;
    }
    case 'healLP': {
      const cap = me.leader.life * 2000;
      const before = me.lp;
      me.lp = Math.min(cap, me.lp + (op.x || 1000));
      logEvent(state, { t: 'heal', side, x: me.lp - before, lp: me.lp, src: cardOrUnit.id });
      break;
    }
    case 'damageLP': {
      const x = op.x || 1000;
      foe.lp -= x;
      logEvent(state, { t: 'lp', side: enemySide, dmg: x, lp: foe.lp, src: cardOrUnit.id });
      if (foe.lp <= 0) {
        state.winner = side;
        state.winReason = 'lp';
        logEvent(state, { t: 'win', winner: side, reason: 'lp' });
      }
      break;
    }
    case 'buffAll': {
      for (const u of me.board) {
        u.buffs.push({ x: op.x, until: op.until || 'turn', src: cardOrUnit.id });
      }
      logEvent(state, { t: 'effectBuffAll', side, x: op.x, n: me.board.length, src: cardOrUnit.id });
      break;
    }
    case 'debuffFoeAll': {
      for (const u of foe.board) {
        u.buffs.push({ x: -op.x, until: op.until || 'battle', src: cardOrUnit.id });
      }
      logEvent(state, { t: 'effectDebuffAll', side: enemySide, x: -op.x, n: foe.board.length, src: cardOrUnit.id });
      break;
    }
    case 'search': {
      // 从牌组找匹配卡入手牌（牌组顶在尾部；找不够=有多少拿多少，不洗牌不透牌序）
      // 条件：faction/type/maxCost/formation 任选组合（archetype 引擎的「找牌」核心原语）
      const n = op.n || 1;
      let found = 0;
      for (let i = me.deck.length - 1; i >= 0 && found < n; i--) {
        const c = me.deck[i];
        const ok = (!op.faction || c.faction === op.faction)
          && (!op.type || c.type === op.type)
          && (op.maxCost == null || c.cost <= op.maxCost)
          && (op.formation == null || c.formation === op.formation);
        if (ok) { me.deck.splice(i, 1); me.hand.push(c); found++; }
      }
      logEvent(state, { t: 'effectSearch', side, n: found, src: cardOrUnit.id });
      break;
    }
    case 'revive': {
      // 从墓场复活一张 ≤maxCost 角色回手牌（回场太强，回手仍需付费打出）
      const maxCost = op.maxCost || 3;
      let idx = -1;
      for (let i = me.trash.length - 1; i >= 0; i--) {
        if (me.trash[i].type === 'char' && me.trash[i].cost <= maxCost) { idx = i; break; }
      }
      if (idx >= 0) {
        const [c] = me.trash.splice(idx, 1);
        me.hand.push(c);
        logEvent(state, { t: 'effectRevive', side, cardId: c.id, src: cardOrUnit.id });
      }
      break;
    }
    case 'discard': {
      // 复合代价段：弃手牌最右 n 张（M1 无选择交互；可付性由 runEffect 预检保证）
      const n = op.n || 1;
      for (let i = 0; i < n; i++) {
        const [c] = me.hand.splice(me.hand.length - 1, 1);
        if (c) me.trash.push(c);
      }
      logEvent(state, { t: 'effectDiscard', side, n, src: cardOrUnit.id });
      break;
    }
    default:
      throw new Error('unknown effect op: ' + op.k);
  }
}

function declareDeckOut(state, side) {
  // v2 牌库耗尽裁定（rules.deckoutByLp 开启时，故事/对战模式用）：
  // 牌库打空不再直接判负，改按 LP（积分）裁定——低者负；同分=平局（winner=-1，区别于 null=进行中）。
  // 开关关闭=旧行为（抽空即负），引擎其他场景不受影响。
  if (state.rules && state.rules.deckoutByLp) {
    const a = state.players[0].lp, b = state.players[1].lp;
    if (a !== b) {
      state.winner = a > b ? 0 : 1;
      state.winReason = 'deckout-lp';
      logEvent(state, { t: 'win', winner: state.winner, reason: 'deckout-lp' });
    } else {
      state.winner = -1;
      state.winReason = 'deckout-draw';
      logEvent(state, { t: 'win', winner: -1, reason: 'deckout-draw' });
    }
    return;
  }
  state.winner = side === 0 ? 1 : 0;
  state.winReason = 'deckout';
  logEvent(state, { t: 'win', winner: state.winner, reason: 'deckout' });
}

// ===== 内部 =====


function unitRefOf(state, side, unit) {
  const idx = state.players[side].board.indexOf(unit);
  return idx >= 0 ? { side, type: 'char', idx } : { side, type: 'leader' };
}

// ===== engine/combat.js =====
// 战斗五步：攻击宣告 → Blocker 顶包 → Counter 窗口 → 结算 → 清算
// 游戏王式积分制：角色互斗比较战力、差额扣 LP；对方场上无角色时才可直攻船长（差额扣 LP）
// 响应窗口通过 state.pending 暴露给双方（人或 AI 均以 action 驱动，无回调）



// 发起攻击：action { t:'attack', attacker:{side,type,idx}, target:'leader'|{type:'char',idx} }
// 目标规则：对方场上有角色 → 必须指定其一（竖=攻击表示，横=守备表示，均可被攻击）；
//           场上无角色 → 只能直攻船长（船长战力为天然防线，伤害=差额）
function startAttack(state, action) {
  if (state.pending) throw new Error('cannot attack while pending window open');
  const { attacker, target } = action;
  const atk = resolveUnit(state, attacker);
  if (!atk) throw new Error('attacker not found');
  if (attacker.type === 'leader') {
    // 船长不横置（视觉常立），每回合限攻一次（试玩反馈：攻击后横放看着傻）
    if (atk.rest) throw new Error('leader is rested');
    if (atk.attackedTurn === state.turn) throw new Error('leader already attacked this turn');
  } else if (atk.rest) throw new Error('attacker is rested');
  if (attacker.type === 'char') {
    const unit = state.players[attacker.side].board[attacker.idx];
    if (unit.playedTurn === state.turn && !hasKeyword(unit, 'rush')) {
      throw new Error('summoning sickness: cannot attack on play turn (no Rush)');
    }
  }
  const defSide = attacker.side === 0 ? 1 : 0;
  const foe = state.players[defSide];

  let targetRef;
  if (target === 'leader' || target.type === 'leader') {
    if (foe.board.length > 0) throw new Error('must attack a character while opponent board is not empty');
    targetRef = { side: defSide, type: 'leader' };
  } else {
    const unit = foe.board[target.idx];
    if (!unit) throw new Error('target not found');
    targetRef = { side: defSide, type: 'char', idx: target.idx };
  }

  // 角色攻击后横置；船长保持立置、记 attackedTurn 限一次（效果横置船长仍生效）
  if (attacker.type === 'leader') atk.attackedTurn = state.turn;
  else atk.rest = true;
  logEvent(state, { t: 'attack', attacker, target: targetRef });

  // When Attacking 效果（横置后、窗口前）
  runEffect(state, atk, 'whenAttacking', { side: attacker.side, self: atk });

  // When Attacked 效果（防守方船长技能：己方单位成为攻击目标时；窗口前生效）
  runEffect(state, foe.leader, 'whenAttacked', { side: defSide, self: null });

  if (state.winner !== null) return; // 效果可能直接终局

  state.pending = {
    kind: 'counter',
    attacker,
    target: targetRef,
    counterBoost: 0,
    countered: [],
  };
  logEvent(state, { t: 'window', kind: 'counter', side: defSide });
}

// 响应窗口：Counter（可多次累加，为防守目标+战力/直攻减伤）：action { t:'counter', cards:[handIdx...] }  |  { t:'passCounter' }
function respondCounter(state, action) {
  const p = expectPending(state, 'counter');
  const defSide = p.target.side;
  const me = state.players[defSide];
  if (action.t === 'counter') {
    if (!Array.isArray(action.cards) || action.cards.length === 0) {
      throw new Error('counter cards required');
    }
    const uniq = [...new Set(action.cards)];
    if (uniq.length !== action.cards.length) throw new Error('duplicate counter card index');
    // 先整体校验再删除：任何一张非法则整批拒绝，不留半完成状态
    for (const hi of uniq) {
      const card = me.hand[hi];
      if (!card) throw new Error('counter card not in hand');
      if (!card.counter) throw new Error('card has no counter value');
    }
    // 倒序删除：多张时正序 splice 会让后续索引位移删错卡
    const added = [];
    let boost = 0;
    for (const hi of uniq.slice().sort((a, b) => b - a)) {
      const [card] = me.hand.splice(hi, 1);
      me.trash.push(card);
      p.counterBoost += card.counter;
      p.countered.push(card.id);
      added.unshift(card.id); // unshift 恢复原手牌顺序
      boost += card.counter;
    }
    // 增量语义：cards/boost = 本次打出；totalCards/totalBoost = 本次战斗累计（旧消费方差分可用 total 字段）
    logEvent(state, {
      t: 'counter', side: defSide,
      cards: added, boost,
      totalCards: p.countered.slice(), totalBoost: p.counterBoost,
    });
    return; // 窗口保持开放，可继续加或 pass
  }
  resolveAttack(state, p);
}

// 步骤4-5：比大小结算（游戏王式）+ 清算
// 坚壁（blocker）：被攻击时防御战力 +1000（横竖皆生效）
const BLOCKER_WALL = 1000;

// 恶魔果实三系循环克制（批1 战斗体系）：超人→自然→动物→超人
// 攻击方克制防守方 → 攻方战力 +1000（互斗/守备/直攻船长一律生效；任一方无果实则不参与）
const FRUIT_BEATS = { paramecia: 'logia', logia: 'zoan', zoan: 'paramecia' };
function fruitEdge(atkDef, defDef) {
  const a = atkDef && atkDef.fruit, d = defDef && defDef.fruit;
  return a && d && FRUIT_BEATS[a] === d ? 1000 : 0;
}

function resolveAttack(state, p) {
  const atk = resolveUnit(state, p.attacker);
  const atkCard = p.attacker.type === 'leader' ? state.players[p.attacker.side].leader : atk;
  const def = resolveUnit(state, p.target);
  const defCard = p.target.type === 'leader' ? state.players[p.target.side].leader : def;
  const edge = fruitEdge(atkCard, defCard); // 果实克制：只利攻击方
  // 阵型光环（方案 C）：攻击按攻击相位、防守按防守相位结算（state.js formationEdge）
  const atkPower = (p.attacker.type === 'leader'
    ? leaderPower(state.players[p.attacker.side], 'attack')
    : powerOfUnit(atk, state.players[p.attacker.side], 'attack')) + edge;
  const defPower = (p.target.type === 'leader'
    ? leaderPower(state.players[p.target.side], 'defense')
    : def // 目标在 counter 窗口期离场（效果竞态）：防线记 0，由下方 !def 分支 noDamage 收场
      ? powerOfUnit(def, state.players[p.target.side], 'defense') + (hasKeyword(def, 'blocker') ? BLOCKER_WALL : 0)
      : 0) + p.counterBoost;

  logEvent(state, { t: 'clash', atkPower, defPower, fruitEdge: edge });

  if (p.target.type === 'leader') {
    // 直攻：伤害=差额（船长战力为防线，Counter 可减伤）；双击=差额×2，猛击=+2000 保底
    let dmg = Math.max(0, atkPower - defPower);
    if (dmg > 0 && hasKeyword(atk, 'doubleAttack')) dmg *= 2;
    if (hasKeyword(atk, 'banish')) dmg += 2000;
    if (dmg <= 0) logEvent(state, { t: 'noDamage', reason: 'power', side: p.target.side, atkPower, defPower });
    else dealLpDamage(state, p.target.side, dmg, p.attacker.side);
  } else if (!def) {
    // 顶包/效果竞态下目标已不在场：无战果收场
    logEvent(state, { t: 'noDamage', reason: 'gone' });
  } else if (def.rest) {
    // 守备表示（横置）：打得动才击沉，无差额伤害；打不动=无战果
    if (atkPower > defPower) {
      koUnit(state, p.target, p.attacker.side, p.attacker);
    } else {
      logEvent(state, { t: 'noDamage', reason: 'defense', side: p.target.side, atkPower, defPower });
    }
  } else {
    // 攻击表示互斗：战力比较，差额扣败方 LP；相等同归于尽（攻击者是船长则船长不沉）
    if (atkPower > defPower) {
      koUnit(state, p.target, p.attacker.side, p.attacker);
      if (state.winner === null) dealLpDamage(state, p.target.side, atkPower - defPower, p.attacker.side);
    } else if (atkPower < defPower) {
      if (p.attacker.type === 'char') koUnit(state, p.attacker, p.target.side, p.target);
      if (state.winner === null) dealLpDamage(state, p.attacker.side, defPower - atkPower, p.target.side);
    } else {
      if (p.attacker.type === 'char') koUnit(state, p.attacker, p.target.side, p.target);
      koUnit(state, p.target, p.attacker.side, p.attacker);
    }
  }

  // 清算：本次战斗的临时增益清空
  clearBattleBuffs(state);
  state.pending = null;
}

// 击沉角色进墓场（触发 onKO / onAllyKO / onKill）
// byRef=击沉发起者 ref（{side,type,idx}；互斗反杀时为守方单位）——onKill 技能的 attacker 上下文
function koUnit(state, ref, bySide, byRef = null) {
  const pl = state.players[ref.side];
  if (ref.type !== 'char') return;
  const [dead] = pl.board.splice(ref.idx, 1);
  if (!dead) return;
  // 装备是独立卡：角色被击沉时装备一并展平进墓场（独立计数/可回收语义）
  if (Array.isArray(dead.gears) && dead.gears.length) {
    pl.trash.push(...dead.gears);
    dead.gears = [];
  }
  pl.trash.push(dead);
  // 击沉回收（v0.9.5）：附着在被击沉单位上的贝里即时回费用区（真实规则语义；此前孤儿引用
  // 挂到下回合开始才脱落=被击沉方凭空锁死一枚可花贝里）——与 fuse 素材回收（phases.js）同款：
  // === 回收、> 前移（否则 takeDon 找不到=引擎不一致）
  for (const d of pl.donArea) {
    if (!d.attached || d.attached.type !== 'char') continue;
    if (d.attached.idx === ref.idx) d.attached = null;
    else if (d.attached.idx > ref.idx) d.attached.idx--;
  }
  logEvent(state, { t: 'ko', side: ref.side, cardId: dead.id });
  runEffect(state, dead, 'onKO', { side: ref.side, self: null });
  // 船长技能：己方角色被击沉钩子（山治回血——阵亡补偿）
  runEffect(state, pl.leader, 'onAllyKO', { side: ref.side, self: null });
  // 船长技能：击沉对方角色钩子（罗抽牌——收割资源）；发起者先于目标移除时不触发
  const byLeader = state.players[bySide] && state.players[bySide].leader;
  if (byLeader) runEffect(state, byLeader, 'onKill', { side: bySide, self: null, attacker: byRef });
}

// LP 伤害与胜负（游戏王式积分制）
function dealLpDamage(state, side, dmg, bySide) {
  const pl = state.players[side];
  pl.lp -= dmg;
  logEvent(state, { t: 'lp', side, dmg, lp: pl.lp });
  if (pl.lp <= 0) {
    state.winner = bySide;
    state.winReason = 'lp';
    logEvent(state, { t: 'win', winner: state.winner, reason: 'lp' });
  }
}

function clearBattleBuffs(state) {
  for (const pl of state.players) {
    pl.leader.buffs = pl.leader.buffs.filter((b) => b.until !== 'battle');
    for (const u of pl.board) u.buffs = u.buffs.filter((b) => b.until !== 'battle');
  }
}

function expectPending(state, kind) {
  if (!state.pending || state.pending.kind !== kind) {
    throw new Error(`expected ${kind} window, got ${state.pending ? state.pending.kind : 'none'}`);
  }
  return state.pending;
}

// ===== engine/phases.js =====
// 回合结构与 Main 阶段动作。所有玩家操作统一为 action 对象，经 applyAction 驱动。
// 确定性：无隐藏随机（洗牌只在 createGame），回放 = seed + action 序列。




// 回合开始自动序列：Refresh → Draw → DON!!（无决策，引擎自动推进到 Main）
function startTurn(state) {
  const pl = state.players[state.active];
  // Refresh：己方全部单位回正、附着 DON!! 收回费用区
  pl.leader.rest = false;
  pl.leader.dons = 0;
  for (const u of pl.board) { u.rest = false; u.dons = 0; }
  for (const d of pl.donArea) { d.rest = false; d.attached = null; }
  logEvent(state, { t: 'refresh', side: state.active });

  // Draw（v0.9.3：先手全局首回合不抽——起手 5 张直接开打，后手保留首抽作多 1 张补偿；
  // 试玩反馈「开局 6 张太多」。牌库空判定照旧在抽牌点结算）
  const isFirstTurnEver = state.turn === 1 && state.firstTurn;
  if (pl.deck.length === 0) { declareDeckOut(state, state.active); return; }
  if (!isFirstTurnEver) {
    const drawn = pl.deck.pop();
    pl.hand.push(drawn);
    logEvent(state, { t: 'draw', side: state.active, cardId: drawn.id });
  }

  // DON!!：先手全局第一回合 +1，其余 +2；牌库不足放剩余；费用区 10 张封顶
  const want = isFirstTurnEver ? 1 : 2;
  const room = DON_CAP - pl.donArea.length;
  const n = Math.min(want, pl.donDeck, room);
  for (let i = 0; i < n; i++) {
    pl.donArea.push({ id: pl.donArea.length, rest: false, attached: null });
    pl.donDeck--;
  }
  logEvent(state, { t: 'donGain', side: state.active, n });

  // 船长技能：回合开始钩子（DON 阶段后、Main 前——香克斯的加速豆在此生效）
  runEffect(state, pl.leader, 'onTurnStart', { side: state.active, self: null });
  if (state.winner !== null) return; // 效果可能触发 deckout 等终局

  state.phase = 'main';
}

// 结束回合：清 turn 增益 → 换边 → 新回合自动序列
function endTurn(state) {
  if (state.pending) throw new Error('cannot end turn while pending window open');
  for (const pl of state.players) {
    pl.leader.buffs = pl.leader.buffs.filter((b) => b.until !== 'turn');
    for (const u of pl.board) u.buffs = u.buffs.filter((b) => b.until !== 'turn');
  }
  state.fuseUsed = [false, false]; // F13 每回合限 1 次融合：换边重置（旧快照缺字段=旧局无融合，重置无害）
  logEvent(state, { t: 'endTurn', side: state.active });
  state.firstTurn = false;
  state.active = 1 - state.active;
  if (state.active === 0) state.turn++;
  startTurn(state);
}

// 出角色：从手牌支付费用登场
function playCharacter(state, side, idx) {
  const me = state.players[side];
  const card = me.hand[idx];
  if (!card || card.type !== 'char') throw new Error('not a character card in hand');
  if (me.board.length >= 5) throw new Error('board limit 5 reached');
  payDons(me, card.cost);
  me.hand.splice(idx, 1);
  const unit = { ...card, rest: false, playedTurn: state.turn, dons: 0, buffs: [], gears: [] };
  me.board.push(unit);
  logEvent(state, { t: 'summon', side, cardId: card.id, cost: card.cost });
  runEffect(state, unit, 'onPlay', { side, self: unit });
  // 船长技能：己方角色登场钩子（娜美抽牌/索隆强化——对新登场单位生效）
  runEffect(state, me.leader, 'onSummon', { side, self: unit });
  // 游击阵型（design-system §4.1）：每当第 3 名游击单位登场（场上游击数恰为 3 的倍数）抽 1
  if (card.formation === 'skirmish') {
    let n = (me.leader.formation === 'skirmish' ? 1 : 0);
    for (const u of me.board) if (u.formation === 'skirmish') n++;
    if (n > 0 && n % 3 === 0) {
      if (me.deck.length === 0) { declareDeckOut(state, side); return; }
      me.hand.push(me.deck.pop());
      logEvent(state, { t: 'skirmishDraw', side, n, src: card.id });
    }
  }
}

// 出事件：付费 → 执行效果 → 进垃圾场
function playEvent(state, side, idx) {
  const me = state.players[side];
  const card = me.hand[idx];
  if (!card || card.type !== 'event') throw new Error('not an event card in hand');
  payDons(me, card.cost);
  me.hand.splice(idx, 1);
  me.trash.push(card);
  logEvent(state, { t: 'event', side, cardId: card.id, cost: card.cost });
  runEffect(state, card, 'onPlay', { side, self: null });
}

// 出舞台：全局唯一，旧的进垃圾场
function playStage(state, side, idx) {
  const me = state.players[side];
  const card = me.hand[idx];
  if (!card || card.type !== 'stage') throw new Error('not a stage card in hand');
  payDons(me, card.cost);
  me.hand.splice(idx, 1);
  const replaced = !!me.stage; // 必须在赋值新舞台前取值，否则恒为 true
  if (replaced) me.trash.push(me.stage);
  me.stage = card;
  logEvent(state, { t: 'stage', side, cardId: card.id, replaced });
  runEffect(state, card, 'onPlay', { side, self: null });
}

// 出装备：付费附着到己方角色（每角色限 1 件，重复出=替换旧的进垃圾场）
function playGear(state, side, idx, to) {
  const me = state.players[side];
  const card = me.hand[idx];
  if (!card || card.type !== 'gear') throw new Error('not a gear card in hand');
  const target = me.board[to && to.idx];
  if (!target) throw new Error('gear target not found');
  payDons(me, card.cost);
  me.hand.splice(idx, 1);
  if (target.gears && target.gears.length) me.trash.push(target.gears[0]); // 替换：旧装备随葬
  target.gears = [card];
  logEvent(state, { t: 'gear', side, cardId: card.id, to: { type: 'char', idx: to.idx } });
}

// ===== 融合（F13）=====
// 融合体登场规则：配方素材（己方场上+手牌各取一份，同名多份取一）全部进墓场 → 融合体直接进 board。
// 融合卡不进卡组（deckOf 跳过 / validateDeck 拒收），只能经 t:'fuse' { side, fusionId } 动作登场。

// 可融合性纯查询（AI 枚举与 UI 按钮共用同一真值源）：返回 null=可融合，否则返回不可融合原因
function fuseLockReason(state, side, def) {
  const me = state.players[side];
  if (!def || !def.fusion) return '未知融合配方';
  if (me.board.length >= 5) return '场上已满 5 名角色，无法融合';
  if (usableDons(me) < def.fusion.cost) return `融合费用不足：需 ${def.fusion.cost} 枚可用贝里（附着贝里=已消耗）`;
  const used = Array.isArray(state.fuseUsed) ? state.fuseUsed : [false, false]; // 旧快照缺字段兜底
  if (used[side]) return '每回合限融合 1 次';
  for (const id of new Set(def.fusion.from)) { // 同名多份取一（配方内重复 id 只需一份）
    if (!me.board.some((u) => u.id === id) && !me.hand.some((c) => c.id === id)) {
      return `素材不足：缺 ${id}（场上或手牌均可）`;
    }
  }
  return null;
}

// 场上素材摘除：与 combat.koUnit 同规则（装备随葬、其后单位索引前移同步修正附着 DON 记账）；
// 差异：素材自身附着的 DON 直接回收（attached=null 回费用区，可用态）；不触发 onKO/onAllyKO/onKill（融合≠击沉）
function scrapBoardUnit(pl, idx) {
  const [unit] = pl.board.splice(idx, 1);
  if (!unit) return null;
  if (Array.isArray(unit.gears) && unit.gears.length) {
    pl.trash.push(...unit.gears);
    unit.gears = [];
  }
  pl.trash.push(unit);
  for (const d of pl.donArea) {
    if (!d.attached || d.attached.type !== 'char') continue;
    if (d.attached.idx === idx) d.attached = null;    // 素材附着贝里回收（勿留悬空指针）
    else if (d.attached.idx > idx) d.attached.idx--;  // 其后单位索引前移
  }
  return unit;
}

// 融合动作主体：素材判定 → 支付 fusion.cost → 素材进墓场 → 融合体登场（playedTurn=当前回合，rush=当回合可攻）
function fuse(state, side, fusionId) {
  const me = state.players[side];
  const def = (state.fusions || []).find((c) => c && c.id === fusionId); // 旧快照缺 fusions=无融合可用
  const why = fuseLockReason(state, side, def);
  if (why) throw new Error(why);
  const fromIds = [...new Set(def.fusion.from)];
  // 定位素材（先场后手）；倒序摘除防索引位移删错卡
  const boardPicks = [], handPicks = [];
  for (const id of fromIds) {
    const bi = me.board.findIndex((u) => u.id === id);
    if (bi >= 0) boardPicks.push(bi);
    else handPicks.push(me.hand.findIndex((c) => c.id === id));
  }
  payDons(me, def.fusion.cost);
  for (const bi of boardPicks.sort((a, b) => b - a)) scrapBoardUnit(me, bi);
  for (const hi of handPicks.sort((a, b) => b - a)) {
    const [c] = me.hand.splice(hi, 1);
    if (c) me.trash.push(c);
  }
  const unit = { ...def, rest: false, playedTurn: state.turn, dons: 0, buffs: [], gears: [] };
  me.board.push(unit);
  state.fuseUsed = Array.isArray(state.fuseUsed) ? state.fuseUsed : [false, false];
  state.fuseUsed[side] = true;
  logEvent(state, { t: 'fuse', side, fusionId: def.id, fromIds });
  runEffect(state, unit, 'onPlay', { side, self: unit });        // 融合体登场效果（buffAll/restEnemy/draw 等）
  runEffect(state, me.leader, 'onSummon', { side, self: unit }); // 船长「角色登场」钩子（与 playCharacter 同语义）
}


// 附着 DON!!：费用区（未横置未附着）→ 己方 Leader/角色
function giveDon(state, side, to, count) {
  const me = state.players[side];
  const avail = me.donArea.filter((d) => !d.rest && !d.attached);
  if (avail.length < count) throw new Error('not enough usable DON!!');
  if (count <= 0) throw new Error('count must be positive');
  let target;
  if (to.type === 'leader') target = me.leader;
  else {
    target = me.board[to.idx];
    if (!target) throw new Error('attach target not found');
  }
  for (let i = 0; i < count; i++) { avail[i].attached = { type: to.type, idx: to.idx }; }
  target.dons += count;
  logEvent(state, { t: 'donAttach', side, to, count });
}

// 收回附着 DON!! 回费用区（Main 阶段可逆操作）
function takeDon(state, side, from, count) {
  if (count <= 0) throw new Error('count must be positive'); // 与 giveDon 对齐，堵 count=0 空操作
  const me = state.players[side];
  const attached = me.donArea.filter((d) => d.attached && d.attached.type === from.type
    && (from.type === 'leader' || d.attached.idx === from.idx));
  if (attached.length < count) throw new Error('not enough attached DON!! on target');
  let target;
  if (from.type === 'leader') target = me.leader;
  else {
    target = me.board[from.idx];
    if (!target) throw new Error('detach target not found'); // 原先缺检：undefined 上 -= 直接 TypeError
  }
  for (let i = 0; i < count; i++) { attached[i].attached = null; }
  target.dons -= count;
  logEvent(state, { t: 'donTake', side, from, count });
}

function payDons(me, cost) {
  if (cost < 0) throw new Error('negative cost');
  if (usableDons(me) < cost) throw new Error('not enough DON!! to pay');
  let left = cost;
  for (const d of me.donArea) {
    if (left === 0) break;
    if (!d.rest && !d.attached) { d.rest = true; left--; }
  }
}

// ===== 动作分发（index.js 调用） =====
function applyAction(state, action) {
  if (state.winner !== null) throw new Error('game over');
  const side = action.side;
  if (typeof side !== 'number') throw new Error('action.side required');

  // 响应窗口（Counter）：行动权在防守方
  if (state.pending) {
    if (side !== state.pending.target.side) throw new Error('not your response window');
    switch (action.t) {
      case 'counter': case 'passCounter': respondCounter(state, action); return;
      default: throw new Error(`illegal action ${action.t} during response window`);
    }
  }

  if (side !== state.active) throw new Error('not your turn');
  switch (action.t) {
    case 'playCharacter': playCharacter(state, side, action.idx); break;
    case 'playEvent': playEvent(state, side, action.idx); break;
    case 'playStage': playStage(state, side, action.idx); break;
    case 'playGear': playGear(state, side, action.idx, action.to); break;
    case 'giveDon': giveDon(state, side, action.to, action.count || 1); break;
    case 'takeDon': takeDon(state, side, action.from, action.count || 1); break;
    case 'fuse': fuse(state, side, action.fusionId); break;
    case 'attack': startAttack(state, action); break;
    case 'endTurn': endTurn(state); break;
    default: throw new Error('unknown action: ' + action.t);
  }
}

// ===== engine/legality.js =====
// 牌组合法性校验（构筑器与后端共用）
// 规则：正好 50 张；颜色全部与 Leader 一致（无色卡不存在于 M1 卡池）；
//       同编号卡最多 4 张；Leader 卡不可入组；融合卡不进卡组（F13：经 t:'fuse' 动作登场）；
//       类型/字段合法性由卡池 schema 保证。
function validateDeck(leaderDef, deckDefs) {
  const errs = [];
  if (!leaderDef || leaderDef.type !== 'leader') errs.push('invalid leader');
  if (!Array.isArray(deckDefs) || deckDefs.length !== 50) {
    errs.push(`deck size must be 50, got ${deckDefs ? deckDefs.length : 'null'}`);
    return errs;
  }
  const counts = new Map();
  for (const c of deckDefs) {
    if (!c || !c.id) { errs.push('null card in deck'); continue; }
    if (c.type === 'leader') errs.push(`leader card ${c.id} cannot be in deck`);
    if (c.fusion) errs.push(`fusion card ${c.id} cannot be in deck (融合卡经融合动作登场，不进卡组)`);
    if (leaderDef && c.color !== leaderDef.color) {
      errs.push(`card ${c.id} color ${c.color} mismatches leader color ${leaderDef.color}`);
    }
    counts.set(c.id, (counts.get(c.id) || 0) + 1);
  }
  for (const [id, n] of counts) {
    if (n > 4) errs.push(`card ${id} appears ${n} times (max 4)`);
  }
  return errs;
}

// ===== engine/deck.js =====
// 默认卡组构造：费段配额对齐 + 段内均匀步长采样
// 背景：旧实现「每卡×4 轮次交错后取前 50」在池扩到 82-85 张/色后 = 池序前 50 张各 1 张，
// char 续号把 event/stage/gear 全挤出默认组，且各色进组新卡服从号序段偏移 → 色间失衡被放大。
// P2 扩池至 988 张后实测（balance-sim 100 局）：纯 id 步长采样把各色池的随机 op 倾斜带进卡组
// （紫色卡组 damageLP/doubleAttack/banish 全 0 → 绿v紫 84% 全场最宽带宽）。
// 修法：char 按固定费段配额构造（v0.9.3：1-8 费 = 4/8/9/9/6/4/2/2 合计 44），段不足向相邻段借；
// 段内步长均匀取样——色间卡组费用曲线强制同构，差异只留词条/效果/阵型风格，确定性（无 rng）。
// v0.9.3 装备位 4→6 张（试玩反馈「很少看到装备卡」：8% 占比一局摸不到 1-2 张）。
const COST_PLAN = [[1, 4], [2, 8], [3, 9], [4, 9], [5, 6], [6, 4], [7, 2], [8, 2]]; // 合计 44

function deckOf(pool, color) {
  // F13：带 fusion 字段的卡不进卡组（只能经 t:'fuse' 动作登场），char46 采样自然不含融合卡
  const cs = pool.cards.filter((c) => c.color === color && !c.fusion);
  const chars = cs.filter((c) => c.type === 'char');
  const byCost = {};
  for (const c of chars) (byCost[c.cost] = byCost[c.cost] || []).push(c);
  // 段内按 id 稳定排序后步长取样（确定性：同池同结果；len<n 时 round-robin 补齐，每卡≤2 份）
  const takeFrom = (arr, n) => {
    const t = [...arr].sort((a, b) => a.id.localeCompare(b.id, 'en', { numeric: true }));
    const out = [];
    for (let i = 0; i < n && t.length; i++) out.push(t[Math.floor((i * t.length) / n) % t.length]);
    return out;
  };
  const deck = [];
  for (const [cost, want] of COST_PLAN) {
    const seg = byCost[cost] || [];
    deck.push(...takeFrom(seg, Math.min(want, seg.length))); // 段不足先全取（保曲线），缺口下方统一补
  }
  if (deck.length < 44) {
    // 缺口补齐：剩余卡按费用升序（贴近低费缺口优先；高费段池内普遍充足不至缺）
    const have = new Set(deck.map((c) => c.id));
    const rest = chars.filter((c) => !have.has(c.id)).sort((a, b) => a.cost - b.cost || a.id.localeCompare(b.id, 'en', { numeric: true }));
    deck.push(...takeFrom(rest, 44 - deck.length));
  }
  const gears = cs.filter((c) => c.type === 'gear');
  return [...deck.slice(0, 44), ...takeFrom(gears, 6)];
}

// ===== engine/index.js =====
// 引擎门面：前后端同构入口










// 建局并自动推进到先手 Main 阶段（含首回合 Refresh/Draw/DON!!+1）
function newGame(opts) {
  const state = createGame(opts);
  startTurn(state);
  return state;
}

// ===== ai/actions.js =====
// 合法动作枚举器：AI 决策与随机模糊测试共用




// 返回当前行动权方的全部合法动作（每张 counter 卡按单张出牌枚举）
function listActions(state) {
  if (state.winner !== null) return [];
  if (state.pending) return defenseActions(state);

  const side = state.active;
  const me = state.players[side];
  const foe = state.players[1 - side];
  const acts = [];
  const dons = usableDons(me);

  // 出牌
  me.hand.forEach((c, i) => {
    if (c.cost > dons) return;
    if (c.type === 'char' && me.board.length >= 5) return;
    if (c.type === 'gear') {
      // 装备：枚举己方每个角色作为目标（每角色限 1 件，替换式）
      me.board.forEach((_, j) => acts.push({ t: 'playGear', side, idx: i, to: { type: 'char', idx: j } }));
      return;
    }
    acts.push({ t: c.type === 'char' ? 'playCharacter' : c.type === 'event' ? 'playEvent' : 'playStage', side, idx: i });
  });

  // 融合（F13）：配方素材齐（场上+手牌）且贝里够、场上未满、本回合未融合 → 逐配方枚举
  // state.fusions 为空（旧调用方未注入）时不产生任何 fuse 动作，行为与 F13 前完全一致
  for (const def of state.fusions || []) {
    if (!def || !def.fusion) continue;
    if (fuseLockReason(state, side, def) === null) acts.push({ t: 'fuse', side, fusionId: def.id });
  }

  // 附着 / 收回 DON!!（按 1 张粒度枚举）
  if (dons >= 1) {
    acts.push({ t: 'giveDon', side, to: { type: 'leader' }, count: 1 });
    me.board.forEach((_, i) => acts.push({ t: 'giveDon', side, to: { type: 'char', idx: i }, count: 1 }));
  }
  if (me.leader.dons > 0) acts.push({ t: 'takeDon', side, from: { type: 'leader' }, count: 1 });
  me.board.forEach((u, i) => {
    if (u.dons > 0) acts.push({ t: 'takeDon', side, from: { type: 'char', idx: i }, count: 1 });
  });

  // 攻击：己方未横置且可行动的单位 × 合法目标
  // 游戏王式：对方场上有角色（竖/横均可）→ 必须指定其一；场上无角色 → 只能直攻船长
  const attackers = [];
  // 船长不横置：每回合限攻一次（attackedTurn 记账，效果横置仍拦）
  if (!me.leader.rest && me.leader.attackedTurn !== state.turn) attackers.push({ side, type: 'leader' });
  me.board.forEach((u, i) => {
    if (u.rest) return;
    if (u.playedTurn === state.turn && !hasKeyword(u, 'rush')) return;
    attackers.push({ side, type: 'char', idx: i });
  });
  const targets = foe.board.length > 0
    ? foe.board.map((_, i) => ({ type: 'char', idx: i }))
    : ['leader'];
  for (const a of attackers) {
    for (const tg of targets) acts.push({ t: 'attack', side, attacker: a, target: tg });
  }

  acts.push({ t: 'endTurn', side });
  return acts;
}

function defenseActions(state) {
  const p = state.pending;
  const side = p.target.side;
  const me = state.players[side];
  const acts = [];
  me.hand.forEach((c, i) => {
    if (c.counter) acts.push({ t: 'counter', side, cards: [i] });
  });
  acts.push({ t: 'passCounter', side });
  return acts;
}

// ===== ai/heuristic.js =====
// 启发式 AI：三档难度（easy=大噪声 / normal=纯启发式 / hard=启发式+浅层前瞻）
// 输入 state + 合法动作列表，输出选中的动作。纯函数式，无引擎副作用。





const LEVELS = ['easy', 'normal', 'hard'];

function createAI(level = 'normal', rng = Math.random) {
  if (!LEVELS.includes(level)) throw new Error('unknown level: ' + level);
  const noise = level === 'easy' ? 25 : level === 'normal' ? 6 : 0;
  const blunder = level === 'easy' ? 0.22 : 0; // easy 概率性纯乱走
  return {
    level,
    choose(state, acts) {
      if (!acts || acts.length === 0) return null;
      const scored = acts.map((a) => ({ a, s: scoreAction(state, a) + rng() * noise }));
      if (blunder > 0 && rng() < blunder) {
        // 乱走排除 takeDon（give/take 互切死循环，AI 诊断实证）；反向选最差——
        // 988 池曲线对齐后随机乱走送分不足（hard 仅 57% 分层），新手档语义=可被碾压
        const pool = scored.filter((x) => x.a.t !== 'takeDon');
        const p = pool.length ? pool : scored;
        p.sort((x, y) => x.s - y.s);
        return p[0].a;
      }
      scored.sort((x, y) => y.s - x.s);
      if (level !== 'hard') return scored[0].a;
      // hard：对 top5 前瞻一步（执行动作+响应全 pass，取局面估值差），叠加启发分
      const me = state.pending ? state.pending.target.side : state.active;
      let best = null;
      let bestV = -Infinity;
      for (const { a, s: s0 } of scored.slice(0, 5)) {
        // hard 攻击折减：前瞻把对手响应强制 pass，但高反击手牌的真实局会翻盘互斗/垫防线
        // （normal 靠 6 分噪声偶有回避，hard noise=0 恒选最高分=恒踩激进坑，红v蓝实证）
        const foeNow = state.players[1 - me];
        const s = a.t === 'attack' ? s0 - foeNow.hand.filter((c) => c.counter).length * 3.0 : s0;
        const st = cloneGame(state);
        try {
          applyAction(st, JSON.parse(JSON.stringify(a)));
          // 响应窗口自动 pass 到结算完毕
          let guard = 4;
          while (st.pending && guard-- > 0) {
            applyAction(st, { t: 'passCounter', side: st.pending.target.side });
          }
          const v = s * 1.8 + evaluate(st, me) * 0.35; // 启发分主导：中间局面估值噪声大（红v蓝实证），evaluate 权重微升拉大与乱走的差距（988 池曲线对齐后 top3+0.3 只剩 57% 分层）
          if (v > bestV) { bestV = v; best = a; }
        } catch { /* 模拟异常则跳过该动作 */ }
      }
      return best || scored[0].a;
    },
  };
}

// 局面估值（越大越好，me 视角）
function evaluate(state, me) {
  if (state.winner === me) return 9999;
  if (state.winner === 1 - me) return -9999;
  let v = 0;
  for (const side of [0, 1]) {
    const pl = state.players[side];
    const sign = side === me ? 1 : -1;
    v += sign * (pl.board.reduce((n, u) => n + powerOfUnit(u) / 1000, 0) * 2);
    v += sign * pl.hand.length * 1.6;
    // 反击牌前瞻盲区补偿：前瞻把对手响应强制 pass，但真实局高 counter 手牌会翻盘互斗/垫高防线
    v += sign * pl.hand.filter((c) => c.counter).length * 0.9;
    v += sign * (pl.lp / 2000) * 2.5; // LP 积分（游戏王式）
    v += sign * usableDons(pl) * 1.2;
    v += sign * leaderPower(pl) / 4000;
    if (pl.stage) v += sign * 2;
    // 阵型聚合度（design-system §4.3）：同阵型单位成簇=光环/触发在线，权重 0.3×单位效用
    const fc = { vanguard: 0, bulwark: 0, skirmish: 0 };
    for (const u of pl.board) if (u.formation && fc[u.formation] != null) fc[u.formation]++;
    if (pl.leader.formation && fc[pl.leader.formation] != null) fc[pl.leader.formation]++;
    for (const n of Object.values(fc)) if (n >= 2) v += sign * (n - 1) * 0.3 * 2;
  }
  return v;
}

function scoreAction(state, act) {
  const side = state.pending ? state.pending.target.side : state.active;
  const me = state.players[side];
  const foe = state.players[1 - side];

  if (state.pending) return scoreDefense(state, act, me, foe);

  switch (act.t) {
    case 'playCharacter': {
      const c = me.hand[act.idx];
      if (!c) return -99;
      let s = 40 + c.cost * 5 + c.power / 500;
      for (const kw of c.keywords || []) s += { rush: 8, blocker: 6, doubleAttack: 12, banish: 8 }[kw] || 0;
      if (c.effect) s += effValue(c.effect, state, side);
      // 阵型聚合倾向：与场上/船长同阵型成对（光环起点）加值
      if (c.formation) {
        const n = me.board.filter((u) => u.formation === c.formation).length
          + (me.leader.formation === c.formation ? 1 : 0);
        if (n >= 1) s += 6;
      }
      return s;
    }
    case 'playEvent': case 'playStage': {
      const c = me.hand[act.idx];
      if (!c) return -99;
      return 32 + c.cost * 5 + effValue(c.effect, state, side);
    }
    case 'playGear': {
      const c = me.hand[act.idx];
      const target = me.board[act.to && act.to.idx];
      if (!c || !target) return -99;
      if (target.gears && target.gears.length) return -50; // 已装备再装=旧件随葬浪费
      let s = 20 + c.cost * 2;
      if (c.gear && c.gear.atk) s += c.gear.atk / 400 + (c.gear.atk >= 3000 ? 6 : 0);
      if (c.gear && (c.gear.gives || []).includes('blocker')) s += 10; // 甲胄=坚壁防御位
      return s;
    }
    case 'fuse': {
      // F13 融合打分（线性近似）：融合体价值（战力+词条+效果）− 素材当前价值 − 融合费用机会成本
      const def = (state.fusions || []).find((c) => c && c.id === act.fusionId);
      if (!def || !def.fusion) return -99;
      let s = 40 + def.power / 500;
      for (const kw of def.keywords || []) s += { rush: 8, blocker: 6, doubleAttack: 12, banish: 8 }[kw] || 0;
      if (def.effect) s += effValue(def.effect, state, side);
      for (const id of new Set(def.fusion.from)) {
        const bu = me.board.find((u) => u.id === id);
        if (bu) s -= powerOfUnit(bu) / 500;                 // 场上素材离场=场面战力损失
        else {
          const hc = me.hand.find((c) => c.id === id);
          s -= 20 + (hc ? hc.cost * 5 + hc.power / 500 : 0); // 手牌素材进墓=资源损失（近似其打出价值）
        }
      }
      s -= def.fusion.cost * 5; // 融合贝里机会成本（与出牌费用同权）
      return s;
    }
    case 'giveDon': return 6;
    case 'takeDon': return -99; // AI 不倒腾 DON!!（与 give 互切会死循环）
    case 'attack': return scoreAttack(state, act, me, foe);
    case 'endTurn': return 0;
    default: return 0;
  }
}

function effValue(eff, state, side) {
  if (!eff) return 0;
  const foe = state.players[1 - side];
  const ops = Array.isArray(eff.op) ? eff.op : [eff.op]; // v2 复合 op（POOL-3）：逐段求和，不再整条走 default 低估
  let v = 0;
  for (const op of ops) {
    switch (op.k) {
      case 'draw': v += 9 * (op.n || 1); break;
      case 'gainDon': v += 7 * (op.n || 1); break;
      case 'koWeakest': v += foe.board.length ? 24 : 2; break;
      case 'restEnemy': v += foe.board.length ? 8 : 1; break;
      case 'powerSelf': v += 5; break;
      case 'powerLeader': v += 5; break;
      default: v += 4;
    }
  }
  return v;
}

function scoreAttack(state, act, me, foe) {
  const atkUnit = act.attacker.type === 'leader'
    ? me.leader : me.board[act.attacker.idx];
  if (!atkUnit) return -99;
  const isLeader = act.attacker.type === 'leader';
  let atk = isLeader ? leaderPower(me) : powerOfUnit(atkUnit);
  // cond 估算（对齐引擎 condMet 语义：不满足=本次不触发）——觉醒线 lpMax 判「自身」LP，
  // v0.9.4 前此处判 foe.lp 方向反了（敌方满血时白加分、自己残血反而不敢出）
  const condOk = (cond) => !cond
    || ((cond.lpMax === undefined || me.lp <= cond.lpMax)
      && (cond.foeBoardMin === undefined || foe.board.length >= cond.foeBoardMin)
      && (cond.foeBoardMax === undefined || foe.board.length <= cond.foeBoardMax)
      && (cond.myBoardMin === undefined || me.board.length >= cond.myBoardMin)
      && (cond.handMin === undefined || me.hand.length >= cond.handMin)
      && (cond.handMax === undefined || me.hand.length <= cond.handMax));
  // whenAttacking 增益预估（复合 op 数组时对 powerSelf 段求和；船长走 skills 数组）
  if (atkUnit.effect && atkUnit.effect.hook === 'whenAttacking' && condOk(atkUnit.effect.cond)) {
    const ops = Array.isArray(atkUnit.effect.op) ? atkUnit.effect.op : [atkUnit.effect.op];
    for (const op of ops) if (op.k === 'powerSelf') atk += op.x || 0;
  }
  for (const s of atkUnit.skills || []) {
    if (s.hook === 'whenAttacking' && condOk(s.cond)) {
      const ops = Array.isArray(s.op) ? s.op : [s.op];
      for (const op of ops) if (op.k === 'powerSelf') atk += op.x || 0;
    }
  }
  const estCounter = foe.hand.filter((c) => c.counter).length * 700; // 反击预期折减
  // 船长攻击技能收益（v0.9.3 修复「敌方船长永不出手」）：出手即白赚 whenAttacking 触发
  //（横置最强/直伤/全体减益——Boss 壳技能大多挂此），此前 AI 完全没计 → 船长常年按兵不动
  const skillBonus = (() => {
    let b = 0;
    for (const s of atkUnit.skills || []) {
      if (s.hook !== 'whenAttacking') continue;
      if (!condOk(s.cond)) continue; // 觉醒线/场面条件不满足=本次出手无技能收益
      const ops = Array.isArray(s.op) ? s.op : [s.op];
      for (const op of ops) {
        if (op.k === 'restEnemy') b += foe.board.length ? 6 : 1;
        else if (op.k === 'damageLP') b += 8;
      }
    }
    return b;
  })();

  if (act.target === 'leader' || act.target.type === 'leader') {
    // 直攻：伤害=差额（船长战力为防线）；无角色才可直攻
    const def = leaderPower(foe) - fruitEdge(atkUnit, foe.leader); // 果实克制抵扣防线
    let dmg = Math.max(0, atk - def - estCounter);
    if (dmg > 0 && hasKeyword(atkUnit, 'doubleAttack')) dmg *= 2;
    if (hasKeyword(atkUnit, 'banish')) dmg += 2000;
    if (dmg >= foe.lp) return 1000; // 致胜一击
    if (dmg > 0) return 24 + dmg / 400 + skillBonus;
    return isLeader ? -1 + skillBonus : -4; // 打不穿船长防线（船长攻击权免费，仅机会成本）
  }
  // 打角色（游戏王式互斗/守备；坚壁 blocker 防御 +1000）
  const victim = foe.board[act.target.idx];
  if (!victim) return -99;
  const def = powerOfUnit(victim) + (hasKeyword(victim, 'blocker') ? 1000 : 0) - fruitEdge(atkUnit, victim)
    + (victim.rest ? 0 : estCounter); // 守备表示无 Counter 加值
  if (victim.rest) {
    // 守备：打得动=击沉无伤害，打不动=无战果
    if (atk > def) return 18 + victim.power / 400;
    return isLeader ? -2 + skillBonus : -6; // 踩墙白费一次攻击（船长不损耗）
  }
  // 攻击表示互斗：差额扣 LP，攻方低则被反杀
  if (atk > def) return 20 + (atk - def) / 400 + victim.power / 400 + skillBonus;
  if (atk < def) {
    // 船长打不过不沉（combat.js koUnit 只移除 char）：损失=LP 差额 ≈2 分/千，技能收益可抵
    return isLeader ? -2 + (atk - def) / 800 + skillBonus : -10; // 角色攻方沉+扣差额
  }
  // 同归：船长不死、对方角色死=净赚（船长攻击版）
  return isLeader ? 16 + victim.power / 400 + skillBonus : -3;
}

function scoreDefense(state, act, me, foe) {
  const p = state.pending;
  const atkUnit = p.attacker.type === 'leader' ? foe.leader : foe.board[p.attacker.idx];
  const defSelf = p.target.type === 'leader' ? me.leader : me.board[p.target.idx];
  if (p.attacker.type !== 'leader' && !atkUnit) return act.t === 'pass' ? 0 : -99; // 攻击者已离场：不再投入
  const atk = (p.attacker.type === 'leader' ? leaderPower(foe) : powerOfUnit(atkUnit))
    + fruitEdge(atkUnit, defSelf); // 攻方克制我方：威胁值上浮，反击阈值随之抬高

  if (act.t === 'counter') {
    const card = me.hand[act.cards[0]];
    // 目标可能在窗口期被 whenAttacking 效果移走（koWeakest/restEnemy）——离场则防线记 0，resolveAttack 的 !def 分支收场
    const defUnit = p.target.type === 'leader' ? null : me.board[p.target.idx];
    const curDef = (p.target.type === 'leader' ? leaderPower(me) : defUnit ? powerOfUnit(defUnit) : 0)
      + (defUnit && hasKeyword(defUnit, 'blocker') ? 1000 : 0)
      + p.counterBoost;
    if (curDef + card.counter >= atk) {
      // 能翻盘才反：致死攻击必反，一般攻击看价值
      if (p.target.type === 'leader' && me.lp <= atk - curDef) return 800;
      return 22;
    }
    return -8; // 补不够就不补
  }
  // pass：被打角色高价值时不该 pass（分低），给 pass 一个参考值
  if (p.target.type === 'leader' && me.lp <= 3000) return -500;
  if (p.target.type === 'char') {
    const victim = me.board[p.target.idx];
    if (victim) return -victim.power / 800; // 高价值单位受损倾向响应
  }
  return 0;
}


// ===== data/cards.json =====
const POOL = {
  "meta": {
    "version": "0.6.0",
    "note": "OPTCG-Battle 自创卡池：仅借 OP-TCG 规则机制，数值与卡面全部自设计；海贼王角色命名仅内网自玩用途，不公开分发；v0.9.3 删除 55 个 AI 编造角色名 127 张卡（Bing 零佐证清单 data/bing-check.json）",
    "colors": {
      "red": "速攻连打：低费速攻、攻击增益、直伤去除",
      "blue": "资源循环：抽牌、高 Counter 值、费用支援",
      "green": "巨兽大怪：高费高战力、费用加速",
      "yellow": "壁垒生存：坚壁密集、领袖强化",
      "purple": "节奏掌控：费用加速、横置干扰",
      "black": "暗黑去除：击倒、Banish 直伤"
    },
    "fruit": {
      "cycle": "超人系→自然系→动物系→超人系（攻击方克制防守方，战力+1000）",
      "paramecia": "超人系",
      "logia": "自然系",
      "zoan": "动物系"
    },
    "pack": "POOL-3 扩充：+308 卡（角色244/事件38/舞台4/装备22）→ 500 卡池（2026-09-20）"
  },
  "leaders": [
    {
      "id": "LEADER-RED",
      "name": "蒙奇·D·路飞",
      "sub": "草帽盟主",
      "type": "leader",
      "color": "red",
      "power": 5000,
      "life": 5,
      "keywords": [],
      "art": "captains/luffy",
      "fruit": "paramecia",
      "skills": [
        {
          "name": "橡胶橡胶·机关枪",
          "desc": "船长攻击宣告时，本次战斗战力 +1000",
          "hook": "whenAttacking",
          "op": {
            "k": "powerSelf",
            "x": 1000,
            "until": "battle"
          }
        },
        {
          "name": "觉醒·五档 解放战士",
          "desc": "LP≤4000 时：我方回合开始抽 1 张（每回合 1 次）",
          "hook": "onTurnStart",
          "op": {
            "k": "draw",
            "n": 1
          },
          "cond": {
            "lpMax": 4000
          },
          "oncePerTurn": true
        }
      ]
    },
    {
      "id": "LEADER-BLUE",
      "name": "娜美",
      "sub": "航海士",
      "type": "leader",
      "color": "blue",
      "power": 5000,
      "life": 5,
      "keywords": [],
      "art": "captains/nami",
      "fruit": null,
      "skills": [
        {
          "name": "天候棒·雷云",
          "desc": "己方费用≥2 角色登场时抽 1 张",
          "hook": "onSummon",
          "op": {
            "k": "draw",
            "n": 1,
            "minCost": 2
          }
        },
        {
          "name": "觉醒·Zeus 雷云",
          "desc": "LP≤4000 时：船长攻击宣告时对方 LP 直接伤害 1000",
          "hook": "whenAttacking",
          "op": {
            "k": "damageLP",
            "x": 1000
          },
          "cond": {
            "lpMax": 4000
          }
        }
      ]
    },
    {
      "id": "LEADER-GREEN",
      "name": "罗罗诺亚·索隆",
      "sub": "海贼猎人",
      "type": "leader",
      "color": "green",
      "power": 5000,
      "life": 5,
      "keywords": [],
      "art": "captains/zoro",
      "fruit": null,
      "skills": [
        {
          "name": "三刀流·鬼气",
          "desc": "己方费用≥7 角色登场时，该角色永久战力 +1000",
          "hook": "onSummon",
          "op": {
            "k": "powerSelf",
            "x": 1000,
            "minCost": 7,
            "until": "forever"
          }
        },
        {
          "name": "觉醒·阿修罗 一雾银",
          "desc": "LP≤4000 时：船长攻击宣告时本次战斗战力 +2000",
          "hook": "whenAttacking",
          "op": {
            "k": "powerSelf",
            "x": 2000,
            "until": "battle"
          },
          "cond": {
            "lpMax": 4000
          }
        }
      ]
    },
    {
      "id": "LEADER-YELLOW",
      "name": "山治",
      "sub": "黑足",
      "type": "leader",
      "color": "yellow",
      "power": 5000,
      "life": 5,
      "keywords": [],
      "art": "captains/sanji",
      "fruit": null,
      "skills": [
        {
          "name": "宴会料理",
          "desc": "己方角色被击沉时 LP 回复 1000（每回合 1 次：主厨一次只上一道菜）",
          "hook": "onAllyKO",
          "op": {
            "k": "healLP",
            "x": 1000
          },
          "oncePerTurn": true
        },
        {
          "name": "觉醒·地狱记忆",
          "desc": "LP≤4000 时：我方回合开始 LP 回复 1000（每回合 1 次）",
          "hook": "onTurnStart",
          "op": {
            "k": "healLP",
            "x": 1000
          },
          "cond": {
            "lpMax": 4000
          },
          "oncePerTurn": true
        }
      ]
    },
    {
      "id": "LEADER-PURPLE",
      "name": "特拉法尔加·罗",
      "sub": "死之外科医生",
      "type": "leader",
      "color": "purple",
      "power": 5500,
      "life": 4,
      "keywords": [],
      "art": "captains/law",
      "fruit": "paramecia",
      "skills": [
        {
          "name": "手术果实·麻醉枪",
          "desc": "船长攻击宣告时：横置对方战力最高的角色，且本次战斗战力 +1000（ROOM 内点穴麻醉）",
          "hook": "whenAttacking",
          "op": [
            {
              "k": "restEnemy",
              "target": "strongest"
            },
            {
              "k": "powerSelf",
              "x": 1000,
              "until": "battle"
            }
          ]
        },
        {
          "name": "ROOM·回收",
          "desc": "击沉对方角色时抽 1 张（每回合 1 次）",
          "hook": "onKill",
          "op": {
            "k": "draw",
            "n": 1
          },
          "oncePerTurn": true
        },
        {
          "name": "觉醒·死之外科医生",
          "desc": "LP≤6000 时：船长攻击宣言时本次战斗战力 +2000",
          "hook": "whenAttacking",
          "op": {
            "k": "powerSelf",
            "x": 2000,
            "until": "battle"
          },
          "cond": {
            "lpMax": 6000
          }
        }
      ]
    },
    {
      "id": "LEADER-BLACK",
      "name": "香克斯",
      "sub": "红发",
      "type": "leader",
      "color": "black",
      "power": 5000,
      "life": 6,
      "keywords": [],
      "art": "captains/shanks",
      "fruit": null,
      "skills": [
        {
          "name": "霸王色的威压",
          "desc": "船长攻击宣告时：对方全体角色本次战斗战力 -1000",
          "hook": "whenAttacking",
          "op": {
            "k": "debuffFoeAll",
            "x": 1000,
            "until": "battle"
          }
        }
      ]
    },
    {
      "id": "LEADER-RED2",
      "name": "波特卡斯·D·艾斯",
      "sub": "火拳",
      "type": "leader",
      "color": "red",
      "power": 5000,
      "life": 5,
      "keywords": [],
      "art": "captains/ace",
      "fruit": "logia",
      "skills": [
        {
          "name": "火拳",
          "desc": "船长攻击宣告时：弃 1 张手牌，本次战斗战力 +2000（手牌不足则不生效）",
          "hook": "whenAttacking",
          "op": [
            {
              "k": "discard",
              "n": 1
            },
            {
              "k": "powerSelf",
              "x": 2000,
              "until": "battle"
            }
          ]
        },
        {
          "name": "觉醒·炎帝",
          "desc": "LP≤4000 时：船长攻击宣告时对方 LP 直接伤害 1000",
          "hook": "whenAttacking",
          "op": {
            "k": "damageLP",
            "x": 1000
          },
          "cond": {
            "lpMax": 4000
          }
        }
      ]
    },
    {
      "id": "LEADER-BLUE2",
      "name": "妮可·罗宾",
      "sub": "恶魔之子",
      "type": "leader",
      "color": "blue",
      "power": 5000,
      "life": 5,
      "keywords": [],
      "art": "captains/robin",
      "fruit": "paramecia",
      "skills": [
        {
          "name": "百花缭乱",
          "desc": "己方费用≥4 角色登场时抽 1 张",
          "hook": "onSummon",
          "op": {
            "k": "draw",
            "n": 1,
            "minCost": 4
          }
        },
        {
          "name": "觉醒·万手压制",
          "desc": "LP≤4000 时：船长攻击宣告时横置对方战力最低的角色",
          "hook": "whenAttacking",
          "op": {
            "k": "restEnemy",
            "target": "weakest"
          },
          "cond": {
            "lpMax": 4000
          }
        }
      ]
    },
    {
      "id": "LEADER-GREEN2",
      "name": "凯多",
      "sub": "最强生物",
      "type": "leader",
      "color": "green",
      "power": 5000,
      "life": 6,
      "keywords": [],
      "art": "captains/kaido",
      "fruit": "zoan",
      "skills": [
        {
          "name": "雷鸣八卦",
          "desc": "击沉对方角色时：对方 LP 额外伤害 1000（每回合 1 次）",
          "hook": "onKill",
          "op": {
            "k": "damageLP",
            "x": 1000
          },
          "oncePerTurn": true
        }
      ]
    },
    {
      "id": "LEADER-YELLOW2",
      "name": "波雅·汉库珂",
      "sub": "蛇姬",
      "type": "leader",
      "color": "yellow",
      "power": 5500,
      "life": 4,
      "keywords": [],
      "art": "captains/hancock",
      "fruit": "paramecia",
      "skills": [
        {
          "name": "虏之箭",
          "desc": "船长攻击宣告时：横置对方战力最高的角色（甜美魅惑·石化）",
          "hook": "whenAttacking",
          "op": {
            "k": "restEnemy",
            "target": "strongest"
          }
        },
        {
          "name": "霸王色的魅力",
          "desc": "己方单位被攻击时：船长本次战斗战力 +1000（每回合 1 次）",
          "hook": "whenAttacked",
          "op": {
            "k": "powerLeader",
            "x": 1000,
            "until": "battle"
          },
          "oncePerTurn": true
        },
        {
          "name": "觉醒·蛇姬之怒",
          "desc": "LP≤6000 时：船长攻击宣告时对方 LP 直接伤害 1000",
          "hook": "whenAttacking",
          "op": {
            "k": "damageLP",
            "x": 1000
          },
          "cond": {
            "lpMax": 6000
          }
        }
      ]
    },
    {
      "id": "LEADER-PURPLE2",
      "name": "唐吉诃德·多弗朗明戈",
      "sub": "天夜叉",
      "type": "leader",
      "color": "purple",
      "power": 5500,
      "life": 4,
      "keywords": [],
      "art": "captains/doflamingo",
      "fruit": "paramecia",
      "skills": [
        {
          "name": "天夜叉的情报网",
          "desc": "我方回合开始时抽 1 张牌",
          "hook": "onTurnStart",
          "op": {
            "k": "draw",
            "n": 1
          }
        },
        {
          "name": "寄生线",
          "desc": "船长攻击宣告时：对方全体角色本次战斗战力 -1000",
          "hook": "whenAttacking",
          "op": {
            "k": "debuffFoeAll",
            "x": 1000,
            "until": "battle"
          }
        },
        {
          "name": "觉醒·鸟笼",
          "desc": "LP≤6000 时：我方回合开始额外获得 1 张贝里（每回合 1 次）",
          "hook": "onTurnStart",
          "op": {
            "k": "gainDon",
            "n": 1
          },
          "cond": {
            "lpMax": 6000
          },
          "oncePerTurn": true
        }
      ]
    },
    {
      "id": "LEADER-BLACK2",
      "name": "乔拉可尔·米霍克",
      "sub": "鹰眼",
      "type": "leader",
      "color": "black",
      "power": 5000,
      "life": 6,
      "keywords": [],
      "art": "captains/mihawk",
      "fruit": null,
      "skills": [
        {
          "name": "黑刀·一骑讨",
          "desc": "对方场上角色≤1 时：船长攻击宣告时本次战斗战力 +2000",
          "hook": "whenAttacking",
          "op": {
            "k": "powerSelf",
            "x": 2000,
            "until": "battle"
          },
          "cond": {
            "foeBoardMax": 1
          }
        }
      ]
    }
  ],
  "cards": [
    {
      "id": "BLACK-01",
      "name": "毒Q",
      "sub": " 死神 ",
      "type": "char",
      "color": "black",
      "cost": 1,
      "power": 2000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "BLACK-01",
      "fruit": null,
      "rarity": "A",
      "faction": "beast",
      "formation": null,
      "arc": "marineford"
    },
    {
      "id": "BLACK-02",
      "name": "拉菲特",
      "sub": " 五号船船长 ",
      "type": "char",
      "color": "black",
      "cost": 2,
      "power": 3000,
      "counter": 2000,
      "keywords": [],
      "effect": null,
      "art": "BLACK-02",
      "fruit": "zoan",
      "rarity": "A",
      "faction": "beast",
      "formation": null,
      "arc": "marineford"
    },
    {
      "id": "BLACK-07",
      "name": "卡特琳娜·蝶美",
      "sub": " 月牙猎人 ",
      "type": "char",
      "color": "black",
      "cost": 6,
      "power": 6000,
      "counter": 2000,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "BLACK-07",
      "fruit": null,
      "rarity": "B",
      "faction": "beast",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "BLACK-08",
      "name": "雨之希留",
      "sub": " 雨 ",
      "type": "char",
      "color": "black",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "BLACK-08",
      "fruit": null,
      "rarity": "B",
      "faction": "beast",
      "formation": "vanguard",
      "arc": "marineford"
    },
    {
      "id": "BLACK-12",
      "name": "汉尼拔",
      "sub": " 因佩尔副署长 ",
      "type": "char",
      "color": "black",
      "cost": 5,
      "power": 5000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "fruit": null,
      "art": "BLACK-12",
      "rarity": "B",
      "faction": "navy",
      "formation": "bulwark",
      "arc": "marineford"
    },
    {
      "id": "BLACK-15",
      "name": "小萨蒂",
      "sub": " 地狱看守长 ",
      "type": "char",
      "color": "black",
      "cost": 4,
      "power": 5000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "fruit": null,
      "art": "BLACK-15",
      "rarity": "A",
      "faction": "navy",
      "formation": null,
      "arc": "marineford"
    },
    {
      "id": "BLACK-16",
      "name": "本·贝克曼",
      "sub": " 红发副船长 ",
      "type": "char",
      "color": "black",
      "cost": 5,
      "power": 6000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "fruit": null,
      "art": "BLACK-16",
      "rarity": "A",
      "faction": "yonko",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLACK-18",
      "name": "拉基·路",
      "sub": " 红发干部 ",
      "type": "char",
      "color": "black",
      "cost": 2,
      "power": 3000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "fruit": null,
      "art": "BLACK-18",
      "rarity": "A",
      "faction": "yonko",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLACK-19",
      "name": "钢骨·空",
      "sub": " 海军元帅 ",
      "type": "char",
      "color": "black",
      "cost": 7,
      "power": 8000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "fruit": null,
      "art": "BLACK-19",
      "rarity": "S",
      "faction": "navy",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLACK-20",
      "name": "鬼蜘蛛",
      "sub": " 海军中将 ",
      "type": "char",
      "color": "black",
      "cost": 5,
      "power": 6000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "fruit": null,
      "art": "BLACK-20",
      "rarity": "A",
      "faction": "navy",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLACK-21",
      "name": "鼯鼠",
      "sub": " 海军中将 ",
      "type": "char",
      "color": "black",
      "cost": 4,
      "power": 5000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "fruit": null,
      "art": "BLACK-21",
      "rarity": "A",
      "faction": "navy",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLACK-22",
      "name": "火烧山",
      "sub": " 海军中将 ",
      "type": "char",
      "color": "black",
      "cost": 3,
      "power": 4000,
      "counter": 2000,
      "keywords": [],
      "effect": null,
      "fruit": null,
      "art": "BLACK-22",
      "rarity": "A",
      "faction": "navy",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLACK-23",
      "name": "范德戴肯",
      "sub": " 靶靶果实 ",
      "type": "char",
      "color": "black",
      "cost": 5,
      "power": 6000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "fruit": "paramecia",
      "art": "BLACK-23",
      "rarity": "A",
      "faction": "beast",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLACK-26",
      "name": "马歇尔·D·蒂奇",
      "sub": " 黑暗果实·吸收 ",
      "type": "char",
      "color": "black",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "powerSelf",
          "x": 1000,
          "until": "battle"
        }
      },
      "art": "BLACK-26",
      "fruit": "logia",
      "rarity": "B",
      "faction": "beast",
      "formation": null,
      "arc": "marineford"
    },
    {
      "id": "BLACK-31",
      "name": "麦哲伦",
      "sub": " 毒龙·地狱审判 ",
      "type": "char",
      "color": "black",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [
        "banish"
      ],
      "effect": null,
      "art": "BLACK-31",
      "fruit": "paramecia",
      "rarity": "S",
      "faction": "navy",
      "formation": "vanguard",
      "arc": "marineford"
    },
    {
      "id": "BLACK-33",
      "name": "耶稣布",
      "sub": " 神射手 ",
      "type": "char",
      "color": "black",
      "cost": 5,
      "power": 5000,
      "counter": null,
      "keywords": [],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "damageLP",
          "x": 1000
        }
      },
      "art": "BLACK-33",
      "fruit": null,
      "rarity": "B",
      "faction": "yonko",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLACK-36",
      "name": "范·奥卡",
      "sub": " 音速弹 ",
      "type": "char",
      "color": "black",
      "cost": 2,
      "power": 3000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "BLACK-36",
      "fruit": null,
      "rarity": "A",
      "faction": "beast",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLACK-40",
      "name": "巴加斯",
      "sub": " 格斗冠军 ",
      "type": "char",
      "color": "black",
      "cost": 2,
      "power": 3000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "BLACK-40",
      "fruit": null,
      "rarity": "A",
      "faction": "beast",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLACK-41",
      "name": "阿巴罗·比萨罗",
      "sub": " 晴岚之王 ",
      "type": "char",
      "color": "black",
      "cost": 4,
      "power": 5000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "BLACK-41",
      "fruit": null,
      "rarity": "A",
      "faction": "beast",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLACK-45",
      "name": "圣胡安·恶狼",
      "sub": " 黑团巨舰 ",
      "type": "char",
      "color": "black",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "BLACK-45",
      "fruit": null,
      "rarity": "B",
      "faction": "beast",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "BLACK-47",
      "name": "泽法",
      "sub": " 黑腕·海楼石 ",
      "type": "char",
      "color": "black",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "debuffFoeAll",
          "x": 1000,
          "until": "battle"
        }
      },
      "art": "BLACK-47",
      "fruit": null,
      "rarity": "B",
      "faction": "navy",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLACK-50",
      "name": "布鲁诺",
      "sub": " 空气门 ",
      "type": "char",
      "color": "black",
      "cost": 4,
      "power": 4000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "BLACK-50",
      "fruit": "paramecia",
      "rarity": "B",
      "faction": "navy",
      "formation": "bulwark",
      "arc": "enies_lobby"
    },
    {
      "id": "BLACK-51",
      "name": "达兹·波涅斯",
      "sub": " 刀刃人 ",
      "type": "char",
      "color": "black",
      "cost": 5,
      "power": 6000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "BLACK-51",
      "fruit": "paramecia",
      "rarity": "A",
      "faction": "warlord",
      "formation": null,
      "arc": "alabasta"
    },
    {
      "id": "BLACK-52",
      "name": "贝比5",
      "sub": " 武器果实 ",
      "type": "char",
      "color": "black",
      "cost": 3,
      "power": 4000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "BLACK-52",
      "fruit": "paramecia",
      "rarity": "A",
      "faction": "warlord",
      "formation": null,
      "arc": "dressrosa"
    },
    {
      "id": "BLACK-54",
      "name": "黑炭大蛇",
      "sub": " 八岐大蛇 ",
      "type": "char",
      "color": "black",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "BLACK-54",
      "fruit": "zoan",
      "rarity": "B",
      "faction": "beast",
      "formation": "bulwark",
      "arc": "wano"
    },
    {
      "id": "BLACK-57",
      "name": "洛克斯·D·吉贝克",
      "sub": " 神之谷对决 ",
      "type": "char",
      "color": "black",
      "cost": 6,
      "power": 7000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "BLACK-57",
      "fruit": null,
      "rarity": "A",
      "faction": "yonko",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLACK-60",
      "name": "多米诺",
      "sub": " 监视之眼 ",
      "type": "char",
      "color": "black",
      "cost": 1,
      "power": 2000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "BLACK-60",
      "fruit": null,
      "rarity": "A",
      "faction": "navy",
      "formation": null,
      "arc": "marineford"
    },
    {
      "id": "BLACK-61",
      "name": "斧手蒙卡",
      "sub": " 东海上校 ",
      "type": "char",
      "color": "black",
      "cost": 1,
      "power": 2000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "BLACK-61",
      "fruit": null,
      "rarity": "A",
      "faction": "navy",
      "formation": null,
      "arc": "east_blue"
    },
    {
      "id": "BLACK-62",
      "name": "Mr.4",
      "sub": " 四号特工 ",
      "type": "char",
      "color": "black",
      "cost": 3,
      "power": 4000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "BLACK-62",
      "fruit": null,
      "rarity": "A",
      "faction": "warlord",
      "formation": null,
      "arc": "alabasta"
    },
    {
      "id": "BLACK-63",
      "name": "Miss圣诞快乐",
      "sub": " 巴洛克工作社 ",
      "type": "char",
      "color": "black",
      "cost": 2,
      "power": 3000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "BLACK-63",
      "fruit": null,
      "rarity": "A",
      "faction": "warlord",
      "formation": null,
      "arc": "alabasta"
    },
    {
      "id": "BLACK-64",
      "name": "Miss双手指",
      "sub": " 荆棘果实 ",
      "type": "char",
      "color": "black",
      "cost": 3,
      "power": 4000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "BLACK-64",
      "fruit": "paramecia",
      "rarity": "A",
      "faction": "warlord",
      "formation": null,
      "arc": "alabasta"
    },
    {
      "id": "BLACK-G1",
      "name": "狙击镜",
      "sub": " 红发狙击手 ",
      "type": "gear",
      "color": "black",
      "cost": 2,
      "power": null,
      "counter": null,
      "keywords": [],
      "gear": {
        "atk": 1000
      },
      "effect": null,
      "fruit": null,
      "art": "BLACK-G1",
      "rarity": "A",
      "faction": "beast",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLACK-G2",
      "name": "黑刀·初代鬼彻",
      "sub": " 妖刀一文字 ",
      "type": "gear",
      "color": "black",
      "cost": 3,
      "power": null,
      "counter": null,
      "keywords": [],
      "gear": {
        "atk": 2000,
        "gives": [
          "blocker"
        ]
      },
      "effect": null,
      "fruit": null,
      "art": "BLACK-G2",
      "rarity": "B",
      "faction": "beast",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLACK-G3",
      "name": "雷雨",
      "sub": " 希留爱用的妖刀 ",
      "type": "gear",
      "color": "black",
      "cost": 2,
      "power": null,
      "counter": null,
      "keywords": [],
      "gear": {
        "atk": 2000
      },
      "effect": null,
      "art": "BLACK-G3",
      "fruit": null,
      "rarity": "B",
      "faction": "beast",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLACK-G6",
      "name": "蒙卡的大斧",
      "sub": " 斧手蒙卡爱斧 ",
      "type": "gear",
      "color": "black",
      "cost": 2,
      "power": null,
      "counter": null,
      "keywords": [],
      "gear": {
        "atk": 2000
      },
      "effect": null,
      "art": "BLACK-G6",
      "fruit": null,
      "rarity": "B",
      "faction": "beast",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLUE-01",
      "name": "哈库",
      "sub": " 鱼人空手道 ",
      "type": "char",
      "color": "blue",
      "cost": 1,
      "power": 2000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "BLUE-01",
      "fruit": null,
      "rarity": "A",
      "faction": "strawhat",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLUE-02",
      "name": "达斯琪",
      "sub": " 海军上校 ",
      "type": "char",
      "color": "blue",
      "cost": 1,
      "power": 2000,
      "counter": 2000,
      "keywords": [],
      "effect": null,
      "art": "BLUE-02",
      "fruit": null,
      "rarity": "A",
      "faction": "navy",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLUE-03",
      "name": "佩德罗",
      "sub": " 骑士团团长 ",
      "type": "char",
      "color": "blue",
      "cost": 2,
      "power": 3000,
      "counter": 2000,
      "keywords": [],
      "effect": null,
      "art": "BLUE-03",
      "fruit": null,
      "rarity": "A",
      "faction": "whitebeard",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLUE-04",
      "name": "罗宾",
      "sub": " 恶魔之子 ",
      "type": "char",
      "color": "blue",
      "cost": 2,
      "power": 3000,
      "counter": null,
      "keywords": [],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "draw",
          "n": 1
        }
      },
      "art": "BLUE-04",
      "fruit": "paramecia",
      "rarity": "B",
      "faction": "strawhat",
      "formation": null,
      "arc": "enies_lobby"
    },
    {
      "id": "BLUE-05",
      "name": "猫蝮蛇",
      "sub": " 摩科莫公国 ",
      "type": "char",
      "color": "blue",
      "cost": 3,
      "power": 4000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "BLUE-05",
      "fruit": null,
      "rarity": "A",
      "faction": "whitebeard",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLUE-06",
      "name": "犬岚公爵",
      "sub": " 摩科莫公国 ",
      "type": "char",
      "color": "blue",
      "cost": 3,
      "power": 4000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "BLUE-06",
      "fruit": null,
      "rarity": "A",
      "faction": "whitebeard",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLUE-08",
      "name": "弗兰奇",
      "sub": " 铁人 ",
      "type": "char",
      "color": "blue",
      "cost": 5,
      "power": 6000,
      "counter": null,
      "keywords": [],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "gainDon",
          "n": 1
        }
      },
      "art": "BLUE-08",
      "fruit": null,
      "rarity": "B",
      "faction": "strawhat",
      "formation": null,
      "arc": "enies_lobby"
    },
    {
      "id": "BLUE-09",
      "name": "甚平",
      "sub": " 海侠 ",
      "type": "char",
      "color": "blue",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "BLUE-09",
      "fruit": null,
      "rarity": "B",
      "faction": "strawhat",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "BLUE-10",
      "name": "青雉",
      "sub": " 库赞 ",
      "type": "char",
      "color": "blue",
      "cost": 7,
      "power": 8000,
      "counter": null,
      "keywords": [],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "restEnemy"
        }
      },
      "art": "BLUE-10",
      "fruit": "logia",
      "rarity": "S",
      "faction": "navy",
      "formation": null,
      "arc": "marineford"
    },
    {
      "id": "BLUE-11",
      "name": "卡普",
      "sub": " 海军英雄 ",
      "type": "char",
      "color": "blue",
      "cost": 5,
      "power": 6000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "fruit": null,
      "art": "BLUE-11",
      "rarity": "A",
      "faction": "navy",
      "formation": null,
      "arc": "marineford"
    },
    {
      "id": "BLUE-13",
      "name": "白星",
      "sub": " 鱼人岛公主 ",
      "type": "char",
      "color": "blue",
      "cost": 3,
      "power": 4000,
      "counter": 2000,
      "keywords": [],
      "effect": null,
      "fruit": null,
      "art": "BLUE-13",
      "rarity": "A",
      "faction": "navy",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLUE-14",
      "name": "尼普顿",
      "sub": " 龙宫王 ",
      "type": "char",
      "color": "blue",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "fruit": null,
      "art": "BLUE-14",
      "rarity": "B",
      "faction": "navy",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "BLUE-15",
      "name": "巴利",
      "sub": " 卡雷拉一号船坞 ",
      "type": "char",
      "color": "blue",
      "cost": 4,
      "power": 5000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "fruit": null,
      "art": "BLUE-15",
      "rarity": "A",
      "faction": "navy",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLUE-16",
      "name": "斯潘达姆",
      "sub": " CP9 长官 ",
      "type": "char",
      "color": "blue",
      "cost": 1,
      "power": 2000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "fruit": null,
      "art": "BLUE-16",
      "rarity": "A",
      "faction": "navy",
      "formation": null,
      "arc": "enies_lobby"
    },
    {
      "id": "BLUE-17",
      "name": "加布拉",
      "sub": " 狗狗果实·狼 ",
      "type": "char",
      "color": "blue",
      "cost": 5,
      "power": 6000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "fruit": "zoan",
      "art": "BLUE-17",
      "rarity": "A",
      "faction": "navy",
      "formation": null,
      "arc": "enies_lobby"
    },
    {
      "id": "BLUE-18",
      "name": "卡库",
      "sub": " 牛牛果实·长颈鹿 ",
      "type": "char",
      "color": "blue",
      "cost": 6,
      "power": 7000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "fruit": "zoan",
      "art": "BLUE-18",
      "rarity": "A",
      "faction": "navy",
      "formation": null,
      "arc": "enies_lobby"
    },
    {
      "id": "BLUE-21",
      "name": "卡莉法",
      "sub": " CP9 唯一女性 ",
      "type": "char",
      "color": "blue",
      "cost": 3,
      "power": 4000,
      "counter": 2000,
      "keywords": [],
      "effect": null,
      "fruit": "paramecia",
      "art": "BLUE-21",
      "rarity": "A",
      "faction": "navy",
      "formation": null,
      "arc": "enies_lobby"
    },
    {
      "id": "BLUE-22",
      "name": "赫尔梅普",
      "sub": " 海军上校之子 ",
      "type": "char",
      "color": "blue",
      "cost": 2,
      "power": 3000,
      "counter": 2000,
      "keywords": [],
      "effect": null,
      "fruit": null,
      "art": "BLUE-22",
      "rarity": "A",
      "faction": "navy",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLUE-23",
      "name": "赞高",
      "sub": " 催眠果实 ",
      "type": "char",
      "color": "blue",
      "cost": 1,
      "power": 2000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "fruit": "paramecia",
      "art": "BLUE-23",
      "rarity": "A",
      "faction": "navy",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLUE-25",
      "name": "娜美",
      "sub": " 天候棒·雷云 ",
      "type": "char",
      "color": "blue",
      "cost": 3,
      "power": 3000,
      "counter": 1000,
      "keywords": [],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "draw",
          "n": 1
        }
      },
      "art": "BLUE-25",
      "fruit": null,
      "rarity": "B",
      "faction": "strawhat",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLUE-30",
      "name": "波尔萨利诺",
      "sub": " 天丛云剑 ",
      "type": "char",
      "color": "blue",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "powerSelf",
          "x": 1000,
          "until": "battle"
        }
      },
      "art": "BLUE-30",
      "fruit": "logia",
      "rarity": "S",
      "faction": "navy",
      "formation": null,
      "arc": "marineford"
    },
    {
      "id": "BLUE-34",
      "name": "斯摩格",
      "sub": " 海军G-5支部 ",
      "type": "char",
      "color": "blue",
      "cost": 4,
      "power": 4000,
      "counter": 1000,
      "keywords": [],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "draw",
          "n": 1
        }
      },
      "art": "BLUE-34",
      "fruit": "logia",
      "rarity": "B",
      "faction": "navy",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLUE-36",
      "name": "战国",
      "sub": " 佛之推波 ",
      "type": "char",
      "color": "blue",
      "cost": 5,
      "power": 5000,
      "counter": null,
      "keywords": [],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "buffAll",
          "x": 1000,
          "until": "turn"
        }
      },
      "art": "BLUE-36",
      "fruit": "paramecia",
      "rarity": "B",
      "faction": "navy",
      "formation": null,
      "arc": "marineford"
    },
    {
      "id": "BLUE-41",
      "name": "克比",
      "sub": " 海军大佐 ",
      "type": "char",
      "color": "blue",
      "cost": 2,
      "power": 3000,
      "counter": 2000,
      "keywords": [],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "draw",
          "n": 1
        }
      },
      "art": "BLUE-41",
      "fruit": null,
      "rarity": "B",
      "faction": "navy",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLUE-46",
      "name": "乙姬",
      "sub": " 王妃的祈愿 ",
      "type": "char",
      "color": "blue",
      "cost": 3,
      "power": 3000,
      "counter": 2000,
      "keywords": [],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "healLP",
          "x": 1000
        }
      },
      "art": "BLUE-46",
      "fruit": null,
      "rarity": "B",
      "faction": "navy",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLUE-47",
      "name": "冰山",
      "sub": " 水之都市长 ",
      "type": "char",
      "color": "blue",
      "cost": 2,
      "power": 3000,
      "counter": 2000,
      "keywords": [],
      "effect": null,
      "art": "BLUE-47",
      "fruit": null,
      "rarity": "A",
      "faction": "navy",
      "formation": null,
      "arc": "enies_lobby"
    },
    {
      "id": "BLUE-48",
      "name": "顿·克利克",
      "sub": " 无敌舰队 ",
      "type": "char",
      "color": "blue",
      "cost": 4,
      "power": 5000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "BLUE-48",
      "fruit": null,
      "rarity": "A",
      "faction": "navy",
      "formation": null,
      "arc": "east_blue"
    },
    {
      "id": "BLUE-49",
      "name": "阿金",
      "sub": " 鬼人 ",
      "type": "char",
      "color": "blue",
      "cost": 2,
      "power": 3000,
      "counter": 2000,
      "keywords": [],
      "effect": null,
      "art": "BLUE-49",
      "fruit": null,
      "rarity": "A",
      "faction": "navy",
      "formation": null,
      "arc": "east_blue"
    },
    {
      "id": "BLUE-50",
      "name": "妮可·罗宾",
      "sub": " 万紫千红 ",
      "type": "char",
      "color": "blue",
      "cost": 4,
      "power": 4000,
      "counter": 1000,
      "keywords": [],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "restEnemy",
          "target": "last"
        }
      },
      "art": "BLUE-50",
      "fruit": "paramecia",
      "rarity": "B",
      "faction": "strawhat",
      "formation": null,
      "arc": "enies_lobby"
    },
    {
      "id": "BLUE-59",
      "name": "马尔科",
      "sub": " 再生之炎 ",
      "type": "char",
      "color": "blue",
      "cost": 5,
      "power": 6000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onKO",
        "op": {
          "k": "healLP",
          "x": 1000
        }
      },
      "art": "BLUE-59",
      "fruit": "paramecia",
      "rarity": "S",
      "faction": "whitebeard",
      "formation": "bulwark",
      "arc": "marineford"
    },
    {
      "id": "BLUE-61",
      "name": "寇布拉",
      "sub": " 阿拉巴斯坦国王 ",
      "type": "char",
      "color": "blue",
      "cost": 2,
      "power": 3000,
      "counter": 2000,
      "keywords": [],
      "effect": null,
      "art": "BLUE-61",
      "fruit": null,
      "rarity": "A",
      "faction": "navy",
      "formation": null,
      "arc": "alabasta"
    },
    {
      "id": "BLUE-62",
      "name": "伊卡莱姆",
      "sub": " 侍卫队长 ",
      "type": "char",
      "color": "blue",
      "cost": 1,
      "power": 2000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "BLUE-62",
      "fruit": null,
      "rarity": "A",
      "faction": "navy",
      "formation": null,
      "arc": "alabasta"
    },
    {
      "id": "BLUE-63",
      "name": "布鲁克",
      "sub": " 灵魂之王 ",
      "type": "char",
      "color": "blue",
      "cost": 3,
      "power": 3000,
      "counter": 1000,
      "keywords": [],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "draw",
          "n": 1
        }
      },
      "art": "BLUE-63",
      "fruit": "paramecia",
      "rarity": "B",
      "faction": "strawhat",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLUE-G1",
      "name": "时雨",
      "sub": " 良业物 ",
      "type": "gear",
      "color": "blue",
      "cost": 2,
      "power": null,
      "counter": null,
      "keywords": [],
      "gear": {
        "atk": 2000
      },
      "effect": null,
      "fruit": null,
      "art": "BLUE-G1",
      "rarity": "B",
      "faction": "navy",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLUE-G2",
      "name": "六式·铁块",
      "sub": " 钢铁之躯 ",
      "type": "gear",
      "color": "blue",
      "cost": 2,
      "power": null,
      "counter": null,
      "keywords": [],
      "gear": {
        "atk": 1000,
        "gives": [
          "blocker"
        ]
      },
      "effect": null,
      "fruit": null,
      "art": "BLUE-G2",
      "rarity": "A",
      "faction": "navy",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLUE-G3",
      "name": "花州",
      "sub": " 达斯琪的名刀 ",
      "type": "gear",
      "color": "blue",
      "cost": 2,
      "power": null,
      "counter": null,
      "keywords": [],
      "gear": {
        "atk": 2000
      },
      "effect": null,
      "art": "BLUE-G3",
      "fruit": null,
      "rarity": "B",
      "faction": "navy",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLUE-G4",
      "name": "冰军刀",
      "sub": " 青雉的冰刃 ",
      "type": "gear",
      "color": "blue",
      "cost": 2,
      "power": null,
      "counter": null,
      "keywords": [],
      "gear": {
        "atk": 2000
      },
      "effect": null,
      "art": "BLUE-G4",
      "fruit": null,
      "rarity": "B",
      "faction": "navy",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLUE-G5",
      "name": "海军披风",
      "sub": " 正义之袍 ",
      "type": "gear",
      "color": "blue",
      "cost": 2,
      "power": null,
      "counter": null,
      "keywords": [],
      "gear": {
        "atk": 1000
      },
      "effect": null,
      "art": "BLUE-G5",
      "fruit": null,
      "rarity": "A",
      "faction": "navy",
      "formation": null,
      "arc": null
    },
    {
      "id": "GREEN-01",
      "name": "乔巴",
      "sub": " 妹尾的驯鹿 ",
      "type": "char",
      "color": "green",
      "cost": 1,
      "power": 2000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "GREEN-01",
      "fruit": "paramecia",
      "rarity": "A",
      "faction": "strawhat",
      "formation": null,
      "arc": null
    },
    {
      "id": "GREEN-02",
      "name": "雷藏",
      "sub": " 忍者 ",
      "type": "char",
      "color": "green",
      "cost": 2,
      "power": 3000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "GREEN-02",
      "fruit": null,
      "rarity": "A",
      "faction": "strawhat",
      "formation": null,
      "arc": "wano"
    },
    {
      "id": "GREEN-03",
      "name": "以藏",
      "sub": " 白胡子海贼团 ",
      "type": "char",
      "color": "green",
      "cost": 3,
      "power": 3000,
      "counter": 2000,
      "keywords": [],
      "effect": null,
      "art": "GREEN-03",
      "fruit": null,
      "rarity": "A",
      "faction": "strawhat",
      "formation": null,
      "arc": "wano"
    },
    {
      "id": "GREEN-04",
      "name": "传次郎",
      "sub": " 赤鞘九侠 ",
      "type": "char",
      "color": "green",
      "cost": 4,
      "power": 4000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "GREEN-04",
      "fruit": null,
      "rarity": "B",
      "faction": "strawhat",
      "formation": "vanguard",
      "arc": "wano"
    },
    {
      "id": "GREEN-05",
      "name": "杰克",
      "sub": " 猛火 ",
      "type": "char",
      "color": "green",
      "cost": 5,
      "power": 5000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "GREEN-05",
      "fruit": "paramecia",
      "rarity": "A",
      "faction": "beast",
      "formation": null,
      "arc": "wano"
    },
    {
      "id": "GREEN-06",
      "name": "马尔科",
      "sub": " 不死鸟 ",
      "type": "char",
      "color": "green",
      "cost": 5,
      "power": 5000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "GREEN-06",
      "fruit": "paramecia",
      "rarity": "B",
      "faction": "whitebeard",
      "formation": "bulwark",
      "arc": "marineford"
    },
    {
      "id": "GREEN-08",
      "name": "金",
      "sub": " 灾火 ",
      "type": "char",
      "color": "green",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [
        "doubleAttack"
      ],
      "effect": null,
      "art": "GREEN-08",
      "fruit": "zoan",
      "rarity": "S",
      "faction": "beast",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "GREEN-10",
      "name": "白胡子",
      "sub": " 爱德华·纽盖特 ",
      "type": "char",
      "color": "green",
      "cost": 8,
      "power": 8000,
      "counter": null,
      "keywords": [],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "powerSelf",
          "x": 1000,
          "until": "battle"
        }
      },
      "art": "GREEN-10",
      "fruit": "paramecia",
      "rarity": "SS",
      "faction": "whitebeard",
      "formation": null,
      "arc": "marineford"
    },
    {
      "id": "GREEN-11",
      "name": "瓦伊帕",
      "sub": " 香狄亚战士 ",
      "type": "char",
      "color": "green",
      "cost": 3,
      "power": 4000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "fruit": null,
      "art": "GREEN-11",
      "rarity": "A",
      "faction": "strawhat",
      "formation": null,
      "arc": "alabasta"
    },
    {
      "id": "GREEN-12",
      "name": "凯多",
      "sub": " 鱼鱼果实·青龙 ",
      "type": "char",
      "color": "green",
      "cost": 8,
      "power": 8000,
      "counter": null,
      "keywords": [],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "powerSelf",
          "x": 1000,
          "until": "battle"
        }
      },
      "fruit": "zoan",
      "art": "GREEN-12",
      "rarity": "SS",
      "faction": "beast",
      "formation": null,
      "arc": "wano"
    },
    {
      "id": "GREEN-13",
      "name": "锦卫门",
      "sub": " 狐火流 ",
      "type": "char",
      "color": "green",
      "cost": 4,
      "power": 4000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "fruit": null,
      "art": "GREEN-13",
      "rarity": "A",
      "faction": "strawhat",
      "formation": null,
      "arc": "wano"
    },
    {
      "id": "GREEN-14",
      "name": "河松",
      "sub": " 川流 ",
      "type": "char",
      "color": "green",
      "cost": 5,
      "power": 5000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "fruit": null,
      "art": "GREEN-14",
      "rarity": "B",
      "faction": "strawhat",
      "formation": "bulwark",
      "arc": "wano"
    },
    {
      "id": "GREEN-15",
      "name": "藤虎",
      "sub": " 重力果实 ",
      "type": "char",
      "color": "green",
      "cost": 7,
      "power": 8000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "fruit": "paramecia",
      "art": "GREEN-15",
      "rarity": "S",
      "faction": "navy",
      "formation": null,
      "arc": null
    },
    {
      "id": "GREEN-16",
      "name": "蕾贝卡",
      "sub": " 不败之女 ",
      "type": "char",
      "color": "green",
      "cost": 2,
      "power": 3000,
      "counter": 2000,
      "keywords": [],
      "effect": null,
      "fruit": null,
      "art": "GREEN-16",
      "rarity": "A",
      "faction": "strawhat",
      "formation": null,
      "arc": "dressrosa"
    },
    {
      "id": "GREEN-17",
      "name": "佩罗斯佩罗",
      "sub": " 糖糖果实 ",
      "type": "char",
      "color": "green",
      "cost": 6,
      "power": 7000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "fruit": "paramecia",
      "art": "GREEN-17",
      "rarity": "A",
      "faction": "yonko",
      "formation": null,
      "arc": "whole_cake"
    },
    {
      "id": "GREEN-18",
      "name": "酒天丸",
      "sub": " 阿修罗童子党 ",
      "type": "char",
      "color": "green",
      "cost": 5,
      "power": 5000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "fruit": null,
      "art": "GREEN-18",
      "rarity": "A",
      "faction": "beast",
      "formation": null,
      "arc": "wano"
    },
    {
      "id": "GREEN-19",
      "name": "霜月康家",
      "sub": " 和之国大名 ",
      "type": "char",
      "color": "green",
      "cost": 2,
      "power": 3000,
      "counter": 2000,
      "keywords": [],
      "effect": null,
      "fruit": null,
      "art": "GREEN-19",
      "rarity": "A",
      "faction": "strawhat",
      "formation": null,
      "arc": "wano"
    },
    {
      "id": "GREEN-20",
      "name": "居鲁士",
      "sub": " 竞技场英雄 ",
      "type": "char",
      "color": "green",
      "cost": 4,
      "power": 4000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "fruit": null,
      "art": "GREEN-20",
      "rarity": "A",
      "faction": "strawhat",
      "formation": null,
      "arc": "dressrosa"
    },
    {
      "id": "GREEN-22",
      "name": "菊之丞",
      "sub": " 花之 Soldiers ",
      "type": "char",
      "color": "green",
      "cost": 3,
      "power": 3000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "fruit": null,
      "art": "GREEN-22",
      "rarity": "A",
      "faction": "strawhat",
      "formation": null,
      "arc": "wano"
    },
    {
      "id": "GREEN-23",
      "name": "忍",
      "sub": " 御庭番众女忍 ",
      "type": "char",
      "color": "green",
      "cost": 3,
      "power": 3000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "fruit": null,
      "art": "GREEN-23",
      "rarity": "A",
      "faction": "strawhat",
      "formation": null,
      "arc": "wano"
    },
    {
      "id": "GREEN-25",
      "name": "罗罗诺亚·索隆",
      "sub": " 三刀流·鬼气 ",
      "type": "char",
      "color": "green",
      "cost": 4,
      "power": 4000,
      "counter": 1000,
      "keywords": [],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "powerSelf",
          "x": 1000,
          "until": "battle"
        }
      },
      "art": "GREEN-25",
      "fruit": null,
      "rarity": "B",
      "faction": "strawhat",
      "formation": null,
      "arc": null
    },
    {
      "id": "GREEN-28",
      "name": "乌索普",
      "sub": " 狙击之王 ",
      "type": "char",
      "color": "green",
      "cost": 4,
      "power": 4000,
      "counter": 1000,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "GREEN-28",
      "fruit": null,
      "rarity": "B",
      "faction": "strawhat",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "GREEN-32",
      "name": "托尼托尼·乔巴",
      "sub": " 暴走魔兽 ",
      "type": "char",
      "color": "green",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "banish"
      ],
      "effect": null,
      "art": "GREEN-32",
      "fruit": "paramecia",
      "rarity": "B",
      "faction": "strawhat",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "GREEN-33",
      "name": "蒙奇·D·龙",
      "sub": " 革命军领袖 ",
      "type": "char",
      "color": "green",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "restEnemy",
          "target": "strongest"
        }
      },
      "art": "GREEN-33",
      "fruit": null,
      "rarity": "S",
      "faction": "revolutionary",
      "formation": null,
      "arc": null
    },
    {
      "id": "GREEN-37",
      "name": "大和",
      "sub": " 大口真神 ",
      "type": "char",
      "color": "green",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "GREEN-37",
      "fruit": "zoan",
      "rarity": "S",
      "faction": "beast",
      "formation": "bulwark",
      "arc": "wano"
    },
    {
      "id": "GREEN-38",
      "name": "奎因",
      "sub": " 疫灾·腕龙 ",
      "type": "char",
      "color": "green",
      "cost": 5,
      "power": 6000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "GREEN-38",
      "fruit": "zoan",
      "rarity": "A",
      "faction": "beast",
      "formation": null,
      "arc": "wano"
    },
    {
      "id": "GREEN-41",
      "name": "佐佐木",
      "sub": " 三角龙 ",
      "type": "char",
      "color": "green",
      "cost": 5,
      "power": 6000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "GREEN-41",
      "fruit": "zoan",
      "rarity": "A",
      "faction": "beast",
      "formation": null,
      "arc": "wano"
    },
    {
      "id": "GREEN-42",
      "name": "黑色玛利亚",
      "sub": " 古代蛛 ",
      "type": "char",
      "color": "green",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "GREEN-42",
      "fruit": "zoan",
      "rarity": "B",
      "faction": "beast",
      "formation": "bulwark",
      "arc": "wano"
    },
    {
      "id": "GREEN-43",
      "name": "乌尔蒂",
      "sub": " 肿头龙 ",
      "type": "char",
      "color": "green",
      "cost": 4,
      "power": 4000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "GREEN-43",
      "fruit": "zoan",
      "rarity": "A",
      "faction": "beast",
      "formation": null,
      "arc": "wano"
    },
    {
      "id": "GREEN-44",
      "name": "润媞",
      "sub": " 飞六胞 ",
      "type": "char",
      "color": "green",
      "cost": 4,
      "power": 4000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "GREEN-44",
      "fruit": "zoan",
      "rarity": "A",
      "faction": "beast",
      "formation": null,
      "arc": "wano"
    },
    {
      "id": "GREEN-45",
      "name": "福斯弗",
      "sub": " 剑齿虎 ",
      "type": "char",
      "color": "green",
      "cost": 4,
      "power": 4000,
      "counter": 1000,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "GREEN-45",
      "fruit": "zoan",
      "rarity": "B",
      "faction": "beast",
      "formation": "vanguard",
      "arc": "wano"
    },
    {
      "id": "GREEN-46",
      "name": "象主",
      "sub": " 千年巨象 ",
      "type": "char",
      "color": "green",
      "cost": 8,
      "power": 9000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "GREEN-46",
      "fruit": null,
      "rarity": "SSS",
      "faction": "strawhat",
      "formation": null,
      "arc": "wano"
    },
    {
      "id": "GREEN-47",
      "name": "爱德华·纽盖特",
      "sub": " 丛云刀 ",
      "type": "char",
      "color": "green",
      "cost": 7,
      "power": 8000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "GREEN-47",
      "fruit": "paramecia",
      "rarity": "S",
      "faction": "whitebeard",
      "formation": null,
      "arc": "marineford"
    },
    {
      "id": "GREEN-49",
      "name": "堪十郎",
      "sub": " 绘画果实 ",
      "type": "char",
      "color": "green",
      "cost": 3,
      "power": 4000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "GREEN-49",
      "fruit": "paramecia",
      "rarity": "A",
      "faction": "strawhat",
      "formation": null,
      "arc": "wano"
    },
    {
      "id": "GREEN-53",
      "name": "绿牛",
      "sub": " 荆棘林 ",
      "type": "char",
      "color": "green",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "powerSelf",
          "x": 1000,
          "until": "battle"
        }
      },
      "art": "GREEN-53",
      "fruit": "logia",
      "rarity": "S",
      "faction": "navy",
      "formation": null,
      "arc": null
    },
    {
      "id": "GREEN-54",
      "name": "奥兹",
      "sub": " 传说中的魔人 ",
      "type": "char",
      "color": "green",
      "cost": 7,
      "power": 8000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "GREEN-54",
      "fruit": null,
      "rarity": "S",
      "faction": "strawhat",
      "formation": null,
      "arc": null
    },
    {
      "id": "GREEN-55",
      "name": "斯库亚德",
      "sub": " 大漩涡蜘蛛 ",
      "type": "char",
      "color": "green",
      "cost": 4,
      "power": 4000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "GREEN-55",
      "fruit": null,
      "rarity": "A",
      "faction": "whitebeard",
      "formation": null,
      "arc": "marineford"
    },
    {
      "id": "GREEN-58",
      "name": "龙马",
      "sub": " 恐怖三桅帆船 ",
      "type": "char",
      "color": "green",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "GREEN-58",
      "fruit": null,
      "rarity": "B",
      "faction": "strawhat",
      "formation": "vanguard",
      "arc": "wano"
    },
    {
      "id": "GREEN-59",
      "name": "霍格巴克",
      "sub": " 僵尸医生 ",
      "type": "char",
      "color": "green",
      "cost": 3,
      "power": 4000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "GREEN-59",
      "fruit": null,
      "rarity": "A",
      "faction": "strawhat",
      "formation": null,
      "arc": null
    },
    {
      "id": "GREEN-60",
      "name": "日和",
      "sub": " 和之国公主 ",
      "type": "char",
      "color": "green",
      "cost": 1,
      "power": 2000,
      "counter": 2000,
      "keywords": [],
      "effect": null,
      "art": "GREEN-60",
      "fruit": null,
      "rarity": "A",
      "faction": "strawhat",
      "formation": null,
      "arc": "wano"
    },
    {
      "id": "GREEN-61",
      "name": "耕四郎",
      "sub": " 一心流道场 ",
      "type": "char",
      "color": "green",
      "cost": 2,
      "power": 3000,
      "counter": 2000,
      "keywords": [],
      "effect": null,
      "art": "GREEN-61",
      "fruit": null,
      "rarity": "A",
      "faction": "strawhat",
      "formation": null,
      "arc": "wano"
    },
    {
      "id": "GREEN-62",
      "name": "古伊娜",
      "sub": " 一心流少女 ",
      "type": "char",
      "color": "green",
      "cost": 1,
      "power": 2000,
      "counter": 2000,
      "keywords": [],
      "effect": null,
      "art": "GREEN-62",
      "fruit": null,
      "rarity": "A",
      "faction": "strawhat",
      "formation": null,
      "arc": "wano"
    },
    {
      "id": "GREEN-64",
      "name": "小玉",
      "sub": " 驯服猛兽 ",
      "type": "char",
      "color": "green",
      "cost": 1,
      "power": 2000,
      "counter": 2000,
      "keywords": [],
      "effect": null,
      "art": "GREEN-64",
      "fruit": null,
      "rarity": "A",
      "faction": "strawhat",
      "formation": null,
      "arc": "wano"
    },
    {
      "id": "GREEN-G1",
      "name": "天羽羽斩",
      "sub": " 和之国黑刀 ",
      "type": "gear",
      "color": "green",
      "cost": 2,
      "power": null,
      "counter": null,
      "keywords": [],
      "gear": {
        "atk": 2000
      },
      "effect": null,
      "fruit": null,
      "art": "GREEN-G1",
      "rarity": "B",
      "faction": "strawhat",
      "formation": null,
      "arc": null
    },
    {
      "id": "GREEN-G2",
      "name": "电击毛皮",
      "sub": " 毛皮族静电 ",
      "type": "gear",
      "color": "green",
      "cost": 2,
      "power": null,
      "counter": null,
      "keywords": [],
      "gear": {
        "atk": 1000,
        "gives": [
          "blocker"
        ]
      },
      "effect": null,
      "fruit": null,
      "art": "GREEN-G2",
      "rarity": "A",
      "faction": "strawhat",
      "formation": null,
      "arc": null
    },
    {
      "id": "GREEN-G3",
      "name": "丛云刀",
      "sub": " 无上大快刀 ",
      "type": "gear",
      "color": "green",
      "cost": 3,
      "power": null,
      "counter": null,
      "keywords": [],
      "gear": {
        "atk": 3000
      },
      "effect": null,
      "art": "GREEN-G3",
      "fruit": null,
      "rarity": "S",
      "faction": "strawhat",
      "formation": null,
      "arc": null
    },
    {
      "id": "GREEN-G4",
      "name": "秋水",
      "sub": " 龙马的遗刀 ",
      "type": "gear",
      "color": "green",
      "cost": 2,
      "power": null,
      "counter": null,
      "keywords": [],
      "gear": {
        "atk": 2000
      },
      "effect": null,
      "art": "GREEN-G4",
      "fruit": null,
      "rarity": "B",
      "faction": "strawhat",
      "formation": null,
      "arc": null
    },
    {
      "id": "GREEN-G5",
      "name": "八卦金棒",
      "sub": " 凯多的刺棒 ",
      "type": "gear",
      "color": "green",
      "cost": 3,
      "power": null,
      "counter": null,
      "keywords": [],
      "gear": {
        "atk": 2000
      },
      "effect": null,
      "art": "GREEN-G5",
      "fruit": null,
      "rarity": "B",
      "faction": "strawhat",
      "formation": null,
      "arc": null
    },
    {
      "id": "GREEN-G6",
      "name": "黑兜",
      "sub": " 乌索普的弹弓 ",
      "type": "gear",
      "color": "green",
      "cost": 2,
      "power": null,
      "counter": null,
      "keywords": [],
      "gear": {
        "atk": 1000
      },
      "effect": null,
      "art": "GREEN-G6",
      "fruit": null,
      "rarity": "A",
      "faction": "strawhat",
      "formation": null,
      "arc": null
    },
    {
      "id": "PURPLE-01",
      "name": "贝波",
      "sub": " 北极熊 ",
      "type": "char",
      "color": "purple",
      "cost": 1,
      "power": 2000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "PURPLE-01",
      "fruit": null,
      "rarity": "A",
      "faction": "warlord",
      "formation": null,
      "arc": null
    },
    {
      "id": "PURPLE-02",
      "name": "夏奇",
      "sub": " 流氓店店主 ",
      "type": "char",
      "color": "purple",
      "cost": 2,
      "power": 3000,
      "counter": 2000,
      "keywords": [],
      "effect": null,
      "art": "PURPLE-02",
      "fruit": null,
      "rarity": "A",
      "faction": "yonko",
      "formation": null,
      "arc": null
    },
    {
      "id": "PURPLE-03",
      "name": "佩金",
      "sub": " 企鹅帽 ",
      "type": "char",
      "color": "purple",
      "cost": 2,
      "power": 3000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "PURPLE-03",
      "fruit": null,
      "rarity": "A",
      "faction": "warlord",
      "formation": null,
      "arc": null
    },
    {
      "id": "PURPLE-06",
      "name": "基拉",
      "sub": " 杀戮武人 ",
      "type": "char",
      "color": "purple",
      "cost": 4,
      "power": 5000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "PURPLE-06",
      "fruit": null,
      "rarity": "B",
      "faction": "supernova",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "PURPLE-07",
      "name": "凯撒·克朗",
      "sub": " 毒气 ",
      "type": "char",
      "color": "purple",
      "cost": 4,
      "power": 5000,
      "counter": null,
      "keywords": [],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "restEnemy"
        }
      },
      "art": "PURPLE-07",
      "fruit": "logia",
      "rarity": "B",
      "faction": "supernova",
      "formation": null,
      "arc": null
    },
    {
      "id": "PURPLE-10",
      "name": "乔拉可尔·米霍克",
      "sub": " 鹰眼 ",
      "type": "char",
      "color": "purple",
      "cost": 7,
      "power": 8000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "PURPLE-10",
      "fruit": null,
      "rarity": "S",
      "faction": "warlord",
      "formation": null,
      "arc": "dressrosa"
    },
    {
      "id": "PURPLE-12",
      "name": "卡彭·贝基",
      "sub": " 坚城果实 ",
      "type": "char",
      "color": "purple",
      "cost": 4,
      "power": 5000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "fruit": "logia",
      "art": "PURPLE-12",
      "rarity": "B",
      "faction": "supernova",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "PURPLE-14",
      "name": "佩吉万",
      "sub": " 龙龙果实·棘背龙 ",
      "type": "char",
      "color": "purple",
      "cost": 6,
      "power": 7000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "fruit": "zoan",
      "art": "PURPLE-14",
      "rarity": "A",
      "faction": "beast",
      "formation": null,
      "arc": "wano"
    },
    {
      "id": "PURPLE-15",
      "name": "卡里布",
      "sub": " 沼沼果实 ",
      "type": "char",
      "color": "purple",
      "cost": 4,
      "power": 5000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "fruit": "logia",
      "art": "PURPLE-15",
      "rarity": "A",
      "faction": "warlord",
      "formation": null,
      "arc": null
    },
    {
      "id": "PURPLE-16",
      "name": "柯拉松",
      "sub": " 唐吉诃德·罗西南迪 ",
      "type": "char",
      "color": "purple",
      "cost": 4,
      "power": 5000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "fruit": null,
      "art": "PURPLE-16",
      "rarity": "A",
      "faction": "navy",
      "formation": null,
      "arc": null
    },
    {
      "id": "PURPLE-17",
      "name": "鹤",
      "sub": " 海军参谋 ",
      "type": "char",
      "color": "purple",
      "cost": 2,
      "power": 3000,
      "counter": 2000,
      "keywords": [],
      "effect": null,
      "fruit": null,
      "art": "PURPLE-17",
      "rarity": "A",
      "faction": "navy",
      "formation": null,
      "arc": null
    },
    {
      "id": "PURPLE-18",
      "name": "让·巴特",
      "sub": " 心脏海贼团 ",
      "type": "char",
      "color": "purple",
      "cost": 8,
      "power": 9000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "fruit": null,
      "art": "PURPLE-18",
      "rarity": "SSS",
      "faction": "warlord",
      "formation": null,
      "arc": null
    },
    {
      "id": "PURPLE-19",
      "name": "雷利",
      "sub": " 冥王 ",
      "type": "char",
      "color": "purple",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "fruit": null,
      "art": "PURPLE-19",
      "rarity": "S",
      "faction": "yonko",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "PURPLE-21",
      "name": "夏洛特·布蕾",
      "sub": " 镜镜果实 ",
      "type": "char",
      "color": "purple",
      "cost": 4,
      "power": 5000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "fruit": "paramecia",
      "art": "PURPLE-21",
      "rarity": "A",
      "faction": "yonko",
      "formation": null,
      "arc": "whole_cake"
    },
    {
      "id": "PURPLE-22",
      "name": "甘福尔",
      "sub": " 空岛神代 ",
      "type": "char",
      "color": "purple",
      "cost": 3,
      "power": 4000,
      "counter": 2000,
      "keywords": [],
      "effect": null,
      "fruit": null,
      "art": "PURPLE-22",
      "rarity": "A",
      "faction": "yonko",
      "formation": null,
      "arc": "skypiea"
    },
    {
      "id": "PURPLE-24",
      "name": "特拉法尔加·罗",
      "sub": " 北海医生 ",
      "type": "char",
      "color": "purple",
      "cost": 1,
      "power": 2000,
      "counter": 2000,
      "keywords": [],
      "effect": null,
      "art": "PURPLE-24",
      "fruit": "paramecia",
      "rarity": "A",
      "faction": "warlord",
      "formation": null,
      "arc": null
    },
    {
      "id": "PURPLE-30",
      "name": "唐吉诃德·多弗朗明戈",
      "sub": " 寄生线 ",
      "type": "char",
      "color": "purple",
      "cost": 5,
      "power": 6000,
      "counter": null,
      "keywords": [],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "restEnemy",
          "target": "last"
        }
      },
      "art": "PURPLE-30",
      "fruit": "paramecia",
      "rarity": "B",
      "faction": "warlord",
      "formation": null,
      "arc": "dressrosa"
    },
    {
      "id": "PURPLE-32",
      "name": "霍金斯",
      "sub": " 占卜 ",
      "type": "char",
      "color": "purple",
      "cost": 1,
      "power": 2000,
      "counter": 2000,
      "keywords": [],
      "effect": null,
      "art": "PURPLE-32",
      "fruit": "paramecia",
      "rarity": "A",
      "faction": "supernova",
      "formation": null,
      "arc": null
    },
    {
      "id": "PURPLE-36",
      "name": "尤斯塔斯·基德",
      "sub": " 磁力·斥力 ",
      "type": "char",
      "color": "purple",
      "cost": 5,
      "power": 5000,
      "counter": null,
      "keywords": [],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "restEnemy",
          "target": "last"
        }
      },
      "art": "PURPLE-36",
      "fruit": "paramecia",
      "rarity": "B",
      "faction": "supernova",
      "formation": null,
      "arc": null
    },
    {
      "id": "PURPLE-41",
      "name": "乔艾莉·波妮",
      "sub": " 年龄操控 ",
      "type": "char",
      "color": "purple",
      "cost": 5,
      "power": 5000,
      "counter": null,
      "keywords": [],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "restEnemy",
          "target": "last"
        }
      },
      "art": "PURPLE-41",
      "fruit": "paramecia",
      "rarity": "B",
      "faction": "supernova",
      "formation": null,
      "arc": null
    },
    {
      "id": "PURPLE-43",
      "name": "阿普",
      "sub": " 长手族DJ ",
      "type": "char",
      "color": "purple",
      "cost": 2,
      "power": 3000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "PURPLE-43",
      "fruit": "paramecia",
      "rarity": "A",
      "faction": "supernova",
      "formation": null,
      "arc": null
    },
    {
      "id": "PURPLE-45",
      "name": "乌尔基",
      "sub": " 因果报应 ",
      "type": "char",
      "color": "purple",
      "cost": 4,
      "power": 5000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "PURPLE-45",
      "fruit": "paramecia",
      "rarity": "A",
      "faction": "supernova",
      "formation": null,
      "arc": null
    },
    {
      "id": "PURPLE-46",
      "name": "X·德雷克",
      "sub": " 异特龙 ",
      "type": "char",
      "color": "purple",
      "cost": 3,
      "power": 4000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "PURPLE-46",
      "fruit": "zoan",
      "rarity": "A",
      "faction": "supernova",
      "formation": null,
      "arc": null
    },
    {
      "id": "PURPLE-51",
      "name": "贝加庞克",
      "sub": " 军事科学家 ",
      "type": "char",
      "color": "purple",
      "cost": 3,
      "power": 3000,
      "counter": 2000,
      "keywords": [],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "draw",
          "n": 1
        }
      },
      "art": "PURPLE-51",
      "fruit": null,
      "rarity": "B",
      "faction": "supernova",
      "formation": null,
      "arc": null
    },
    {
      "id": "PURPLE-54",
      "name": "席尔巴斯·雷利",
      "sub": " 冥王·三色霸气 ",
      "type": "char",
      "color": "purple",
      "cost": 8,
      "power": 8000,
      "counter": null,
      "keywords": [],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "powerSelf",
          "x": 2000,
          "until": "battle"
        }
      },
      "art": "PURPLE-54",
      "fruit": null,
      "rarity": "SS",
      "faction": "yonko",
      "formation": null,
      "arc": null
    },
    {
      "id": "PURPLE-57",
      "name": "唐吉诃德·罗西南迪",
      "sub": " 白衣的守护 ",
      "type": "char",
      "color": "purple",
      "cost": 5,
      "power": 5000,
      "counter": null,
      "keywords": [],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "healLP",
          "x": 1000
        }
      },
      "art": "PURPLE-57",
      "fruit": "paramecia",
      "rarity": "B",
      "faction": "navy",
      "formation": null,
      "arc": null
    },
    {
      "id": "PURPLE-59",
      "name": "维奥莱特",
      "sub": " 千里眼 ",
      "type": "char",
      "color": "purple",
      "cost": 4,
      "power": 5000,
      "counter": 1000,
      "keywords": [],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "draw",
          "n": 1
        }
      },
      "art": "PURPLE-59",
      "fruit": "paramecia",
      "rarity": "B",
      "faction": "warlord",
      "formation": null,
      "arc": "dressrosa"
    },
    {
      "id": "PURPLE-62",
      "name": "乔拉",
      "sub": " 艺术果实 ",
      "type": "char",
      "color": "purple",
      "cost": 3,
      "power": 4000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "PURPLE-62",
      "fruit": "paramecia",
      "rarity": "A",
      "faction": "warlord",
      "formation": null,
      "arc": "dressrosa"
    },
    {
      "id": "PURPLE-63",
      "name": "Mr.3",
      "sub": " 蜡烛果实 ",
      "type": "char",
      "color": "purple",
      "cost": 3,
      "power": 3000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "PURPLE-63",
      "fruit": "paramecia",
      "rarity": "B",
      "faction": "warlord",
      "formation": "bulwark",
      "arc": "alabasta"
    },
    {
      "id": "PURPLE-64",
      "name": "Miss黄金周",
      "sub": " 自由画家 ",
      "type": "char",
      "color": "purple",
      "cost": 1,
      "power": 2000,
      "counter": 2000,
      "keywords": [],
      "effect": null,
      "art": "PURPLE-64",
      "fruit": null,
      "rarity": "A",
      "faction": "warlord",
      "formation": null,
      "arc": "alabasta"
    },
    {
      "id": "PURPLE-G1",
      "name": "鬼哭",
      "sub": " 诅咒之刀 ",
      "type": "gear",
      "color": "purple",
      "cost": 2,
      "power": null,
      "counter": null,
      "keywords": [],
      "gear": {
        "atk": 2000
      },
      "effect": null,
      "fruit": null,
      "art": "PURPLE-G1",
      "rarity": "B",
      "faction": "warlord",
      "formation": null,
      "arc": null
    },
    {
      "id": "PURPLE-G4",
      "name": "天夜叉的墨镜",
      "sub": " 多弗朗明戈 ",
      "type": "gear",
      "color": "purple",
      "cost": 2,
      "power": null,
      "counter": null,
      "keywords": [],
      "gear": {
        "atk": 1000
      },
      "effect": null,
      "art": "PURPLE-G4",
      "fruit": null,
      "rarity": "A",
      "faction": "warlord",
      "formation": null,
      "arc": null
    },
    {
      "id": "PURPLE-G5",
      "name": "诅咒稻草人",
      "sub": " 霍金斯的诅咒 ",
      "type": "gear",
      "color": "purple",
      "cost": 3,
      "power": null,
      "counter": null,
      "keywords": [],
      "gear": {
        "atk": 1000
      },
      "effect": null,
      "art": "PURPLE-G5",
      "fruit": null,
      "rarity": "A",
      "faction": "warlord",
      "formation": null,
      "arc": null
    },
    {
      "id": "RED-01",
      "name": "乌索普",
      "sub": " 狙击之王 ",
      "type": "char",
      "color": "red",
      "cost": 1,
      "power": 2000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "RED-01",
      "fruit": null,
      "rarity": "A",
      "faction": "strawhat",
      "formation": null,
      "arc": null
    },
    {
      "id": "RED-02",
      "name": "克比",
      "sub": " 海军本部少将 ",
      "type": "char",
      "color": "red",
      "cost": 1,
      "power": 2000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "RED-02",
      "fruit": null,
      "rarity": "A",
      "faction": "navy",
      "formation": null,
      "arc": null
    },
    {
      "id": "RED-03",
      "name": "山治",
      "sub": " 黑脚 ",
      "type": "char",
      "color": "red",
      "cost": 2,
      "power": 2000,
      "counter": 1000,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "RED-03",
      "fruit": null,
      "rarity": "B",
      "faction": "strawhat",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "RED-04",
      "name": "贝拉米",
      "sub": " 鬣狗 ",
      "type": "char",
      "color": "red",
      "cost": 2,
      "power": 3000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "RED-04",
      "fruit": "paramecia",
      "rarity": "A",
      "faction": "strawhat",
      "formation": null,
      "arc": null
    },
    {
      "id": "RED-05",
      "name": "冯克雷",
      "sub": " Mr.2 人妖之王 ",
      "type": "char",
      "color": "red",
      "cost": 3,
      "power": 3000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "RED-05",
      "fruit": "paramecia",
      "rarity": "B",
      "faction": "warlord",
      "formation": "bulwark",
      "arc": "alabasta"
    },
    {
      "id": "RED-06",
      "name": "萨博",
      "sub": " 革命军参谋总长 ",
      "type": "char",
      "color": "red",
      "cost": 4,
      "power": 4000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "RED-06",
      "fruit": null,
      "rarity": "B",
      "faction": "revolutionary",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "RED-08",
      "name": "艾斯",
      "sub": " 火拳 ",
      "type": "char",
      "color": "red",
      "cost": 4,
      "power": 4000,
      "counter": null,
      "keywords": [],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "powerSelf",
          "x": 1000,
          "until": "battle"
        }
      },
      "art": "RED-08",
      "fruit": "logia",
      "rarity": "B",
      "faction": "whitebeard",
      "formation": null,
      "arc": "marineford"
    },
    {
      "id": "RED-10",
      "name": "青椒",
      "sub": " 锥龙钉钉头 ",
      "type": "char",
      "color": "red",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "powerSelf",
          "x": 1000,
          "until": "battle"
        }
      },
      "art": "RED-10",
      "fruit": null,
      "rarity": "S",
      "faction": "strawhat",
      "formation": null,
      "arc": null
    },
    {
      "id": "RED-11",
      "name": "巴基",
      "sub": " 四分五裂 ",
      "type": "char",
      "color": "red",
      "cost": 2,
      "power": 3000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "fruit": "paramecia",
      "art": "RED-11",
      "rarity": "A",
      "faction": "warlord",
      "formation": null,
      "arc": "east_blue"
    },
    {
      "id": "RED-12",
      "name": "亚尔丽塔",
      "sub": " 滑滑果实 ",
      "type": "char",
      "color": "red",
      "cost": 1,
      "power": 2000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "fruit": "paramecia",
      "art": "RED-12",
      "rarity": "A",
      "faction": "strawhat",
      "formation": null,
      "arc": "east_blue"
    },
    {
      "id": "RED-13",
      "name": "克洛",
      "sub": " 百计 ",
      "type": "char",
      "color": "red",
      "cost": 3,
      "power": 4000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "fruit": null,
      "art": "RED-13",
      "rarity": "A",
      "faction": "strawhat",
      "formation": null,
      "arc": "east_blue"
    },
    {
      "id": "RED-14",
      "name": "阿龙",
      "sub": " 锯齿鲨鱼人 ",
      "type": "char",
      "color": "red",
      "cost": 5,
      "power": 6000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "fruit": null,
      "art": "RED-14",
      "rarity": "A",
      "faction": "strawhat",
      "formation": null,
      "arc": "east_blue"
    },
    {
      "id": "RED-15",
      "name": "薇薇",
      "sub": " 阿拉巴斯坦公主 ",
      "type": "char",
      "color": "red",
      "cost": 1,
      "power": 2000,
      "counter": 2000,
      "keywords": [],
      "effect": null,
      "fruit": null,
      "art": "RED-15",
      "rarity": "A",
      "faction": "strawhat",
      "formation": null,
      "arc": "alabasta"
    },
    {
      "id": "RED-16",
      "name": "达兹·波涅斯",
      "sub": " 快斩果实 ",
      "type": "char",
      "color": "red",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "fruit": "paramecia",
      "art": "RED-16",
      "rarity": "B",
      "faction": "warlord",
      "formation": "vanguard",
      "arc": "alabasta"
    },
    {
      "id": "RED-19",
      "name": "萨卡斯基",
      "sub": " 岩浆果实 ",
      "type": "char",
      "color": "red",
      "cost": 8,
      "power": 8000,
      "counter": null,
      "keywords": [],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "powerSelf",
          "x": 1000,
          "until": "battle"
        }
      },
      "fruit": "logia",
      "art": "RED-19",
      "rarity": "SS",
      "faction": "navy",
      "formation": null,
      "arc": "marineford"
    },
    {
      "id": "RED-20",
      "name": "伊万科夫",
      "sub": " 荷尔蒙果实 ",
      "type": "char",
      "color": "red",
      "cost": 6,
      "power": 7000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "fruit": "paramecia",
      "art": "RED-20",
      "rarity": "A",
      "faction": "revolutionary",
      "formation": null,
      "arc": null
    },
    {
      "id": "RED-21",
      "name": "克尔拉",
      "sub": " 革命军参谋 ",
      "type": "char",
      "color": "red",
      "cost": 3,
      "power": 4000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "fruit": null,
      "art": "RED-21",
      "rarity": "A",
      "faction": "strawhat",
      "formation": null,
      "arc": null
    },
    {
      "id": "RED-22",
      "name": "萨奇",
      "sub": " 白团四番队长 ",
      "type": "char",
      "color": "red",
      "cost": 4,
      "power": 5000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "fruit": null,
      "art": "RED-22",
      "rarity": "A",
      "faction": "whitebeard",
      "formation": null,
      "arc": "marineford"
    },
    {
      "id": "RED-23",
      "name": "战桃丸",
      "sub": " 海军科学班 ",
      "type": "char",
      "color": "red",
      "cost": 5,
      "power": 5000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "fruit": null,
      "art": "RED-23",
      "rarity": "B",
      "faction": "navy",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "RED-27",
      "name": "蒙奇·D·路飞",
      "sub": " 五档·尼卡 ",
      "type": "char",
      "color": "red",
      "cost": 8,
      "power": 8000,
      "counter": null,
      "keywords": [],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "powerSelf",
          "x": 2000,
          "until": "battle"
        }
      },
      "art": "RED-27",
      "fruit": "zoan",
      "rarity": "SS",
      "faction": "strawhat",
      "formation": null,
      "arc": null
    },
    {
      "id": "RED-29",
      "name": "波特卡斯·D·艾斯",
      "sub": " 炎帝 ",
      "type": "char",
      "color": "red",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "powerSelf",
          "x": 1000,
          "until": "battle"
        }
      },
      "art": "RED-29",
      "fruit": "logia",
      "rarity": "S",
      "faction": "whitebeard",
      "formation": null,
      "arc": "marineford"
    },
    {
      "id": "RED-33",
      "name": "萨波",
      "sub": " 烧烧果实继承者 ",
      "type": "char",
      "color": "red",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "powerSelf",
          "x": 1000,
          "until": "battle"
        }
      },
      "art": "RED-33",
      "fruit": "logia",
      "rarity": "B",
      "faction": "revolutionary",
      "formation": null,
      "arc": null
    },
    {
      "id": "RED-35",
      "name": "香克斯",
      "sub": " 霸王色的威压 ",
      "type": "char",
      "color": "red",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "debuffFoeAll",
          "x": 1000,
          "until": "battle"
        }
      },
      "art": "RED-35",
      "fruit": null,
      "rarity": "B",
      "faction": "yonko",
      "formation": null,
      "arc": null
    },
    {
      "id": "RED-37",
      "name": "哥尔·D·罗杰",
      "sub": " 海贼王 ",
      "type": "char",
      "color": "red",
      "cost": 8,
      "power": 8000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "RED-37",
      "fruit": null,
      "rarity": "SS",
      "faction": "yonko",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "RED-39",
      "name": "光月御田",
      "sub": " 天羽羽斩·双刀流 ",
      "type": "char",
      "color": "red",
      "cost": 7,
      "power": 8000,
      "counter": null,
      "keywords": [
        "doubleAttack"
      ],
      "effect": null,
      "art": "RED-39",
      "fruit": null,
      "rarity": "S",
      "faction": "whitebeard",
      "formation": "vanguard",
      "arc": "wano"
    },
    {
      "id": "RED-41",
      "name": "光月桃之助",
      "sub": " 人造青龙果实 ",
      "type": "char",
      "color": "red",
      "cost": 1,
      "power": 2000,
      "counter": 2000,
      "keywords": [],
      "effect": null,
      "art": "RED-41",
      "fruit": "zoan",
      "rarity": "A",
      "faction": "strawhat",
      "formation": null,
      "arc": "wano"
    },
    {
      "id": "RED-44",
      "name": "锦卫门",
      "sub": " 赤鞘九侠 ",
      "type": "char",
      "color": "red",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "RED-44",
      "fruit": "paramecia",
      "rarity": "B",
      "faction": "strawhat",
      "formation": "bulwark",
      "arc": "wano"
    },
    {
      "id": "RED-45",
      "name": "狂死郎",
      "sub": " 九霞流 ",
      "type": "char",
      "color": "red",
      "cost": 7,
      "power": 8000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "RED-45",
      "fruit": null,
      "rarity": "S",
      "faction": "strawhat",
      "formation": null,
      "arc": "wano"
    },
    {
      "id": "RED-47",
      "name": "卡文迪许",
      "sub": " 隆美尔的镰鼬 ",
      "type": "char",
      "color": "red",
      "cost": 4,
      "power": 4000,
      "counter": 1000,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "RED-47",
      "fruit": null,
      "rarity": "B",
      "faction": "supernova",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "RED-48",
      "name": "威布尔",
      "sub": " 白胡子二世 ",
      "type": "char",
      "color": "red",
      "cost": 6,
      "power": 7000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "RED-48",
      "fruit": null,
      "rarity": "A",
      "faction": "warlord",
      "formation": null,
      "arc": null
    },
    {
      "id": "RED-50",
      "name": "金狮子史基",
      "sub": " 飘飘果实 ",
      "type": "char",
      "color": "red",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "restEnemy",
          "target": "last"
        }
      },
      "art": "RED-50",
      "fruit": "paramecia",
      "rarity": "S",
      "faction": "yonko",
      "formation": null,
      "arc": null
    },
    {
      "id": "RED-52",
      "name": "爱德华·纽盖特",
      "sub": " 震震果实 ",
      "type": "char",
      "color": "red",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "restEnemy",
          "target": "last"
        }
      },
      "art": "RED-52",
      "fruit": "paramecia",
      "rarity": "S",
      "faction": "whitebeard",
      "formation": null,
      "arc": "marineford"
    },
    {
      "id": "RED-53",
      "name": "巴托洛米奥",
      "sub": " 屏障·冲撞 ",
      "type": "char",
      "color": "red",
      "cost": 3,
      "power": 3000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "RED-53",
      "fruit": "paramecia",
      "rarity": "B",
      "faction": "supernova",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "RED-55",
      "name": "瓦尔波",
      "sub": " 吞吞果实 ",
      "type": "char",
      "color": "red",
      "cost": 2,
      "power": 3000,
      "counter": 2000,
      "keywords": [],
      "effect": null,
      "art": "RED-55",
      "fruit": "paramecia",
      "rarity": "A",
      "faction": "strawhat",
      "formation": null,
      "arc": "east_blue"
    },
    {
      "id": "RED-56",
      "name": "Mr.5",
      "sub": " 炸弹果实 ",
      "type": "char",
      "color": "red",
      "cost": 1,
      "power": 2000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "RED-56",
      "fruit": "paramecia",
      "rarity": "A",
      "faction": "warlord",
      "formation": null,
      "arc": "alabasta"
    },
    {
      "id": "RED-57",
      "name": "洛克斯达",
      "sub": " 红发的使者 ",
      "type": "char",
      "color": "red",
      "cost": 1,
      "power": 2000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "RED-57",
      "fruit": null,
      "rarity": "A",
      "faction": "yonko",
      "formation": null,
      "arc": null
    },
    {
      "id": "RED-58",
      "name": "贝洛·贝蒂",
      "sub": " 激励果实 ",
      "type": "char",
      "color": "red",
      "cost": 3,
      "power": 3000,
      "counter": 1000,
      "keywords": [],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "buffAll",
          "x": 1000,
          "until": "turn"
        }
      },
      "art": "RED-58",
      "fruit": "paramecia",
      "rarity": "B",
      "faction": "strawhat",
      "formation": null,
      "arc": null
    },
    {
      "id": "RED-60",
      "name": "东利",
      "sub": " 青鬼 ",
      "type": "char",
      "color": "red",
      "cost": 5,
      "power": 6000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "RED-60",
      "fruit": null,
      "rarity": "A",
      "faction": "strawhat",
      "formation": null,
      "arc": null
    },
    {
      "id": "RED-61",
      "name": "布罗吉",
      "sub": " 红鬼 ",
      "type": "char",
      "color": "red",
      "cost": 5,
      "power": 6000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "RED-61",
      "fruit": null,
      "rarity": "A",
      "faction": "strawhat",
      "formation": null,
      "arc": null
    },
    {
      "id": "RED-62",
      "name": "罗布·路奇",
      "sub": " 六王枪 ",
      "type": "char",
      "color": "red",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "RED-62",
      "fruit": "zoan",
      "rarity": "B",
      "faction": "navy",
      "formation": "vanguard",
      "arc": "enies_lobby"
    },
    {
      "id": "RED-63",
      "name": "卡塔库栗",
      "sub": " 见闻色·未来视 ",
      "type": "char",
      "color": "red",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "powerSelf",
          "x": 1000,
          "until": "battle"
        }
      },
      "art": "RED-63",
      "fruit": "paramecia",
      "rarity": "B",
      "faction": "yonko",
      "formation": null,
      "arc": "whole_cake"
    },
    {
      "id": "RED-64",
      "name": "巴法罗",
      "sub": " 转转果实 ",
      "type": "char",
      "color": "red",
      "cost": 3,
      "power": 4000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "RED-64",
      "fruit": "paramecia",
      "rarity": "A",
      "faction": "strawhat",
      "formation": null,
      "arc": null
    },
    {
      "id": "RED-G1",
      "name": "三代鬼彻",
      "sub": " 和之国妖刀 ",
      "type": "gear",
      "color": "red",
      "cost": 2,
      "power": null,
      "counter": null,
      "keywords": [],
      "gear": {
        "atk": 2000
      },
      "effect": null,
      "fruit": null,
      "art": "RED-G1",
      "rarity": "B",
      "faction": "strawhat",
      "formation": null,
      "arc": null
    },
    {
      "id": "RED-G2",
      "name": "武装色·硬化",
      "sub": " 全身武装 ",
      "type": "gear",
      "color": "red",
      "cost": 2,
      "power": null,
      "counter": null,
      "keywords": [],
      "gear": {
        "atk": 1000,
        "gives": [
          "blocker"
        ]
      },
      "effect": null,
      "fruit": null,
      "art": "RED-G2",
      "rarity": "A",
      "faction": "strawhat",
      "formation": null,
      "arc": null
    },
    {
      "id": "RED-G3",
      "name": "和道一文字",
      "sub": " 大快刀二十一工 ",
      "type": "gear",
      "color": "red",
      "cost": 2,
      "power": null,
      "counter": null,
      "keywords": [],
      "gear": {
        "atk": 2000
      },
      "effect": null,
      "art": "RED-G3",
      "fruit": null,
      "rarity": "B",
      "faction": "strawhat",
      "formation": null,
      "arc": null
    },
    {
      "id": "RED-G4",
      "name": "阎魔",
      "sub": " 御田的爱刀 ",
      "type": "gear",
      "color": "red",
      "cost": 3,
      "power": null,
      "counter": null,
      "keywords": [],
      "gear": {
        "atk": 3000
      },
      "effect": null,
      "art": "RED-G4",
      "fruit": null,
      "rarity": "S",
      "faction": "strawhat",
      "formation": null,
      "arc": null
    },
    {
      "id": "RED-G5",
      "name": "雪走",
      "sub": " 良业物·快刀 ",
      "type": "gear",
      "color": "red",
      "cost": 2,
      "power": null,
      "counter": null,
      "keywords": [],
      "gear": {
        "atk": 2000
      },
      "effect": null,
      "art": "RED-G5",
      "fruit": null,
      "rarity": "B",
      "faction": "strawhat",
      "formation": null,
      "arc": null
    },
    {
      "id": "RED-G6",
      "name": "海楼石手铐",
      "sub": " 果实封锁 ",
      "type": "gear",
      "color": "red",
      "cost": 2,
      "power": null,
      "counter": null,
      "keywords": [],
      "gear": {
        "atk": 1000
      },
      "effect": null,
      "art": "RED-G6",
      "fruit": null,
      "rarity": "A",
      "faction": "strawhat",
      "formation": null,
      "arc": null
    },
    {
      "id": "YELLOW-01",
      "name": "佩罗娜",
      "sub": " 幽灵公主 ",
      "type": "char",
      "color": "yellow",
      "cost": 1,
      "power": 2000,
      "counter": 2000,
      "keywords": [],
      "effect": null,
      "art": "YELLOW-01",
      "fruit": "paramecia",
      "rarity": "A",
      "faction": "warlord",
      "formation": null,
      "arc": null
    },
    {
      "id": "YELLOW-02",
      "name": "桑达索尼亚",
      "sub": " 蛇蛇果实 ",
      "type": "char",
      "color": "yellow",
      "cost": 2,
      "power": 2000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "YELLOW-02",
      "fruit": "zoan",
      "rarity": "B",
      "faction": "warlord",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "YELLOW-03",
      "name": "玛丽哥德",
      "sub": " 蛇蛇果实 ",
      "type": "char",
      "color": "yellow",
      "cost": 2,
      "power": 2000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "YELLOW-03",
      "fruit": "zoan",
      "rarity": "B",
      "faction": "warlord",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "YELLOW-04",
      "name": "莫利亚",
      "sub": " 影子大王 ",
      "type": "char",
      "color": "yellow",
      "cost": 3,
      "power": 4000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "YELLOW-04",
      "fruit": "paramecia",
      "rarity": "A",
      "faction": "warlord",
      "formation": null,
      "arc": null
    },
    {
      "id": "YELLOW-05",
      "name": "比斯塔",
      "sub": " 花剑 ",
      "type": "char",
      "color": "yellow",
      "cost": 4,
      "power": 4000,
      "counter": 2000,
      "keywords": [],
      "effect": null,
      "art": "YELLOW-05",
      "fruit": null,
      "rarity": "A",
      "faction": "whitebeard",
      "formation": null,
      "arc": "marineford"
    },
    {
      "id": "YELLOW-06",
      "name": "乔兹",
      "sub": " 钻石 ",
      "type": "char",
      "color": "yellow",
      "cost": 4,
      "power": 4000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "YELLOW-06",
      "fruit": "paramecia",
      "rarity": "B",
      "faction": "whitebeard",
      "formation": "bulwark",
      "arc": "marineford"
    },
    {
      "id": "YELLOW-09",
      "name": "多弗朗明戈",
      "sub": " 天夜叉 ",
      "type": "char",
      "color": "yellow",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "powerSelf",
          "x": 1000,
          "until": "battle"
        }
      },
      "art": "YELLOW-09",
      "fruit": "paramecia",
      "rarity": "B",
      "faction": "warlord",
      "formation": null,
      "arc": "dressrosa"
    },
    {
      "id": "YELLOW-13",
      "name": "塞尼奥尔·皮克",
      "sub": " 游游果实 ",
      "type": "char",
      "color": "yellow",
      "cost": 4,
      "power": 4000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "fruit": "paramecia",
      "art": "YELLOW-13",
      "rarity": "B",
      "faction": "warlord",
      "formation": "bulwark",
      "arc": "dressrosa"
    },
    {
      "id": "YELLOW-14",
      "name": "迪亚曼蒂",
      "sub": " 飘飘果实 ",
      "type": "char",
      "color": "yellow",
      "cost": 5,
      "power": 5000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "fruit": "paramecia",
      "art": "YELLOW-14",
      "rarity": "A",
      "faction": "warlord",
      "formation": null,
      "arc": null
    },
    {
      "id": "YELLOW-15",
      "name": "皮卡",
      "sub": " 石石果实 ",
      "type": "char",
      "color": "yellow",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "fruit": "paramecia",
      "art": "YELLOW-15",
      "rarity": "S",
      "faction": "warlord",
      "formation": "bulwark",
      "arc": "dressrosa"
    },
    {
      "id": "YELLOW-16",
      "name": "特雷波尔",
      "sub": " 粘粘果实 ",
      "type": "char",
      "color": "yellow",
      "cost": 3,
      "power": 4000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "fruit": "paramecia",
      "art": "YELLOW-16",
      "rarity": "A",
      "faction": "warlord",
      "formation": null,
      "arc": "dressrosa"
    },
    {
      "id": "YELLOW-17",
      "name": "维尔戈",
      "sub": " 鬼竹 ",
      "type": "char",
      "color": "yellow",
      "cost": 5,
      "power": 5000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "fruit": null,
      "art": "YELLOW-17",
      "rarity": "B",
      "faction": "warlord",
      "formation": "bulwark",
      "arc": "dressrosa"
    },
    {
      "id": "YELLOW-18",
      "name": "莫奈",
      "sub": " 鸟鸟果实·夜枭 ",
      "type": "char",
      "color": "yellow",
      "cost": 4,
      "power": 4000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "fruit": "zoan",
      "art": "YELLOW-18",
      "rarity": "A",
      "faction": "warlord",
      "formation": null,
      "arc": "dressrosa"
    },
    {
      "id": "YELLOW-19",
      "name": "阿布萨罗姆",
      "sub": " 透明果实 ",
      "type": "char",
      "color": "yellow",
      "cost": 3,
      "power": 3000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "fruit": "paramecia",
      "art": "YELLOW-19",
      "rarity": "B",
      "faction": "warlord",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "YELLOW-20",
      "name": "斯慕吉",
      "sub": " 榨榨果实 ",
      "type": "char",
      "color": "yellow",
      "cost": 4,
      "power": 4000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "fruit": "paramecia",
      "art": "YELLOW-20",
      "rarity": "A",
      "faction": "yonko",
      "formation": null,
      "arc": "whole_cake"
    },
    {
      "id": "YELLOW-21",
      "name": "布琳",
      "sub": " 记忆果实 ",
      "type": "char",
      "color": "yellow",
      "cost": 3,
      "power": 4000,
      "counter": 2000,
      "keywords": [],
      "effect": null,
      "fruit": "paramecia",
      "art": "YELLOW-21",
      "rarity": "A",
      "faction": "yonko",
      "formation": null,
      "arc": "whole_cake"
    },
    {
      "id": "YELLOW-22",
      "name": "欧文",
      "sub": " 热热果实 ",
      "type": "char",
      "color": "yellow",
      "cost": 4,
      "power": 4000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "fruit": "paramecia",
      "art": "YELLOW-22",
      "rarity": "A",
      "faction": "yonko",
      "formation": null,
      "arc": "whole_cake"
    },
    {
      "id": "YELLOW-23",
      "name": "砂糖",
      "sub": " 玩具果实 ",
      "type": "char",
      "color": "yellow",
      "cost": 2,
      "power": 3000,
      "counter": 2000,
      "keywords": [],
      "effect": null,
      "fruit": "paramecia",
      "art": "YELLOW-23",
      "rarity": "A",
      "faction": "warlord",
      "formation": null,
      "arc": "dressrosa"
    },
    {
      "id": "YELLOW-24",
      "name": "山治",
      "sub": " 见习厨师 ",
      "type": "char",
      "color": "yellow",
      "cost": 1,
      "power": 2000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "YELLOW-24",
      "fruit": null,
      "rarity": "A",
      "faction": "strawhat",
      "formation": null,
      "arc": null
    },
    {
      "id": "YELLOW-29",
      "name": "波雅·汉库珂",
      "sub": " 甜甜甘风 ",
      "type": "char",
      "color": "yellow",
      "cost": 4,
      "power": 4000,
      "counter": 1000,
      "keywords": [],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "restEnemy",
          "target": "last"
        }
      },
      "art": "YELLOW-29",
      "fruit": "paramecia",
      "rarity": "B",
      "faction": "warlord",
      "formation": null,
      "arc": null
    },
    {
      "id": "YELLOW-36",
      "name": "艾涅尔",
      "sub": " 放电 ",
      "type": "char",
      "color": "yellow",
      "cost": 1,
      "power": 2000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "YELLOW-36",
      "fruit": "logia",
      "rarity": "A",
      "faction": "yonko",
      "formation": null,
      "arc": "skypiea"
    },
    {
      "id": "YELLOW-38",
      "name": "克洛克达尔",
      "sub": " 沙暴 ",
      "type": "char",
      "color": "yellow",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "powerSelf",
          "x": 1000,
          "until": "battle"
        }
      },
      "art": "YELLOW-38",
      "fruit": "logia",
      "rarity": "B",
      "faction": "warlord",
      "formation": null,
      "arc": "east_blue"
    },
    {
      "id": "YELLOW-39",
      "name": "犬岚公爵",
      "sub": " 月狮 ",
      "type": "char",
      "color": "yellow",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "YELLOW-39",
      "fruit": null,
      "rarity": "B",
      "faction": "whitebeard",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "YELLOW-40",
      "name": "猫蝮蛇",
      "sub": " 月狮 ",
      "type": "char",
      "color": "yellow",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "YELLOW-40",
      "fruit": null,
      "rarity": "B",
      "faction": "whitebeard",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "YELLOW-41",
      "name": "夏洛特·玲玲",
      "sub": " 魂魂果实 ",
      "type": "char",
      "color": "yellow",
      "cost": 8,
      "power": 8000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "YELLOW-41",
      "fruit": "paramecia",
      "rarity": "SS",
      "faction": "yonko",
      "formation": "bulwark",
      "arc": "whole_cake"
    },
    {
      "id": "YELLOW-44",
      "name": "克力架",
      "sub": " 饼干士兵 ",
      "type": "char",
      "color": "yellow",
      "cost": 3,
      "power": 3000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "YELLOW-44",
      "fruit": "paramecia",
      "rarity": "B",
      "faction": "yonko",
      "formation": "bulwark",
      "arc": "whole_cake"
    },
    {
      "id": "YELLOW-46",
      "name": "玛格丽特",
      "sub": " 蛇岛弓手 ",
      "type": "char",
      "color": "yellow",
      "cost": 3,
      "power": 4000,
      "counter": 2000,
      "keywords": [],
      "effect": null,
      "art": "YELLOW-46",
      "fruit": null,
      "rarity": "A",
      "faction": "warlord",
      "formation": null,
      "arc": null
    },
    {
      "id": "YELLOW-47",
      "name": "西尔尔克",
      "sub": " 樱花庸医 ",
      "type": "char",
      "color": "yellow",
      "cost": 1,
      "power": 1000,
      "counter": 2000,
      "keywords": [],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "healLP",
          "x": 1000
        }
      },
      "art": "YELLOW-47",
      "fruit": null,
      "rarity": "B",
      "faction": "warlord",
      "formation": null,
      "arc": null
    },
    {
      "id": "YELLOW-48",
      "name": "库蕾哈",
      "sub": " 医婆 ",
      "type": "char",
      "color": "yellow",
      "cost": 2,
      "power": 3000,
      "counter": 2000,
      "keywords": [],
      "effect": null,
      "art": "YELLOW-48",
      "fruit": null,
      "rarity": "A",
      "faction": "warlord",
      "formation": null,
      "arc": null
    },
    {
      "id": "YELLOW-49",
      "name": "巴索罗缪·熊",
      "sub": " 拍飞 ",
      "type": "char",
      "color": "yellow",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "restEnemy",
          "target": "last"
        }
      },
      "art": "YELLOW-49",
      "fruit": "paramecia",
      "rarity": "S",
      "faction": "warlord",
      "formation": null,
      "arc": null
    },
    {
      "id": "YELLOW-50",
      "name": "月光·莫利亚",
      "sub": " 影子军团 ",
      "type": "char",
      "color": "yellow",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "YELLOW-50",
      "fruit": "paramecia",
      "rarity": "B",
      "faction": "warlord",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "YELLOW-51",
      "name": "格拉迪乌斯",
      "sub": " 泡泡果实 ",
      "type": "char",
      "color": "yellow",
      "cost": 3,
      "power": 4000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "YELLOW-51",
      "fruit": "paramecia",
      "rarity": "A",
      "faction": "warlord",
      "formation": null,
      "arc": "dressrosa"
    },
    {
      "id": "YELLOW-52",
      "name": "德林杰",
      "sub": " 人鱼斗士 ",
      "type": "char",
      "color": "yellow",
      "cost": 2,
      "power": 3000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "YELLOW-52",
      "fruit": null,
      "rarity": "A",
      "faction": "warlord",
      "formation": null,
      "arc": "dressrosa"
    },
    {
      "id": "YELLOW-53",
      "name": "拉奥G",
      "sub": " 格斗拳 ",
      "type": "char",
      "color": "yellow",
      "cost": 4,
      "power": 5000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "YELLOW-53",
      "fruit": null,
      "rarity": "A",
      "faction": "warlord",
      "formation": null,
      "arc": "dressrosa"
    },
    {
      "id": "YELLOW-54",
      "name": "蕾贝卡",
      "sub": " 圆顶斗技场 ",
      "type": "char",
      "color": "yellow",
      "cost": 3,
      "power": 4000,
      "counter": 2000,
      "keywords": [],
      "effect": null,
      "art": "YELLOW-54",
      "fruit": null,
      "rarity": "A",
      "faction": "warlord",
      "formation": null,
      "arc": "dressrosa"
    },
    {
      "id": "YELLOW-55",
      "name": "夏洛特·欧文",
      "sub": " 热热果实 ",
      "type": "char",
      "color": "yellow",
      "cost": 5,
      "power": 6000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "YELLOW-55",
      "fruit": "paramecia",
      "rarity": "A",
      "faction": "yonko",
      "formation": null,
      "arc": "whole_cake"
    },
    {
      "id": "YELLOW-56",
      "name": "夏洛特·布琳",
      "sub": " 三眼族 ",
      "type": "char",
      "color": "yellow",
      "cost": 4,
      "power": 5000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "YELLOW-56",
      "fruit": "paramecia",
      "rarity": "A",
      "faction": "yonko",
      "formation": null,
      "arc": "whole_cake"
    },
    {
      "id": "YELLOW-57",
      "name": "斯莱曼",
      "sub": " 剑斗士 ",
      "type": "char",
      "color": "yellow",
      "cost": 3,
      "power": 4000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "YELLOW-57",
      "fruit": null,
      "rarity": "A",
      "faction": "warlord",
      "formation": null,
      "arc": "dressrosa"
    },
    {
      "id": "YELLOW-G1",
      "name": "黑刀·夜",
      "sub": " 世界最强黑刀 ",
      "type": "gear",
      "color": "yellow",
      "cost": 3,
      "power": null,
      "counter": null,
      "keywords": [],
      "gear": {
        "atk": 3000
      },
      "effect": null,
      "fruit": null,
      "art": "YELLOW-G1",
      "rarity": "S",
      "faction": "warlord",
      "formation": null,
      "arc": null
    },
    {
      "id": "YELLOW-G4",
      "name": "九蛇弓",
      "sub": " 女帝护卫弓 ",
      "type": "gear",
      "color": "yellow",
      "cost": 2,
      "power": null,
      "counter": null,
      "keywords": [],
      "gear": {
        "atk": 1000
      },
      "effect": null,
      "art": "YELLOW-G4",
      "fruit": null,
      "rarity": "A",
      "faction": "warlord",
      "formation": null,
      "arc": null
    },
    {
      "id": "YELLOW-G6",
      "name": "威布尔巨斧",
      "sub": " 白胡子二世战斧 ",
      "type": "gear",
      "color": "yellow",
      "cost": 3,
      "power": null,
      "counter": null,
      "keywords": [],
      "gear": {
        "atk": 2000
      },
      "effect": null,
      "art": "YELLOW-G6",
      "fruit": null,
      "rarity": "B",
      "faction": "yonko",
      "formation": null,
      "arc": null
    },
    {
      "id": "FUSION-RED1",
      "name": "火之意志 艾斯&萨博",
      "sub": " 融合 ",
      "type": "char",
      "color": "red",
      "cost": 8,
      "power": 8000,
      "counter": null,
      "keywords": [
        "rush",
        "doubleAttack"
      ],
      "effect": null,
      "art": "RED-08",
      "fruit": null,
      "fusion": {
        "from": [
          "RED-08",
          "RED-06"
        ],
        "cost": 3
      },
      "rarity": "SSS",
      "faction": "strawhat",
      "formation": "vanguard",
      "arc": "marineford"
    },
    {
      "id": "FUSION-RED2",
      "name": "传承之火 路飞&罗杰",
      "sub": " 融合 ",
      "type": "char",
      "color": "red",
      "cost": 8,
      "power": 8000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "powerSelf",
          "x": 1000,
          "until": "battle"
        }
      },
      "art": "RED-27",
      "fruit": null,
      "fusion": {
        "from": [
          "RED-27",
          "RED-37"
        ],
        "cost": 2
      },
      "rarity": "SSS",
      "faction": "strawhat",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "FUSION-BLUE1",
      "name": "海军的魂 战国&卡普",
      "sub": " 融合 ",
      "type": "char",
      "color": "blue",
      "cost": 8,
      "power": 8000,
      "counter": null,
      "keywords": [
        "rush",
        "blocker"
      ],
      "effect": null,
      "art": "BLUE-11",
      "fruit": null,
      "fusion": {
        "from": [
          "BLUE-36",
          "BLUE-11"
        ],
        "cost": 3
      },
      "rarity": "SSS",
      "faction": "navy",
      "formation": "bulwark",
      "arc": "marineford"
    },
    {
      "id": "FUSION-BLUE2",
      "name": "阳光号的伙伴 娜美&弗兰奇",
      "sub": " 融合 ",
      "type": "char",
      "color": "blue",
      "cost": 8,
      "power": 8000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "buffAll",
          "x": 1000,
          "until": "turn"
        }
      },
      "art": "BLUE-25",
      "fruit": null,
      "fusion": {
        "from": [
          "BLUE-25",
          "BLUE-08"
        ],
        "cost": 3
      },
      "rarity": "SSS",
      "faction": "navy",
      "formation": "vanguard",
      "arc": "enies_lobby"
    },
    {
      "id": "FUSION-GREEN1",
      "name": "鬼岛决战 凯多&大和",
      "sub": " 融合 ",
      "type": "char",
      "color": "green",
      "cost": 8,
      "power": 8000,
      "counter": null,
      "keywords": [
        "rush",
        "banish"
      ],
      "effect": null,
      "art": "GREEN-12",
      "fruit": null,
      "fusion": {
        "from": [
          "GREEN-12",
          "GREEN-37"
        ],
        "cost": 2
      },
      "rarity": "SSS",
      "faction": "strawhat",
      "formation": "vanguard",
      "arc": "wano"
    },
    {
      "id": "FUSION-GREEN2",
      "name": "斩铁之魂 索隆&龙马",
      "sub": " 融合 ",
      "type": "char",
      "color": "green",
      "cost": 8,
      "power": 8000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "powerSelf",
          "x": 1000,
          "until": "battle"
        }
      },
      "art": "GREEN-25",
      "fruit": null,
      "fusion": {
        "from": [
          "GREEN-25",
          "GREEN-58"
        ],
        "cost": 3
      },
      "rarity": "SSS",
      "faction": "strawhat",
      "formation": "vanguard",
      "arc": "wano"
    },
    {
      "id": "FUSION-YELLOW1",
      "name": "九蛇岛的守护 汉库珂&玛格丽特",
      "sub": " 融合 ",
      "type": "char",
      "color": "yellow",
      "cost": 8,
      "power": 8000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "restEnemy"
        }
      },
      "art": "YELLOW-29",
      "fruit": null,
      "fusion": {
        "from": [
          "YELLOW-29",
          "YELLOW-46"
        ],
        "cost": 3
      },
      "rarity": "SSS",
      "faction": "warlord",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "FUSION-YELLOW2",
      "name": "万国的威压 玲玲&斯慕吉",
      "sub": " 融合 ",
      "type": "char",
      "color": "yellow",
      "cost": 8,
      "power": 8000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "damageLP",
          "x": 1000
        }
      },
      "art": "YELLOW-20",
      "fruit": null,
      "fusion": {
        "from": [
          "YELLOW-20",
          "YELLOW-41"
        ],
        "cost": 2
      },
      "rarity": "SSS",
      "faction": "warlord",
      "formation": "vanguard",
      "arc": "whole_cake"
    },
    {
      "id": "FUSION-PURPLE1",
      "name": "最恶世代 罗与基德",
      "sub": " 融合 ",
      "type": "char",
      "color": "purple",
      "cost": 8,
      "power": 8000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "draw",
          "n": 1
        }
      },
      "art": "PURPLE-24",
      "fruit": null,
      "fusion": {
        "from": [
          "PURPLE-24",
          "PURPLE-36"
        ],
        "cost": 3
      },
      "rarity": "SSS",
      "faction": "warlord",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "FUSION-PURPLE2",
      "name": "剑士的顶点 米霍克&雷利",
      "sub": " 融合 ",
      "type": "char",
      "color": "purple",
      "cost": 8,
      "power": 8000,
      "counter": null,
      "keywords": [
        "rush",
        "doubleAttack"
      ],
      "effect": null,
      "art": "PURPLE-10",
      "fruit": null,
      "fusion": {
        "from": [
          "PURPLE-10",
          "PURPLE-19"
        ],
        "cost": 2
      },
      "rarity": "SSS",
      "faction": "warlord",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "FUSION-BLACK1",
      "name": "黑团双巨头 蒂奇&希留",
      "sub": " 融合 ",
      "type": "char",
      "color": "black",
      "cost": 8,
      "power": 8000,
      "counter": null,
      "keywords": [
        "rush",
        "banish"
      ],
      "effect": null,
      "art": "BLACK-26",
      "fruit": null,
      "fusion": {
        "from": [
          "BLACK-26",
          "BLACK-08"
        ],
        "cost": 3
      },
      "rarity": "SSS",
      "faction": "beast",
      "formation": "vanguard",
      "arc": "marineford"
    },
    {
      "id": "FUSION-BLACK2",
      "name": "海底监狱的铁壁 麦哲伦&汉尼拔",
      "sub": " 融合 ",
      "type": "char",
      "color": "black",
      "cost": 8,
      "power": 8000,
      "counter": null,
      "keywords": [
        "rush",
        "blocker"
      ],
      "effect": null,
      "art": "BLACK-31",
      "fruit": null,
      "fusion": {
        "from": [
          "BLACK-31",
          "BLACK-12"
        ],
        "cost": 3
      },
      "rarity": "SSS",
      "faction": "beast",
      "formation": "bulwark",
      "arc": "marineford"
    },
    {
      "id": "RED-65",
      "name": "蕾贝卡",
      "sub": " 新星 ",
      "type": "char",
      "color": "red",
      "cost": 2,
      "power": 2000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "RED-65",
      "fruit": null,
      "rarity": "B",
      "faction": "supernova",
      "formation": "skirmish",
      "arc": "east_blue"
    },
    {
      "id": "RED-66",
      "name": "布拉曼克",
      "sub": " 一番队长 ",
      "type": "char",
      "color": "red",
      "cost": 3,
      "power": 3000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "damageLP",
          "x": 1000
        }
      },
      "art": "RED-66",
      "fruit": null,
      "rarity": "S",
      "faction": "whitebeard",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "RED-67",
      "name": "莫莉",
      "sub": " 最恶世代 ",
      "type": "char",
      "color": "red",
      "cost": 5,
      "power": 5000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "RED-67",
      "fruit": null,
      "rarity": "B",
      "faction": "supernova",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "RED-69",
      "name": "以藏",
      "sub": " 残火 ",
      "type": "char",
      "color": "red",
      "cost": 1,
      "power": 2000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "RED-69",
      "fruit": "zoan",
      "rarity": "A",
      "faction": "whitebeard",
      "formation": "vanguard",
      "arc": "east_blue"
    },
    {
      "id": "RED-70",
      "name": "基拉",
      "sub": " 出头天 ",
      "type": "char",
      "color": "red",
      "cost": 1,
      "power": 1000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "RED-70",
      "fruit": null,
      "rarity": "B",
      "faction": "supernova",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "RED-71",
      "name": "河松",
      "sub": " 赤鞘武士 ",
      "type": "char",
      "color": "red",
      "cost": 4,
      "power": 5000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "RED-71",
      "fruit": null,
      "rarity": "A",
      "faction": "whitebeard",
      "formation": "skirmish",
      "arc": "east_blue"
    },
    {
      "id": "RED-72",
      "name": "波特卡斯·D·艾斯·一番队",
      "sub": " 残火 ",
      "type": "char",
      "color": "red",
      "cost": 1,
      "power": 2000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "RED-72",
      "fruit": "paramecia",
      "rarity": "A",
      "faction": "whitebeard",
      "formation": "skirmish",
      "arc": "east_blue"
    },
    {
      "id": "RED-74",
      "name": "龙马",
      "sub": " 赤鞘武士 ",
      "type": "char",
      "color": "red",
      "cost": 1,
      "power": 2000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "RED-74",
      "fruit": null,
      "rarity": "A",
      "faction": "whitebeard",
      "formation": "skirmish",
      "arc": "east_blue"
    },
    {
      "id": "RED-75",
      "name": "泽法",
      "sub": " 最恶世代 ",
      "type": "char",
      "color": "red",
      "cost": 3,
      "power": 3000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onKO",
        "op": {
          "k": "damageLP",
          "x": 2000
        }
      },
      "art": "RED-75",
      "fruit": "zoan",
      "rarity": "S",
      "faction": "supernova",
      "formation": "skirmish",
      "arc": "east_blue"
    },
    {
      "id": "RED-76",
      "name": "犬岚公爵",
      "sub": " 一番队长 ",
      "type": "char",
      "color": "red",
      "cost": 4,
      "power": 4000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "damageLP",
          "x": 1000
        }
      },
      "art": "RED-76",
      "fruit": "zoan",
      "rarity": "S",
      "faction": "whitebeard",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "RED-77",
      "name": "阿健",
      "sub": " 伙伴之力 ",
      "type": "char",
      "color": "red",
      "cost": 1,
      "power": 1000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "damageLP",
          "x": 2000
        }
      },
      "art": "RED-77",
      "fruit": null,
      "rarity": "S",
      "faction": "strawhat",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "RED-78",
      "name": "可可罗",
      "sub": " 冒险者 ",
      "type": "char",
      "color": "red",
      "cost": 3,
      "power": 4000,
      "counter": 2000,
      "keywords": [],
      "effect": null,
      "art": "RED-78",
      "fruit": "zoan",
      "rarity": "A",
      "faction": "strawhat",
      "formation": "skirmish",
      "arc": "east_blue"
    },
    {
      "id": "RED-79",
      "name": "居鲁士",
      "sub": " 结盟 ",
      "type": "char",
      "color": "red",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [
        "doubleAttack"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "restEnemy"
        }
      },
      "art": "RED-79",
      "fruit": null,
      "rarity": "S",
      "faction": "supernova",
      "formation": null,
      "arc": null
    },
    {
      "id": "RED-80",
      "name": "鬼蜘蛛",
      "sub": " 铁律 ",
      "type": "char",
      "color": "red",
      "cost": 2,
      "power": 3000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "RED-80",
      "fruit": null,
      "rarity": "A",
      "faction": "navy",
      "formation": "skirmish",
      "arc": "east_blue"
    },
    {
      "id": "RED-81",
      "name": "顿·克利克",
      "sub": " 结盟 ",
      "type": "char",
      "color": "red",
      "cost": 3,
      "power": 3000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "RED-81",
      "fruit": "zoan",
      "rarity": "B",
      "faction": "supernova",
      "formation": null,
      "arc": "east_blue"
    },
    {
      "id": "RED-82",
      "name": "罗罗诺亚·索隆",
      "sub": " 伙伴之力 ",
      "type": "char",
      "color": "red",
      "cost": 4,
      "power": 4000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "debuffFoeAll",
          "x": 2000
        }
      },
      "art": "RED-82",
      "fruit": null,
      "rarity": "S",
      "faction": "strawhat",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "RED-83",
      "name": "鼯鼠",
      "sub": " 执法者 ",
      "type": "char",
      "color": "red",
      "cost": 4,
      "power": 4000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "RED-83",
      "fruit": "zoan",
      "rarity": "B",
      "faction": "navy",
      "formation": null,
      "arc": null
    },
    {
      "id": "RED-84",
      "name": "帕帕古",
      "sub": " 扬帆起航 ",
      "type": "char",
      "color": "red",
      "cost": 4,
      "power": 4000,
      "counter": 1000,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "restEnemy"
        }
      },
      "art": "RED-84",
      "fruit": null,
      "rarity": "S",
      "faction": "strawhat",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "RED-85",
      "name": "福萨",
      "sub": " 白胡子一族 ",
      "type": "char",
      "color": "red",
      "cost": 3,
      "power": 4000,
      "counter": 2000,
      "keywords": [],
      "effect": null,
      "art": "RED-85",
      "fruit": null,
      "rarity": "A",
      "faction": "whitebeard",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "RED-86",
      "name": "一笑",
      "sub": " 执法者 ",
      "type": "char",
      "color": "red",
      "cost": 1,
      "power": 2000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "RED-86",
      "fruit": "zoan",
      "rarity": "A",
      "faction": "navy",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "RED-87",
      "name": "乌尔基",
      "sub": " 野心家 ",
      "type": "char",
      "color": "red",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "doubleAttack"
      ],
      "effect": null,
      "art": "RED-87",
      "fruit": null,
      "rarity": "B",
      "faction": "supernova",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "RED-88",
      "name": "黑色玛利亚",
      "sub": " 遗志 ",
      "type": "char",
      "color": "red",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "doubleAttack"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "draw",
          "n": 2
        }
      },
      "art": "RED-88",
      "fruit": null,
      "rarity": "S",
      "faction": "whitebeard",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "RED-89",
      "name": "提娜",
      "sub": " 海军将校 ",
      "type": "char",
      "color": "red",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "RED-89",
      "fruit": null,
      "rarity": "S",
      "faction": "navy",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "RED-91",
      "name": "斯摩格",
      "sub": " 铁律 ",
      "type": "char",
      "color": "red",
      "cost": 4,
      "power": 5000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "RED-91",
      "fruit": null,
      "rarity": "A",
      "faction": "navy",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "RED-92",
      "name": "巴兹尔·霍金斯·最恶世代",
      "sub": " 乱世枭雄 ",
      "type": "char",
      "color": "red",
      "cost": 3,
      "power": 4000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "RED-92",
      "fruit": null,
      "rarity": "A",
      "faction": "supernova",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "RED-93",
      "name": "乔兹·一番队",
      "sub": " 遗志 ",
      "type": "char",
      "color": "red",
      "cost": 3,
      "power": 3000,
      "counter": 1000,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "RED-93",
      "fruit": null,
      "rarity": "B",
      "faction": "whitebeard",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "RED-94",
      "name": "忍",
      "sub": " 赤鞘武士 ",
      "type": "char",
      "color": "red",
      "cost": 5,
      "power": 5000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onKO",
        "op": {
          "k": "damageLP",
          "x": 1000
        }
      },
      "art": "RED-94",
      "fruit": null,
      "rarity": "S",
      "faction": "whitebeard",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "RED-95",
      "name": "凯撒·克朗",
      "sub": " 出头天 ",
      "type": "char",
      "color": "red",
      "cost": 4,
      "power": 5000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "RED-95",
      "fruit": null,
      "rarity": "A",
      "faction": "supernova",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "RED-96",
      "name": "猫蝮蛇",
      "sub": " 家族之绊 ",
      "type": "char",
      "color": "red",
      "cost": 1,
      "power": 1000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "RED-96",
      "fruit": null,
      "rarity": "B",
      "faction": "whitebeard",
      "formation": "vanguard",
      "arc": "east_blue"
    },
    {
      "id": "RED-97",
      "name": "阿拉马基",
      "sub": " 正义之师 ",
      "type": "char",
      "color": "red",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "blocker",
        "rush"
      ],
      "effect": null,
      "art": "RED-97",
      "fruit": null,
      "rarity": "SS",
      "faction": "navy",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "RED-98",
      "name": "哈库",
      "sub": " 最恶世代 ",
      "type": "char",
      "color": "red",
      "cost": 3,
      "power": 4000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "RED-98",
      "fruit": null,
      "rarity": "A",
      "faction": "supernova",
      "formation": "skirmish",
      "arc": "east_blue"
    },
    {
      "id": "RED-99",
      "name": "娜美·新世界",
      "sub": " 冒险王 ",
      "type": "char",
      "color": "red",
      "cost": 4,
      "power": 4000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "RED-99",
      "fruit": "zoan",
      "rarity": "B",
      "faction": "strawhat",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "RED-100",
      "name": "酒天丸",
      "sub": " 家族之绊 ",
      "type": "char",
      "color": "red",
      "cost": 1,
      "power": 1000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "RED-100",
      "fruit": null,
      "rarity": "B",
      "faction": "whitebeard",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "RED-101",
      "name": "小玉",
      "sub": " 残火 ",
      "type": "char",
      "color": "red",
      "cost": 8,
      "power": 8000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "gainDon",
          "n": 1
        }
      },
      "art": "RED-101",
      "fruit": "zoan",
      "rarity": "SSS",
      "faction": "whitebeard",
      "formation": "vanguard",
      "arc": "east_blue"
    },
    {
      "id": "RED-102",
      "name": "邦尼",
      "sub": " 乱世枭雄 ",
      "type": "char",
      "color": "red",
      "cost": 4,
      "power": 5000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "RED-102",
      "fruit": null,
      "rarity": "A",
      "faction": "supernova",
      "formation": null,
      "arc": null
    },
    {
      "id": "RED-103",
      "name": "瓦伊帕",
      "sub": " 冒险王 ",
      "type": "char",
      "color": "red",
      "cost": 4,
      "power": 5000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "RED-103",
      "fruit": null,
      "rarity": "A",
      "faction": "strawhat",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "RED-104",
      "name": "斯库亚德",
      "sub": " 残火 ",
      "type": "char",
      "color": "red",
      "cost": 5,
      "power": 5000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "buffAll",
          "x": 2000
        }
      },
      "art": "RED-104",
      "fruit": null,
      "rarity": "S",
      "faction": "whitebeard",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "RED-105",
      "name": "贝鲁梅特尔",
      "sub": " 铁律 ",
      "type": "char",
      "color": "red",
      "cost": 6,
      "power": 7000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "RED-105",
      "fruit": null,
      "rarity": "A",
      "faction": "navy",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "RED-106",
      "name": "尤斯塔斯·基德",
      "sub": " 结盟 ",
      "type": "char",
      "color": "red",
      "cost": 2,
      "power": 2000,
      "counter": null,
      "keywords": [
        "blocker",
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "gainDon",
          "n": 2
        }
      },
      "art": "RED-106",
      "fruit": null,
      "rarity": "SS",
      "faction": "supernova",
      "formation": "vanguard",
      "arc": "east_blue"
    },
    {
      "id": "RED-107",
      "name": "雷藏",
      "sub": " 赤鞘武士 ",
      "type": "char",
      "color": "red",
      "cost": 5,
      "power": 5000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "draw",
          "n": 2
        }
      },
      "art": "RED-107",
      "fruit": "paramecia",
      "rarity": "S",
      "faction": "whitebeard",
      "formation": "vanguard",
      "arc": "east_blue"
    },
    {
      "id": "RED-108",
      "name": "拉库约",
      "sub": " 白胡子一族 ",
      "type": "char",
      "color": "red",
      "cost": 2,
      "power": 2000,
      "counter": 1000,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "RED-108",
      "fruit": null,
      "rarity": "B",
      "faction": "whitebeard",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "RED-109",
      "name": "托尼托尼·乔巴·新世界",
      "sub": " 扬帆起航 ",
      "type": "char",
      "color": "red",
      "cost": 5,
      "power": 5000,
      "counter": 2000,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "RED-109",
      "fruit": "logia",
      "rarity": "B",
      "faction": "strawhat",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "RED-110",
      "name": "贝鲁梅伯",
      "sub": " 执法者 ",
      "type": "char",
      "color": "red",
      "cost": 3,
      "power": 4000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "RED-110",
      "fruit": null,
      "rarity": "A",
      "faction": "navy",
      "formation": null,
      "arc": null
    },
    {
      "id": "RED-111",
      "name": "X·德雷克",
      "sub": " 结盟 ",
      "type": "char",
      "color": "red",
      "cost": 2,
      "power": 2000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "RED-111",
      "fruit": "logia",
      "rarity": "B",
      "faction": "supernova",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "RED-112",
      "name": "福萨·一番队",
      "sub": " 赤鞘武士 ",
      "type": "char",
      "color": "red",
      "cost": 3,
      "power": 3000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "draw",
          "n": 1
        }
      },
      "art": "RED-112",
      "fruit": null,
      "rarity": "S",
      "faction": "whitebeard",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "RED-113",
      "name": "卡拉斯",
      "sub": " 出头天 ",
      "type": "char",
      "color": "red",
      "cost": 5,
      "power": 5000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "RED-113",
      "fruit": "paramecia",
      "rarity": "B",
      "faction": "supernova",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "RED-114",
      "name": "菊之丞",
      "sub": " 遗志 ",
      "type": "char",
      "color": "red",
      "cost": 5,
      "power": 5000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "debuffFoeAll",
          "x": 2000
        }
      },
      "art": "RED-114",
      "fruit": null,
      "rarity": "S",
      "faction": "whitebeard",
      "formation": "skirmish",
      "arc": "east_blue"
    },
    {
      "id": "RED-115",
      "name": "耕四郎",
      "sub": " 一番队长 ",
      "type": "char",
      "color": "red",
      "cost": 5,
      "power": 5000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "RED-115",
      "fruit": null,
      "rarity": "B",
      "faction": "whitebeard",
      "formation": null,
      "arc": null
    },
    {
      "id": "RED-116",
      "name": "比斯塔",
      "sub": " 一番队长 ",
      "type": "char",
      "color": "red",
      "cost": 4,
      "power": 4000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "RED-116",
      "fruit": "zoan",
      "rarity": "B",
      "faction": "whitebeard",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "RED-117",
      "name": "堪十郎",
      "sub": " 遗志 ",
      "type": "char",
      "color": "red",
      "cost": 2,
      "power": 3000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "RED-117",
      "fruit": null,
      "rarity": "A",
      "faction": "whitebeard",
      "formation": "vanguard",
      "arc": "east_blue"
    },
    {
      "id": "RED-118",
      "name": "蒙奇·D·路飞·新世界",
      "sub": " 自由之海 ",
      "type": "char",
      "color": "red",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "RED-118",
      "fruit": null,
      "rarity": "S",
      "faction": "strawhat",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "RED-119",
      "name": "缇娜",
      "sub": " 执法者 ",
      "type": "char",
      "color": "red",
      "cost": 3,
      "power": 3000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "healLP",
          "x": 2000
        }
      },
      "art": "RED-119",
      "fruit": null,
      "rarity": "S",
      "faction": "navy",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "RED-120",
      "name": "拉布",
      "sub": " 伙伴之力 ",
      "type": "char",
      "color": "red",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "RED-120",
      "fruit": null,
      "rarity": "S",
      "faction": "strawhat",
      "formation": "skirmish",
      "arc": "east_blue"
    },
    {
      "id": "RED-121",
      "name": "乌尔基·最恶世代",
      "sub": " 野心家 ",
      "type": "char",
      "color": "red",
      "cost": 2,
      "power": 3000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "RED-121",
      "fruit": null,
      "rarity": "A",
      "faction": "supernova",
      "formation": null,
      "arc": null
    },
    {
      "id": "RED-123",
      "name": "杰克",
      "sub": " 赤鞘武士 ",
      "type": "char",
      "color": "red",
      "cost": 2,
      "power": 3000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "RED-123",
      "fruit": null,
      "rarity": "A",
      "faction": "whitebeard",
      "formation": "vanguard",
      "arc": "east_blue"
    },
    {
      "id": "RED-124",
      "name": "贝波",
      "sub": " 新星 ",
      "type": "char",
      "color": "red",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "RED-124",
      "fruit": null,
      "rarity": "B",
      "faction": "supernova",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "RED-125",
      "name": "战国",
      "sub": " 正义之师 ",
      "type": "char",
      "color": "red",
      "cost": 2,
      "power": 3000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "RED-125",
      "fruit": null,
      "rarity": "A",
      "faction": "navy",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "RED-126",
      "name": "宾兹",
      "sub": " 乱世枭雄 ",
      "type": "char",
      "color": "red",
      "cost": 1,
      "power": 1000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "damageLP",
          "x": 2000
        }
      },
      "art": "RED-126",
      "fruit": null,
      "rarity": "S",
      "faction": "supernova",
      "formation": null,
      "arc": null
    },
    {
      "id": "RED-127",
      "name": "大和",
      "sub": " 一番队长 ",
      "type": "char",
      "color": "red",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [
        "rush",
        "doubleAttack"
      ],
      "effect": null,
      "art": "RED-127",
      "fruit": null,
      "rarity": "SS",
      "faction": "whitebeard",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "RED-128",
      "name": "T·彭",
      "sub": " 缉捕令 ",
      "type": "char",
      "color": "red",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "RED-128",
      "fruit": null,
      "rarity": "B",
      "faction": "navy",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "RED-129",
      "name": "尼普顿",
      "sub": " 冒险王 ",
      "type": "char",
      "color": "red",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "buffAll",
          "x": 1000
        }
      },
      "art": "RED-129",
      "fruit": null,
      "rarity": "S",
      "faction": "strawhat",
      "formation": "skirmish",
      "arc": "east_blue"
    },
    {
      "id": "RED-130",
      "name": "赞高",
      "sub": " 缉捕令 ",
      "type": "char",
      "color": "red",
      "cost": 5,
      "power": 5000,
      "counter": 1000,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "RED-130",
      "fruit": null,
      "rarity": "B",
      "faction": "navy",
      "formation": "vanguard",
      "arc": "east_blue"
    },
    {
      "id": "RED-131",
      "name": "弗兰奇·新世界",
      "sub": " 冒险王 ",
      "type": "char",
      "color": "red",
      "cost": 4,
      "power": 4000,
      "counter": 1000,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "RED-131",
      "fruit": "paramecia",
      "rarity": "B",
      "faction": "strawhat",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "RED-132",
      "name": "巴兹尔·霍金斯",
      "sub": " 最恶世代 ",
      "type": "char",
      "color": "red",
      "cost": 1,
      "power": 2000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "RED-132",
      "fruit": null,
      "rarity": "A",
      "faction": "supernova",
      "formation": "bulwark",
      "arc": "east_blue"
    },
    {
      "id": "RED-133",
      "name": "妮可·罗宾·新世界",
      "sub": " 冒险王 ",
      "type": "char",
      "color": "red",
      "cost": 4,
      "power": 4000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "RED-133",
      "fruit": null,
      "rarity": "B",
      "faction": "strawhat",
      "formation": "vanguard",
      "arc": "east_blue"
    },
    {
      "id": "RED-135",
      "name": "贝加庞克",
      "sub": " 出头天 ",
      "type": "char",
      "color": "red",
      "cost": 1,
      "power": 1000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "RED-135",
      "fruit": null,
      "rarity": "B",
      "faction": "supernova",
      "formation": null,
      "arc": "east_blue"
    },
    {
      "id": "RED-136",
      "name": "飞六胞",
      "sub": " 家族之绊 ",
      "type": "char",
      "color": "red",
      "cost": 8,
      "power": 8000,
      "counter": null,
      "keywords": [
        "doubleAttack"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "search",
          "n": 1,
          "faction": "whitebeard"
        }
      },
      "art": "RED-136",
      "fruit": null,
      "rarity": "SSS",
      "faction": "whitebeard",
      "formation": null,
      "arc": null
    },
    {
      "id": "RED-137",
      "name": "爱德华·纽盖特·一番队",
      "sub": " 一番队长 ",
      "type": "char",
      "color": "red",
      "cost": 8,
      "power": 8000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "search",
          "n": 1,
          "faction": "whitebeard"
        }
      },
      "art": "RED-137",
      "fruit": null,
      "rarity": "SSS",
      "faction": "whitebeard",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "RED-138",
      "name": "布鲁克·新世界",
      "sub": " 伙伴之力 ",
      "type": "char",
      "color": "red",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "buffAll",
          "x": 2000
        }
      },
      "art": "RED-138",
      "fruit": "zoan",
      "rarity": "S",
      "faction": "strawhat",
      "formation": "skirmish",
      "arc": "east_blue"
    },
    {
      "id": "RED-139",
      "name": "修佐",
      "sub": " 出头天 ",
      "type": "char",
      "color": "red",
      "cost": 2,
      "power": 3000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "RED-139",
      "fruit": "paramecia",
      "rarity": "A",
      "faction": "supernova",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "RED-140",
      "name": "乔艾莉·波妮",
      "sub": " 野心家 ",
      "type": "char",
      "color": "red",
      "cost": 1,
      "power": 1000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "RED-140",
      "fruit": null,
      "rarity": "B",
      "faction": "supernova",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "RED-141",
      "name": "阿金",
      "sub": " 出头天 ",
      "type": "char",
      "color": "red",
      "cost": 5,
      "power": 6000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "RED-141",
      "fruit": "paramecia",
      "rarity": "A",
      "faction": "supernova",
      "formation": "skirmish",
      "arc": "east_blue"
    },
    {
      "id": "RED-142",
      "name": "以藏·一番队",
      "sub": " 家族之绊 ",
      "type": "char",
      "color": "red",
      "cost": 2,
      "power": 2000,
      "counter": 1000,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "RED-142",
      "fruit": null,
      "rarity": "B",
      "faction": "whitebeard",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "RED-143",
      "name": "维尔戈",
      "sub": " 巡逻舰队 ",
      "type": "char",
      "color": "red",
      "cost": 3,
      "power": 4000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "RED-143",
      "fruit": null,
      "rarity": "A",
      "faction": "navy",
      "formation": "skirmish",
      "arc": "east_blue"
    },
    {
      "id": "RED-144",
      "name": "萨奇·一番队",
      "sub": " 家族之绊 ",
      "type": "char",
      "color": "red",
      "cost": 2,
      "power": 2000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onKO",
        "op": {
          "k": "powerSelf",
          "x": 1000,
          "until": "battle"
        }
      },
      "art": "RED-144",
      "fruit": null,
      "rarity": "S",
      "faction": "whitebeard",
      "formation": "skirmish",
      "arc": "east_blue"
    },
    {
      "id": "RED-145",
      "name": "布鲁克",
      "sub": " 伙伴之力 ",
      "type": "char",
      "color": "red",
      "cost": 1,
      "power": 1000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "RED-145",
      "fruit": null,
      "rarity": "B",
      "faction": "strawhat",
      "formation": "skirmish",
      "arc": "east_blue"
    },
    {
      "id": "RED-147",
      "name": "X·德雷克·最恶世代",
      "sub": " 新星 ",
      "type": "char",
      "color": "red",
      "cost": 5,
      "power": 5000,
      "counter": 1000,
      "keywords": [
        "blocker",
        "blocker"
      ],
      "effect": null,
      "art": "RED-147",
      "fruit": null,
      "rarity": "SS",
      "faction": "supernova",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "RED-148",
      "name": "日和",
      "sub": " 遗志 ",
      "type": "char",
      "color": "red",
      "cost": 4,
      "power": 4000,
      "counter": 1000,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onKO",
        "op": {
          "k": "draw",
          "n": 1
        }
      },
      "art": "RED-148",
      "fruit": "zoan",
      "rarity": "S",
      "faction": "whitebeard",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "RED-149",
      "name": "卡库",
      "sub": " 正义之师 ",
      "type": "char",
      "color": "red",
      "cost": 3,
      "power": 3000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "gainDon",
          "n": 2
        }
      },
      "art": "RED-149",
      "fruit": "paramecia",
      "rarity": "S",
      "faction": "navy",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "RED-150",
      "name": "林德伯格",
      "sub": " 结盟 ",
      "type": "char",
      "color": "red",
      "cost": 8,
      "power": 8000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "RED-150",
      "fruit": "paramecia",
      "rarity": "SS",
      "faction": "supernova",
      "formation": null,
      "arc": null
    },
    {
      "id": "RED-151",
      "name": "冰山",
      "sub": " 扬帆起航 ",
      "type": "char",
      "color": "red",
      "cost": 2,
      "power": 3000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "RED-151",
      "fruit": "zoan",
      "rarity": "A",
      "faction": "strawhat",
      "formation": "vanguard",
      "arc": "east_blue"
    },
    {
      "id": "RED-152",
      "name": "天羽羽斩",
      "sub": " 遗志 ",
      "type": "char",
      "color": "red",
      "cost": 8,
      "power": 8000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "draw",
          "n": 1
        }
      },
      "art": "RED-152",
      "fruit": null,
      "rarity": "SSS",
      "faction": "whitebeard",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "RED-154",
      "name": "佩金",
      "sub": " 乱世枭雄 ",
      "type": "char",
      "color": "red",
      "cost": 5,
      "power": 5000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "gainDon",
          "n": 2
        }
      },
      "art": "RED-154",
      "fruit": null,
      "rarity": "S",
      "faction": "supernova",
      "formation": null,
      "arc": null
    },
    {
      "id": "RED-155",
      "name": "马尔科·一番队",
      "sub": " 白胡子一族 ",
      "type": "char",
      "color": "red",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "RED-155",
      "fruit": "zoan",
      "rarity": "S",
      "faction": "whitebeard",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "RED-156",
      "name": "比斯塔·一番队",
      "sub": " 家族之绊 ",
      "type": "char",
      "color": "red",
      "cost": 2,
      "power": 2000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "RED-156",
      "fruit": null,
      "rarity": "B",
      "faction": "whitebeard",
      "formation": "skirmish",
      "arc": "east_blue"
    },
    {
      "id": "RED-157",
      "name": "古伊娜",
      "sub": " 赤鞘武士 ",
      "type": "char",
      "color": "red",
      "cost": 4,
      "power": 4000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "healLP",
          "x": 2000
        }
      },
      "art": "RED-157",
      "fruit": null,
      "rarity": "S",
      "faction": "whitebeard",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "RED-158",
      "name": "传次郎",
      "sub": " 残火 ",
      "type": "char",
      "color": "red",
      "cost": 4,
      "power": 4000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "RED-158",
      "fruit": null,
      "rarity": "B",
      "faction": "whitebeard",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "RED-160",
      "name": "甚平",
      "sub": " 草帽一伙 ",
      "type": "char",
      "color": "red",
      "cost": 4,
      "power": 4000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "healLP",
          "x": 1000
        }
      },
      "art": "RED-160",
      "fruit": null,
      "rarity": "S",
      "faction": "strawhat",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "RED-161",
      "name": "火烧山",
      "sub": " 缉捕令 ",
      "type": "char",
      "color": "red",
      "cost": 3,
      "power": 3000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "restEnemy"
        }
      },
      "art": "RED-161",
      "fruit": null,
      "rarity": "S",
      "faction": "navy",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "RED-162",
      "name": "乌索普·新世界",
      "sub": " 自由之海 ",
      "type": "char",
      "color": "red",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "draw",
          "n": 1
        }
      },
      "art": "RED-162",
      "fruit": null,
      "rarity": "S",
      "faction": "strawhat",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "RED-163",
      "name": "拳骨",
      "sub": " 新星 ",
      "type": "char",
      "color": "red",
      "cost": 3,
      "power": 3000,
      "counter": 2000,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "RED-163",
      "fruit": "paramecia",
      "rarity": "B",
      "faction": "supernova",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "RED-164",
      "name": "尤斯塔斯·基德·最恶世代",
      "sub": " 结盟 ",
      "type": "char",
      "color": "red",
      "cost": 5,
      "power": 5000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "RED-164",
      "fruit": null,
      "rarity": "B",
      "faction": "supernova",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "RED-166",
      "name": "杀手",
      "sub": " 结盟 ",
      "type": "char",
      "color": "red",
      "cost": 4,
      "power": 4000,
      "counter": 1000,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "draw",
          "n": 2
        }
      },
      "art": "RED-166",
      "fruit": "paramecia",
      "rarity": "S",
      "faction": "supernova",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "RED-167",
      "name": "佐佐木",
      "sub": " 白胡子一族 ",
      "type": "char",
      "color": "red",
      "cost": 3,
      "power": 3000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "restEnemy"
        }
      },
      "art": "RED-167",
      "fruit": null,
      "rarity": "S",
      "faction": "whitebeard",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "RED-168",
      "name": "路奇",
      "sub": " 巡逻舰队 ",
      "type": "char",
      "color": "red",
      "cost": 2,
      "power": 3000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "RED-168",
      "fruit": null,
      "rarity": "A",
      "faction": "navy",
      "formation": null,
      "arc": null
    },
    {
      "id": "RED-169",
      "name": "托尼托尼·乔巴",
      "sub": " 伙伴之力 ",
      "type": "char",
      "color": "red",
      "cost": 5,
      "power": 5000,
      "counter": 1000,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "healLP",
          "x": 2000
        }
      },
      "art": "RED-169",
      "fruit": null,
      "rarity": "S",
      "faction": "strawhat",
      "formation": "skirmish",
      "arc": "east_blue"
    },
    {
      "id": "RED-171",
      "name": "孔美奥",
      "sub": " 巡逻舰队 ",
      "type": "char",
      "color": "red",
      "cost": 7,
      "power": 8000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "RED-171",
      "fruit": "zoan",
      "rarity": "S",
      "faction": "navy",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "RED-172",
      "name": "娜美",
      "sub": " 冒险者 ",
      "type": "char",
      "color": "red",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "debuffFoeAll",
          "x": 1000
        }
      },
      "art": "RED-172",
      "fruit": null,
      "rarity": "S",
      "faction": "strawhat",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "RED-173",
      "name": "阿托摩斯",
      "sub": " 白胡子一族 ",
      "type": "char",
      "color": "red",
      "cost": 5,
      "power": 5000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "debuffFoeAll",
          "x": 1000
        }
      },
      "art": "RED-173",
      "fruit": "paramecia",
      "rarity": "S",
      "faction": "whitebeard",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "RED-174",
      "name": "乔兹",
      "sub": " 一番队长 ",
      "type": "char",
      "color": "red",
      "cost": 5,
      "power": 5000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "restEnemy"
        }
      },
      "art": "RED-174",
      "fruit": null,
      "rarity": "S",
      "faction": "whitebeard",
      "formation": "skirmish",
      "arc": "east_blue"
    },
    {
      "id": "RED-175",
      "name": "卡鲁",
      "sub": " 扬帆起航 ",
      "type": "char",
      "color": "red",
      "cost": 2,
      "power": 2000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "draw",
          "n": 1
        }
      },
      "art": "RED-175",
      "fruit": null,
      "rarity": "S",
      "faction": "strawhat",
      "formation": "bulwark",
      "arc": "east_blue"
    },
    {
      "id": "RED-176",
      "name": "弗兰奇",
      "sub": " 冒险王 ",
      "type": "char",
      "color": "red",
      "cost": 1,
      "power": 2000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "RED-176",
      "fruit": null,
      "rarity": "A",
      "faction": "strawhat",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "RED-177",
      "name": "斯莱曼",
      "sub": " 乱世枭雄 ",
      "type": "char",
      "color": "red",
      "cost": 5,
      "power": 5000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "RED-177",
      "fruit": null,
      "rarity": "B",
      "faction": "supernova",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "RED-178",
      "name": "罗罗诺亚·索隆·新世界",
      "sub": " 自由之海 ",
      "type": "char",
      "color": "red",
      "cost": 5,
      "power": 5000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "RED-178",
      "fruit": "zoan",
      "rarity": "B",
      "faction": "strawhat",
      "formation": null,
      "arc": null
    },
    {
      "id": "RED-180",
      "name": "乙姬",
      "sub": " 冒险王 ",
      "type": "char",
      "color": "red",
      "cost": 4,
      "power": 4000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "restEnemy"
        }
      },
      "art": "RED-180",
      "fruit": "paramecia",
      "rarity": "S",
      "faction": "strawhat",
      "formation": "skirmish",
      "arc": "east_blue"
    },
    {
      "id": "RED-181",
      "name": "莱德菲尔德",
      "sub": " 野心家 ",
      "type": "char",
      "color": "red",
      "cost": 4,
      "power": 4000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "RED-181",
      "fruit": null,
      "rarity": "B",
      "faction": "supernova",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "BLUE-65",
      "name": "Mr.3",
      "sub": " 处刑人 ",
      "type": "char",
      "color": "blue",
      "cost": 2,
      "power": 2000,
      "counter": 1000,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "damageLP",
          "x": 1000
        }
      },
      "art": "BLUE-65",
      "fruit": null,
      "rarity": "S",
      "faction": "warlord",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLUE-66",
      "name": "砂糖",
      "sub": " 王下七武海 ",
      "type": "char",
      "color": "blue",
      "cost": 5,
      "power": 5000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "BLUE-66",
      "fruit": null,
      "rarity": "B",
      "faction": "warlord",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "BLUE-67",
      "name": "乔拉可尔·米霍克",
      "sub": " 王下七武海 ",
      "type": "char",
      "color": "blue",
      "cost": 3,
      "power": 3000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "damageLP",
          "x": 1000
        }
      },
      "art": "BLUE-67",
      "fruit": null,
      "rarity": "S",
      "faction": "warlord",
      "formation": "skirmish",
      "arc": "enies_lobby"
    },
    {
      "id": "BLUE-68",
      "name": "波雅·汉库珂",
      "sub": " 夜行 ",
      "type": "char",
      "color": "blue",
      "cost": 4,
      "power": 4000,
      "counter": 1000,
      "keywords": [
        "blocker",
        "blocker"
      ],
      "effect": null,
      "art": "BLUE-68",
      "fruit": "paramecia",
      "rarity": "SS",
      "faction": "warlord",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "BLUE-69",
      "name": "哈雷达斯",
      "sub": " 解放战线 ",
      "type": "char",
      "color": "blue",
      "cost": 3,
      "power": 3000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "damageLP",
          "x": 1000
        }
      },
      "art": "BLUE-69",
      "fruit": null,
      "rarity": "S",
      "faction": "revolutionary",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "BLUE-70",
      "name": "莫莉",
      "sub": " 起义者 ",
      "type": "char",
      "color": "blue",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "rush",
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "damageLP",
          "x": 1000
        }
      },
      "art": "BLUE-70",
      "fruit": null,
      "rarity": "SS",
      "faction": "revolutionary",
      "formation": "skirmish",
      "arc": "enies_lobby"
    },
    {
      "id": "BLUE-71",
      "name": "基德娜",
      "sub": " 解放战线 ",
      "type": "char",
      "color": "blue",
      "cost": 4,
      "power": 4000,
      "counter": 2000,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "restEnemy"
        }
      },
      "art": "BLUE-71",
      "fruit": null,
      "rarity": "S",
      "faction": "revolutionary",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "BLUE-72",
      "name": "卡拉斯·革命军",
      "sub": " 疾风 ",
      "type": "char",
      "color": "blue",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "BLUE-72",
      "fruit": "paramecia",
      "rarity": "B",
      "faction": "revolutionary",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "BLUE-73",
      "name": "皮卡",
      "sub": " 处刑人 ",
      "type": "char",
      "color": "blue",
      "cost": 5,
      "power": 5000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "buffAll",
          "x": 2000
        }
      },
      "art": "BLUE-73",
      "fruit": null,
      "rarity": "S",
      "faction": "warlord",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLUE-74",
      "name": "夏姆洛克",
      "sub": " 起义者 ",
      "type": "char",
      "color": "blue",
      "cost": 5,
      "power": 5000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "restEnemy",
          "target": "strongest"
        }
      },
      "art": "BLUE-74",
      "fruit": null,
      "rarity": "S",
      "faction": "revolutionary",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLUE-77",
      "name": "莫奈",
      "sub": " 孤高之刃 ",
      "type": "char",
      "color": "blue",
      "cost": 7,
      "power": 8000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "BLUE-77",
      "fruit": null,
      "rarity": "S",
      "faction": "warlord",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "BLUE-78",
      "name": "东塔克斯三世",
      "sub": " 风之军 ",
      "type": "char",
      "color": "blue",
      "cost": 3,
      "power": 4000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "BLUE-78",
      "fruit": "logia",
      "rarity": "A",
      "faction": "revolutionary",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "BLUE-79",
      "name": "维奥莱特",
      "sub": " 暗流涌动 ",
      "type": "char",
      "color": "blue",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "damageLP",
          "x": 1000
        }
      },
      "art": "BLUE-79",
      "fruit": null,
      "rarity": "S",
      "faction": "warlord",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "BLUE-80",
      "name": "一笑",
      "sub": " 海军将校 ",
      "type": "char",
      "color": "blue",
      "cost": 4,
      "power": 4000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "draw",
          "n": 1
        }
      },
      "art": "BLUE-80",
      "fruit": "paramecia",
      "rarity": "S",
      "faction": "navy",
      "formation": "bulwark",
      "arc": "enies_lobby"
    },
    {
      "id": "BLUE-81",
      "name": "贝比5",
      "sub": " 孤高之刃 ",
      "type": "char",
      "color": "blue",
      "cost": 4,
      "power": 5000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "BLUE-81",
      "fruit": "zoan",
      "rarity": "A",
      "faction": "warlord",
      "formation": null,
      "arc": "enies_lobby"
    },
    {
      "id": "BLUE-82",
      "name": "小八",
      "sub": " 隐密行动 ",
      "type": "char",
      "color": "blue",
      "cost": 5,
      "power": 5000,
      "counter": 1000,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "buffAll",
          "x": 2000
        }
      },
      "art": "BLUE-82",
      "fruit": null,
      "rarity": "S",
      "faction": "revolutionary",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "BLUE-85",
      "name": "安布里奥·伊万科夫",
      "sub": " 风之军 ",
      "type": "char",
      "color": "blue",
      "cost": 4,
      "power": 4000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "BLUE-85",
      "fruit": null,
      "rarity": "B",
      "faction": "revolutionary",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "BLUE-86",
      "name": "萨博·革命军",
      "sub": " 起义者 ",
      "type": "char",
      "color": "blue",
      "cost": 4,
      "power": 4000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "BLUE-86",
      "fruit": "paramecia",
      "rarity": "B",
      "faction": "revolutionary",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLUE-87",
      "name": "贝洛·贝蒂·革命军",
      "sub": " 疾风 ",
      "type": "char",
      "color": "blue",
      "cost": 3,
      "power": 3000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "BLUE-87",
      "fruit": null,
      "rarity": "B",
      "faction": "revolutionary",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "BLUE-88",
      "name": "卡拉斯",
      "sub": " 疾风 ",
      "type": "char",
      "color": "blue",
      "cost": 5,
      "power": 6000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "BLUE-88",
      "fruit": null,
      "rarity": "A",
      "faction": "revolutionary",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "BLUE-90",
      "name": "Miss圣诞快乐",
      "sub": " 王下七武海 ",
      "type": "char",
      "color": "blue",
      "cost": 4,
      "power": 4000,
      "counter": 1000,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "debuffFoeAll",
          "x": 2000
        }
      },
      "art": "BLUE-90",
      "fruit": null,
      "rarity": "S",
      "faction": "warlord",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "BLUE-91",
      "name": "鹤·大将",
      "sub": " 正义之师 ",
      "type": "char",
      "color": "blue",
      "cost": 4,
      "power": 4000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "restEnemy"
        }
      },
      "art": "BLUE-91",
      "fruit": null,
      "rarity": "S",
      "faction": "navy",
      "formation": "bulwark",
      "arc": "enies_lobby"
    },
    {
      "id": "BLUE-92",
      "name": "Mr.1",
      "sub": " 孤高之刃 ",
      "type": "char",
      "color": "blue",
      "cost": 2,
      "power": 2000,
      "counter": 1000,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "BLUE-92",
      "fruit": null,
      "rarity": "B",
      "faction": "warlord",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "BLUE-93",
      "name": "玛丽哥德",
      "sub": " 暗流涌动 ",
      "type": "char",
      "color": "blue",
      "cost": 5,
      "power": 5000,
      "counter": 1000,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "gainDon",
          "n": 2
        }
      },
      "art": "BLUE-93",
      "fruit": "paramecia",
      "rarity": "S",
      "faction": "warlord",
      "formation": "skirmish",
      "arc": "enies_lobby"
    },
    {
      "id": "BLUE-94",
      "name": "爱德华·威布尔",
      "sub": " 王下七武海 ",
      "type": "char",
      "color": "blue",
      "cost": 2,
      "power": 2000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "BLUE-94",
      "fruit": null,
      "rarity": "B",
      "faction": "warlord",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "BLUE-96",
      "name": "巴索罗缪·熊",
      "sub": " 孤高之刃 ",
      "type": "char",
      "color": "blue",
      "cost": 5,
      "power": 6000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "BLUE-96",
      "fruit": null,
      "rarity": "A",
      "faction": "warlord",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLUE-97",
      "name": "闪电·革命军",
      "sub": " 革命军干部 ",
      "type": "char",
      "color": "blue",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "restEnemy"
        }
      },
      "art": "BLUE-97",
      "fruit": null,
      "rarity": "S",
      "faction": "revolutionary",
      "formation": "skirmish",
      "arc": "enies_lobby"
    },
    {
      "id": "BLUE-98",
      "name": "佩罗娜",
      "sub": " 王下七武海 ",
      "type": "char",
      "color": "blue",
      "cost": 2,
      "power": 2000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "BLUE-98",
      "fruit": null,
      "rarity": "B",
      "faction": "warlord",
      "formation": "skirmish",
      "arc": "enies_lobby"
    },
    {
      "id": "BLUE-99",
      "name": "波尔萨利诺·大将",
      "sub": " 铁律 ",
      "type": "char",
      "color": "blue",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [
        "doubleAttack"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "buffAll",
          "x": 1000
        }
      },
      "art": "BLUE-99",
      "fruit": "paramecia",
      "rarity": "S",
      "faction": "navy",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "BLUE-102",
      "name": "莫莉·革命军",
      "sub": " 起义者 ",
      "type": "char",
      "color": "blue",
      "cost": 3,
      "power": 3000,
      "counter": 1000,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "draw",
          "n": 1
        }
      },
      "art": "BLUE-102",
      "fruit": null,
      "rarity": "S",
      "faction": "revolutionary",
      "formation": "skirmish",
      "arc": "enies_lobby"
    },
    {
      "id": "BLUE-103",
      "name": "T·彭",
      "sub": " 执法者 ",
      "type": "char",
      "color": "blue",
      "cost": 3,
      "power": 3000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "BLUE-103",
      "fruit": null,
      "rarity": "B",
      "faction": "navy",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "BLUE-104",
      "name": "Miss黄金周",
      "sub": " 夜行 ",
      "type": "char",
      "color": "blue",
      "cost": 5,
      "power": 5000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "healLP",
          "x": 2000
        }
      },
      "art": "BLUE-104",
      "fruit": null,
      "rarity": "S",
      "faction": "warlord",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "BLUE-105",
      "name": "鼯鼠",
      "sub": " 巡逻舰队 ",
      "type": "char",
      "color": "blue",
      "cost": 1,
      "power": 2000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "BLUE-105",
      "fruit": null,
      "rarity": "A",
      "faction": "navy",
      "formation": "skirmish",
      "arc": "enies_lobby"
    },
    {
      "id": "BLUE-106",
      "name": "库赞",
      "sub": " 正义之师 ",
      "type": "char",
      "color": "blue",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "debuffFoeAll",
          "x": 2000
        }
      },
      "art": "BLUE-106",
      "fruit": "logia",
      "rarity": "S",
      "faction": "navy",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "BLUE-107",
      "name": "缇娜",
      "sub": " 正义之师 ",
      "type": "char",
      "color": "blue",
      "cost": 3,
      "power": 4000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "BLUE-107",
      "fruit": null,
      "rarity": "A",
      "faction": "navy",
      "formation": "skirmish",
      "arc": "enies_lobby"
    },
    {
      "id": "BLUE-108",
      "name": "阿拉马基·大将",
      "sub": " 巡逻舰队 ",
      "type": "char",
      "color": "blue",
      "cost": 2,
      "power": 2000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "BLUE-108",
      "fruit": null,
      "rarity": "B",
      "faction": "navy",
      "formation": "bulwark",
      "arc": "enies_lobby"
    },
    {
      "id": "BLUE-109",
      "name": "强纳森",
      "sub": " 铁律 ",
      "type": "char",
      "color": "blue",
      "cost": 8,
      "power": 8000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "restEnemy"
        }
      },
      "art": "BLUE-109",
      "fruit": null,
      "rarity": "SSS",
      "faction": "navy",
      "formation": "skirmish",
      "arc": "enies_lobby"
    },
    {
      "id": "BLUE-111",
      "name": "孔美奥",
      "sub": " 执法者 ",
      "type": "char",
      "color": "blue",
      "cost": 5,
      "power": 5000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "draw",
          "n": 1
        }
      },
      "art": "BLUE-111",
      "fruit": null,
      "rarity": "S",
      "faction": "navy",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "BLUE-113",
      "name": "克尔拉",
      "sub": " 革命军干部 ",
      "type": "char",
      "color": "blue",
      "cost": 3,
      "power": 3000,
      "counter": 1000,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "healLP",
          "x": 2000
        }
      },
      "art": "BLUE-113",
      "fruit": null,
      "rarity": "S",
      "faction": "revolutionary",
      "formation": null,
      "arc": "enies_lobby"
    },
    {
      "id": "BLUE-114",
      "name": "迪亚玛蒂",
      "sub": " 王下七武海 ",
      "type": "char",
      "color": "blue",
      "cost": 5,
      "power": 5000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "powerSelf",
          "x": 1000,
          "until": "battle"
        }
      },
      "art": "BLUE-114",
      "fruit": "zoan",
      "rarity": "S",
      "faction": "warlord",
      "formation": "bulwark",
      "arc": "enies_lobby"
    },
    {
      "id": "BLUE-116",
      "name": "萨博",
      "sub": " 革命军干部 ",
      "type": "char",
      "color": "blue",
      "cost": 3,
      "power": 4000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "BLUE-116",
      "fruit": null,
      "rarity": "A",
      "faction": "revolutionary",
      "formation": "vanguard",
      "arc": "enies_lobby"
    },
    {
      "id": "BLUE-117",
      "name": "桑达索尼亚",
      "sub": " 暗流涌动 ",
      "type": "char",
      "color": "blue",
      "cost": 2,
      "power": 2000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "BLUE-117",
      "fruit": "paramecia",
      "rarity": "B",
      "faction": "warlord",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "BLUE-118",
      "name": "贝鲁梅特尔",
      "sub": " 执法者 ",
      "type": "char",
      "color": "blue",
      "cost": 3,
      "power": 3000,
      "counter": 1000,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onKO",
        "op": {
          "k": "damageLP",
          "x": 1000
        }
      },
      "art": "BLUE-118",
      "fruit": null,
      "rarity": "S",
      "faction": "navy",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "BLUE-119",
      "name": "蒙奇·D·龙",
      "sub": " 隐密行动 ",
      "type": "char",
      "color": "blue",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "draw",
          "n": 2
        }
      },
      "art": "BLUE-119",
      "fruit": null,
      "rarity": "S",
      "faction": "revolutionary",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "BLUE-120",
      "name": "林德伯格",
      "sub": " 解放战线 ",
      "type": "char",
      "color": "blue",
      "cost": 8,
      "power": 9000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "BLUE-120",
      "fruit": null,
      "rarity": "SSS",
      "faction": "revolutionary",
      "formation": null,
      "arc": "enies_lobby"
    },
    {
      "id": "BLUE-127",
      "name": "林德伯格·革命军",
      "sub": " 疾风 ",
      "type": "char",
      "color": "blue",
      "cost": 8,
      "power": 8000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onKO",
        "op": {
          "k": "gainDon",
          "n": 1
        }
      },
      "art": "BLUE-127",
      "fruit": null,
      "rarity": "SSS",
      "faction": "revolutionary",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "BLUE-129",
      "name": "纳尔逊",
      "sub": " 海军将校 ",
      "type": "char",
      "color": "blue",
      "cost": 1,
      "power": 2000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "BLUE-129",
      "fruit": null,
      "rarity": "A",
      "faction": "navy",
      "formation": "skirmish",
      "arc": "enies_lobby"
    },
    {
      "id": "BLUE-130",
      "name": "一笑·大将",
      "sub": " 铁律 ",
      "type": "char",
      "color": "blue",
      "cost": 3,
      "power": 4000,
      "counter": 2000,
      "keywords": [],
      "effect": null,
      "art": "BLUE-130",
      "fruit": null,
      "rarity": "A",
      "faction": "navy",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLUE-131",
      "name": "萨卡斯基·大将",
      "sub": " 执法者 ",
      "type": "char",
      "color": "blue",
      "cost": 5,
      "power": 6000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "BLUE-131",
      "fruit": "zoan",
      "rarity": "A",
      "faction": "navy",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "BLUE-135",
      "name": "火烧山",
      "sub": " 巡逻舰队 ",
      "type": "char",
      "color": "blue",
      "cost": 4,
      "power": 5000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "BLUE-135",
      "fruit": null,
      "rarity": "A",
      "faction": "navy",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "BLUE-136",
      "name": "路奇",
      "sub": " 海军将校 ",
      "type": "char",
      "color": "blue",
      "cost": 1,
      "power": 2000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "BLUE-136",
      "fruit": null,
      "rarity": "A",
      "faction": "navy",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "BLUE-137",
      "name": "布鲁诺",
      "sub": " 执法者 ",
      "type": "char",
      "color": "blue",
      "cost": 4,
      "power": 5000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "BLUE-137",
      "fruit": null,
      "rarity": "A",
      "faction": "navy",
      "formation": "bulwark",
      "arc": "enies_lobby"
    },
    {
      "id": "BLUE-138",
      "name": "阿布萨罗姆",
      "sub": " 傀儡师 ",
      "type": "char",
      "color": "blue",
      "cost": 4,
      "power": 4000,
      "counter": 1000,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "debuffFoeAll",
          "x": 2000
        }
      },
      "art": "BLUE-138",
      "fruit": null,
      "rarity": "S",
      "faction": "warlord",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "BLUE-139",
      "name": "克尔拉·革命军",
      "sub": " 隐密行动 ",
      "type": "char",
      "color": "blue",
      "cost": 3,
      "power": 3000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "restEnemy"
        }
      },
      "art": "BLUE-139",
      "fruit": null,
      "rarity": "S",
      "faction": "revolutionary",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "BLUE-140",
      "name": "库赞·大将",
      "sub": " 缉捕令 ",
      "type": "char",
      "color": "blue",
      "cost": 3,
      "power": 3000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onKO",
        "op": {
          "k": "healLP",
          "x": 1000
        }
      },
      "art": "BLUE-140",
      "fruit": "paramecia",
      "rarity": "S",
      "faction": "navy",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "BLUE-141",
      "name": "月光·莫利亚",
      "sub": " 暗流涌动 ",
      "type": "char",
      "color": "blue",
      "cost": 4,
      "power": 5000,
      "counter": 2000,
      "keywords": [],
      "effect": null,
      "art": "BLUE-141",
      "fruit": null,
      "rarity": "A",
      "faction": "warlord",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "BLUE-142",
      "name": "安布里奥·伊万科夫·革命军",
      "sub": " 隐密行动 ",
      "type": "char",
      "color": "blue",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "blocker",
        "rush"
      ],
      "effect": null,
      "art": "BLUE-142",
      "fruit": null,
      "rarity": "SS",
      "faction": "revolutionary",
      "formation": null,
      "arc": "enies_lobby"
    },
    {
      "id": "BLUE-144",
      "name": "蒙奇·D·龙·革命军",
      "sub": " 隐密行动 ",
      "type": "char",
      "color": "blue",
      "cost": 3,
      "power": 3000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "BLUE-144",
      "fruit": null,
      "rarity": "B",
      "faction": "revolutionary",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "BLUE-145",
      "name": "摩兹",
      "sub": " 解放战线 ",
      "type": "char",
      "color": "blue",
      "cost": 3,
      "power": 3000,
      "counter": 2000,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "BLUE-145",
      "fruit": null,
      "rarity": "B",
      "faction": "revolutionary",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "BLUE-146",
      "name": "维尔戈",
      "sub": " 铁律 ",
      "type": "char",
      "color": "blue",
      "cost": 5,
      "power": 5000,
      "counter": null,
      "keywords": [
        "rush",
        "rush"
      ],
      "effect": null,
      "art": "BLUE-146",
      "fruit": null,
      "rarity": "SS",
      "faction": "navy",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "BLUE-148",
      "name": "特雷波尔",
      "sub": " 暗流涌动 ",
      "type": "char",
      "color": "blue",
      "cost": 3,
      "power": 3000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "gainDon",
          "n": 1
        }
      },
      "art": "BLUE-148",
      "fruit": "paramecia",
      "rarity": "S",
      "faction": "warlord",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "BLUE-149",
      "name": "闪电",
      "sub": " 解放战线 ",
      "type": "char",
      "color": "blue",
      "cost": 3,
      "power": 4000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "BLUE-149",
      "fruit": null,
      "rarity": "A",
      "faction": "revolutionary",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "BLUE-150",
      "name": "阿拉马基",
      "sub": " 铁律 ",
      "type": "char",
      "color": "blue",
      "cost": 8,
      "power": 9000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "BLUE-150",
      "fruit": "paramecia",
      "rarity": "SSS",
      "faction": "navy",
      "formation": null,
      "arc": "enies_lobby"
    },
    {
      "id": "BLUE-151",
      "name": "诺兰",
      "sub": " 起义者 ",
      "type": "char",
      "color": "blue",
      "cost": 5,
      "power": 6000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "BLUE-151",
      "fruit": "paramecia",
      "rarity": "A",
      "faction": "revolutionary",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "BLUE-152",
      "name": "斯摩格·大将",
      "sub": " 正义之师 ",
      "type": "char",
      "color": "blue",
      "cost": 1,
      "power": 2000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "BLUE-152",
      "fruit": null,
      "rarity": "A",
      "faction": "navy",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "BLUE-154",
      "name": "蒙奇·D·卡普·大将",
      "sub": " 正义之师 ",
      "type": "char",
      "color": "blue",
      "cost": 1,
      "power": 1000,
      "counter": 1000,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "BLUE-154",
      "fruit": "paramecia",
      "rarity": "B",
      "faction": "navy",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "BLUE-155",
      "name": "哈库·革命军",
      "sub": " 疾风 ",
      "type": "char",
      "color": "blue",
      "cost": 1,
      "power": 2000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "BLUE-155",
      "fruit": null,
      "rarity": "A",
      "faction": "revolutionary",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLUE-157",
      "name": "鬼蜘蛛",
      "sub": " 执法者 ",
      "type": "char",
      "color": "blue",
      "cost": 1,
      "power": 2000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "BLUE-157",
      "fruit": null,
      "rarity": "A",
      "faction": "navy",
      "formation": "vanguard",
      "arc": "enies_lobby"
    },
    {
      "id": "BLUE-158",
      "name": "格拉迪乌斯",
      "sub": " 暗流涌动 ",
      "type": "char",
      "color": "blue",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "buffAll",
          "x": 2000
        }
      },
      "art": "BLUE-158",
      "fruit": null,
      "rarity": "S",
      "faction": "warlord",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "BLUE-159",
      "name": "鹤",
      "sub": " 缉捕令 ",
      "type": "char",
      "color": "blue",
      "cost": 2,
      "power": 2000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "restEnemy"
        }
      },
      "art": "BLUE-159",
      "fruit": null,
      "rarity": "S",
      "faction": "navy",
      "formation": "bulwark",
      "arc": "enies_lobby"
    },
    {
      "id": "BLUE-160",
      "name": "贝洛·贝蒂",
      "sub": " 疾风 ",
      "type": "char",
      "color": "blue",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [
        "doubleAttack"
      ],
      "effect": null,
      "art": "BLUE-160",
      "fruit": null,
      "rarity": "S",
      "faction": "revolutionary",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "BLUE-163",
      "name": "提娜",
      "sub": " 正义之师 ",
      "type": "char",
      "color": "blue",
      "cost": 8,
      "power": 8000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "powerSelf",
          "x": 3000,
          "until": "battle"
        }
      },
      "art": "BLUE-163",
      "fruit": null,
      "rarity": "SSS",
      "faction": "navy",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "BLUE-164",
      "name": "巴斯提雍",
      "sub": " 执法者 ",
      "type": "char",
      "color": "blue",
      "cost": 4,
      "power": 4000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "BLUE-164",
      "fruit": null,
      "rarity": "B",
      "faction": "navy",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "BLUE-167",
      "name": "蒙奇·D·卡普",
      "sub": " 巡逻舰队 ",
      "type": "char",
      "color": "blue",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "draw",
          "n": 2
        }
      },
      "art": "BLUE-167",
      "fruit": null,
      "rarity": "S",
      "faction": "navy",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "BLUE-168",
      "name": "战国·大将",
      "sub": " 正义之师 ",
      "type": "char",
      "color": "blue",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [
        "doubleAttack"
      ],
      "effect": {
        "hook": "onKO",
        "op": {
          "k": "healLP",
          "x": 2000
        }
      },
      "art": "BLUE-168",
      "fruit": null,
      "rarity": "S",
      "faction": "navy",
      "formation": "bulwark",
      "arc": "enies_lobby"
    },
    {
      "id": "BLUE-169",
      "name": "萨卡斯基",
      "sub": " 缉捕令 ",
      "type": "char",
      "color": "blue",
      "cost": 1,
      "power": 1000,
      "counter": 2000,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "BLUE-169",
      "fruit": "paramecia",
      "rarity": "B",
      "faction": "navy",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLUE-171",
      "name": "达斯琪·大将",
      "sub": " 巡逻舰队 ",
      "type": "char",
      "color": "blue",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "blocker",
        "doubleAttack"
      ],
      "effect": null,
      "art": "BLUE-171",
      "fruit": "paramecia",
      "rarity": "SS",
      "faction": "navy",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "BLUE-173",
      "name": "贝鲁梅伯",
      "sub": " 执法者 ",
      "type": "char",
      "color": "blue",
      "cost": 4,
      "power": 5000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "BLUE-173",
      "fruit": null,
      "rarity": "A",
      "faction": "navy",
      "formation": "bulwark",
      "arc": "enies_lobby"
    },
    {
      "id": "GREEN-65",
      "name": "闪电",
      "sub": " 最恶世代 ",
      "type": "char",
      "color": "green",
      "cost": 2,
      "power": 3000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "GREEN-65",
      "fruit": null,
      "rarity": "A",
      "faction": "supernova",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "GREEN-66",
      "name": "卡莉法",
      "sub": " 缉捕令 ",
      "type": "char",
      "color": "green",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "damageLP",
          "x": 2000
        }
      },
      "art": "GREEN-66",
      "fruit": "paramecia",
      "rarity": "S",
      "faction": "navy",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "GREEN-67",
      "name": "贝拉米",
      "sub": " 冒险王 ",
      "type": "char",
      "color": "green",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "GREEN-67",
      "fruit": null,
      "rarity": "S",
      "faction": "strawhat",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "GREEN-68",
      "name": "贝加庞克",
      "sub": " 结盟 ",
      "type": "char",
      "color": "green",
      "cost": 2,
      "power": 2000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onKO",
        "op": {
          "k": "damageLP",
          "x": 1000
        }
      },
      "art": "GREEN-68",
      "fruit": "zoan",
      "rarity": "S",
      "faction": "supernova",
      "formation": null,
      "arc": null
    },
    {
      "id": "GREEN-69",
      "name": "库赞",
      "sub": " 执法者 ",
      "type": "char",
      "color": "green",
      "cost": 4,
      "power": 4000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "GREEN-69",
      "fruit": null,
      "rarity": "B",
      "faction": "navy",
      "formation": "bulwark",
      "arc": "wano"
    },
    {
      "id": "GREEN-70",
      "name": "赞高",
      "sub": " 正义之师 ",
      "type": "char",
      "color": "green",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "blocker",
        "doubleAttack"
      ],
      "effect": {
        "hook": "onKO",
        "op": {
          "k": "damageLP",
          "x": 1000
        }
      },
      "art": "GREEN-70",
      "fruit": null,
      "rarity": "SS",
      "faction": "navy",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "GREEN-71",
      "name": "飞六胞",
      "sub": " 遗志 ",
      "type": "char",
      "color": "green",
      "cost": 1,
      "power": 1000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "GREEN-71",
      "fruit": null,
      "rarity": "B",
      "faction": "whitebeard",
      "formation": null,
      "arc": null
    },
    {
      "id": "GREEN-72",
      "name": "艾恩",
      "sub": " 乱世枭雄 ",
      "type": "char",
      "color": "green",
      "cost": 4,
      "power": 4000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "damageLP",
          "x": 2000
        }
      },
      "art": "GREEN-72",
      "fruit": null,
      "rarity": "S",
      "faction": "supernova",
      "formation": "skirmish",
      "arc": "wano"
    },
    {
      "id": "GREEN-74",
      "name": "纳尔逊",
      "sub": " 巡逻舰队 ",
      "type": "char",
      "color": "green",
      "cost": 4,
      "power": 5000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "GREEN-74",
      "fruit": null,
      "rarity": "A",
      "faction": "navy",
      "formation": "bulwark",
      "arc": "wano"
    },
    {
      "id": "GREEN-75",
      "name": "库赞·大将",
      "sub": " 执法者 ",
      "type": "char",
      "color": "green",
      "cost": 2,
      "power": 2000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "healLP",
          "x": 1000
        }
      },
      "art": "GREEN-75",
      "fruit": null,
      "rarity": "S",
      "faction": "navy",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "GREEN-76",
      "name": "拉库约",
      "sub": " 残火 ",
      "type": "char",
      "color": "green",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "banish"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "gainDon",
          "n": 1
        }
      },
      "art": "GREEN-76",
      "fruit": null,
      "rarity": "S",
      "faction": "whitebeard",
      "formation": "vanguard",
      "arc": "wano"
    },
    {
      "id": "GREEN-77",
      "name": "强纳森",
      "sub": " 正义之师 ",
      "type": "char",
      "color": "green",
      "cost": 5,
      "power": 5000,
      "counter": null,
      "keywords": [
        "banish"
      ],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "draw",
          "n": 1
        }
      },
      "art": "GREEN-77",
      "fruit": null,
      "rarity": "S",
      "faction": "navy",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "GREEN-78",
      "name": "卡库",
      "sub": " 海军将校 ",
      "type": "char",
      "color": "green",
      "cost": 5,
      "power": 6000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "GREEN-78",
      "fruit": null,
      "rarity": "A",
      "faction": "navy",
      "formation": "skirmish",
      "arc": "wano"
    },
    {
      "id": "GREEN-79",
      "name": "鼯鼠",
      "sub": " 巡逻舰队 ",
      "type": "char",
      "color": "green",
      "cost": 3,
      "power": 3000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "GREEN-79",
      "fruit": null,
      "rarity": "B",
      "faction": "navy",
      "formation": "bulwark",
      "arc": "wano"
    },
    {
      "id": "GREEN-80",
      "name": "维尔戈",
      "sub": " 正义之师 ",
      "type": "char",
      "color": "green",
      "cost": 4,
      "power": 4000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "healLP",
          "x": 1000
        }
      },
      "art": "GREEN-80",
      "fruit": null,
      "rarity": "S",
      "faction": "navy",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "GREEN-81",
      "name": "贝波",
      "sub": " 新星 ",
      "type": "char",
      "color": "green",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "damageLP",
          "x": 1000
        }
      },
      "art": "GREEN-81",
      "fruit": null,
      "rarity": "S",
      "faction": "supernova",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "GREEN-82",
      "name": "蒙奇·D·卡普·大将",
      "sub": " 执法者 ",
      "type": "char",
      "color": "green",
      "cost": 3,
      "power": 4000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "GREEN-82",
      "fruit": null,
      "rarity": "A",
      "faction": "navy",
      "formation": "bulwark",
      "arc": "wano"
    },
    {
      "id": "GREEN-83",
      "name": "哈库",
      "sub": " 出头天 ",
      "type": "char",
      "color": "green",
      "cost": 4,
      "power": 4000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "damageLP",
          "x": 1000
        }
      },
      "art": "GREEN-83",
      "fruit": null,
      "rarity": "S",
      "faction": "supernova",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "GREEN-84",
      "name": "布鲁诺",
      "sub": " 执法者 ",
      "type": "char",
      "color": "green",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "gainDon",
          "n": 1
        }
      },
      "art": "GREEN-84",
      "fruit": null,
      "rarity": "S",
      "faction": "navy",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "GREEN-85",
      "name": "一笑",
      "sub": " 巡逻舰队 ",
      "type": "char",
      "color": "green",
      "cost": 2,
      "power": 2000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "GREEN-85",
      "fruit": null,
      "rarity": "B",
      "faction": "navy",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "GREEN-86",
      "name": "火烧山",
      "sub": " 执法者 ",
      "type": "char",
      "color": "green",
      "cost": 8,
      "power": 8000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "search",
          "n": 1,
          "faction": "navy"
        }
      },
      "art": "GREEN-86",
      "fruit": "logia",
      "rarity": "SSS",
      "faction": "navy",
      "formation": "bulwark",
      "arc": "wano"
    },
    {
      "id": "GREEN-88",
      "name": "布罗吉",
      "sub": " 冒险王 ",
      "type": "char",
      "color": "green",
      "cost": 5,
      "power": 5000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "gainDon",
          "n": 1
        }
      },
      "art": "GREEN-88",
      "fruit": "paramecia",
      "rarity": "S",
      "faction": "strawhat",
      "formation": "skirmish",
      "arc": "wano"
    },
    {
      "id": "GREEN-89",
      "name": "亚尔丽塔",
      "sub": " 野心家 ",
      "type": "char",
      "color": "green",
      "cost": 3,
      "power": 3000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "damageLP",
          "x": 2000
        }
      },
      "art": "GREEN-89",
      "fruit": null,
      "rarity": "S",
      "faction": "supernova",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "GREEN-90",
      "name": "瓦尔波",
      "sub": " 乱世枭雄 ",
      "type": "char",
      "color": "green",
      "cost": 3,
      "power": 3000,
      "counter": 2000,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "healLP",
          "x": 2000
        }
      },
      "art": "GREEN-90",
      "fruit": null,
      "rarity": "S",
      "faction": "supernova",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "GREEN-91",
      "name": "达斯琪",
      "sub": " 缉捕令 ",
      "type": "char",
      "color": "green",
      "cost": 1,
      "power": 2000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "GREEN-91",
      "fruit": "paramecia",
      "rarity": "A",
      "faction": "navy",
      "formation": "skirmish",
      "arc": "wano"
    },
    {
      "id": "GREEN-92",
      "name": "妮可·罗宾",
      "sub": " 伙伴之力 ",
      "type": "char",
      "color": "green",
      "cost": 5,
      "power": 6000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "GREEN-92",
      "fruit": null,
      "rarity": "A",
      "faction": "strawhat",
      "formation": "bulwark",
      "arc": "wano"
    },
    {
      "id": "GREEN-93",
      "name": "尤斯塔斯·基德",
      "sub": " 乱世枭雄 ",
      "type": "char",
      "color": "green",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [
        "doubleAttack"
      ],
      "effect": null,
      "art": "GREEN-93",
      "fruit": null,
      "rarity": "S",
      "faction": "supernova",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "GREEN-94",
      "name": "卡拉斯",
      "sub": " 乱世枭雄 ",
      "type": "char",
      "color": "green",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "rush",
        "blocker"
      ],
      "effect": null,
      "art": "GREEN-94",
      "fruit": "zoan",
      "rarity": "SS",
      "faction": "supernova",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "GREEN-95",
      "name": "斯莱曼",
      "sub": " 结盟 ",
      "type": "char",
      "color": "green",
      "cost": 1,
      "power": 2000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "GREEN-95",
      "fruit": "paramecia",
      "rarity": "A",
      "faction": "supernova",
      "formation": "bulwark",
      "arc": "wano"
    },
    {
      "id": "GREEN-96",
      "name": "库洛卡斯",
      "sub": " 自由之海 ",
      "type": "char",
      "color": "green",
      "cost": 4,
      "power": 4000,
      "counter": 1000,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "healLP",
          "x": 1000
        }
      },
      "art": "GREEN-96",
      "fruit": null,
      "rarity": "S",
      "faction": "strawhat",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "GREEN-97",
      "name": "路奇",
      "sub": " 缉捕令 ",
      "type": "char",
      "color": "green",
      "cost": 3,
      "power": 4000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "GREEN-97",
      "fruit": "logia",
      "rarity": "A",
      "faction": "navy",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "GREEN-98",
      "name": "萨波",
      "sub": " 结盟 ",
      "type": "char",
      "color": "green",
      "cost": 1,
      "power": 2000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "GREEN-98",
      "fruit": null,
      "rarity": "A",
      "faction": "supernova",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "GREEN-99",
      "name": "波尔萨利诺",
      "sub": " 海军将校 ",
      "type": "char",
      "color": "green",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "banish"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "draw",
          "n": 1
        }
      },
      "art": "GREEN-99",
      "fruit": null,
      "rarity": "S",
      "faction": "navy",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "GREEN-100",
      "name": "娜美·新世界",
      "sub": " 冒险者 ",
      "type": "char",
      "color": "green",
      "cost": 4,
      "power": 4000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "buffAll",
          "x": 1000
        }
      },
      "art": "GREEN-100",
      "fruit": null,
      "rarity": "S",
      "faction": "strawhat",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "GREEN-101",
      "name": "乌索普·新世界",
      "sub": " 草帽一伙 ",
      "type": "char",
      "color": "green",
      "cost": 4,
      "power": 4000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "GREEN-101",
      "fruit": null,
      "rarity": "B",
      "faction": "strawhat",
      "formation": "bulwark",
      "arc": "wano"
    },
    {
      "id": "GREEN-102",
      "name": "山治·新世界",
      "sub": " 伙伴之力 ",
      "type": "char",
      "color": "green",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "draw",
          "n": 1
        }
      },
      "art": "GREEN-102",
      "fruit": null,
      "rarity": "S",
      "faction": "strawhat",
      "formation": "skirmish",
      "arc": "wano"
    },
    {
      "id": "GREEN-103",
      "name": "东利",
      "sub": " 扬帆起航 ",
      "type": "char",
      "color": "green",
      "cost": 2,
      "power": 3000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "GREEN-103",
      "fruit": null,
      "rarity": "A",
      "faction": "strawhat",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "GREEN-104",
      "name": "尼普顿",
      "sub": " 冒险者 ",
      "type": "char",
      "color": "green",
      "cost": 4,
      "power": 4000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "GREEN-104",
      "fruit": null,
      "rarity": "B",
      "faction": "strawhat",
      "formation": "bulwark",
      "arc": "wano"
    },
    {
      "id": "GREEN-105",
      "name": "X·德雷克",
      "sub": " 野心家 ",
      "type": "char",
      "color": "green",
      "cost": 4,
      "power": 4000,
      "counter": 1000,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "restEnemy"
        }
      },
      "art": "GREEN-105",
      "fruit": null,
      "rarity": "S",
      "faction": "supernova",
      "formation": "bulwark",
      "arc": "wano"
    },
    {
      "id": "GREEN-106",
      "name": "蒙奇·D·路飞·新世界",
      "sub": " 自由之海 ",
      "type": "char",
      "color": "green",
      "cost": 1,
      "power": 1000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "GREEN-106",
      "fruit": null,
      "rarity": "B",
      "faction": "strawhat",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "GREEN-107",
      "name": "阿龙",
      "sub": " 最恶世代 ",
      "type": "char",
      "color": "green",
      "cost": 3,
      "power": 3000,
      "counter": 2000,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "draw",
          "n": 2
        }
      },
      "art": "GREEN-107",
      "fruit": null,
      "rarity": "S",
      "faction": "supernova",
      "formation": "skirmish",
      "arc": "wano"
    },
    {
      "id": "GREEN-108",
      "name": "阿托摩斯",
      "sub": " 遗志 ",
      "type": "char",
      "color": "green",
      "cost": 4,
      "power": 4000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "GREEN-108",
      "fruit": null,
      "rarity": "B",
      "faction": "whitebeard",
      "formation": "bulwark",
      "arc": "wano"
    },
    {
      "id": "GREEN-109",
      "name": "T·彭",
      "sub": " 巡逻舰队 ",
      "type": "char",
      "color": "green",
      "cost": 2,
      "power": 3000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "GREEN-109",
      "fruit": null,
      "rarity": "A",
      "faction": "navy",
      "formation": "bulwark",
      "arc": "wano"
    },
    {
      "id": "GREEN-110",
      "name": "斯摩格",
      "sub": " 海军将校 ",
      "type": "char",
      "color": "green",
      "cost": 3,
      "power": 3000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "restEnemy"
        }
      },
      "art": "GREEN-110",
      "fruit": "paramecia",
      "rarity": "S",
      "faction": "navy",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "GREEN-111",
      "name": "乔艾莉·波妮",
      "sub": " 结盟 ",
      "type": "char",
      "color": "green",
      "cost": 5,
      "power": 5000,
      "counter": 2000,
      "keywords": [
        "banish"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "powerSelf",
          "x": 3000,
          "until": "battle"
        }
      },
      "art": "GREEN-111",
      "fruit": null,
      "rarity": "S",
      "faction": "supernova",
      "formation": "bulwark",
      "arc": "wano"
    },
    {
      "id": "GREEN-112",
      "name": "鬼蜘蛛",
      "sub": " 正义之师 ",
      "type": "char",
      "color": "green",
      "cost": 2,
      "power": 2000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "GREEN-112",
      "fruit": "logia",
      "rarity": "B",
      "faction": "navy",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "GREEN-113",
      "name": "林德伯格",
      "sub": " 新星 ",
      "type": "char",
      "color": "green",
      "cost": 3,
      "power": 4000,
      "counter": 2000,
      "keywords": [],
      "effect": null,
      "art": "GREEN-113",
      "fruit": null,
      "rarity": "A",
      "faction": "supernova",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "GREEN-114",
      "name": "弗兰奇",
      "sub": " 扬帆起航 ",
      "type": "char",
      "color": "green",
      "cost": 2,
      "power": 2000,
      "counter": 1000,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "GREEN-114",
      "fruit": null,
      "rarity": "B",
      "faction": "strawhat",
      "formation": null,
      "arc": "wano"
    },
    {
      "id": "GREEN-115",
      "name": "冰山",
      "sub": " 自由之海 ",
      "type": "char",
      "color": "green",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "doubleAttack"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "healLP",
          "x": 2000
        }
      },
      "art": "GREEN-115",
      "fruit": null,
      "rarity": "S",
      "faction": "strawhat",
      "formation": "bulwark",
      "arc": "wano"
    },
    {
      "id": "GREEN-116",
      "name": "贝鲁梅特尔",
      "sub": " 海军将校 ",
      "type": "char",
      "color": "green",
      "cost": 4,
      "power": 5000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "GREEN-116",
      "fruit": null,
      "rarity": "A",
      "faction": "navy",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "GREEN-117",
      "name": "罗罗诺亚·索隆·新世界",
      "sub": " 扬帆起航 ",
      "type": "char",
      "color": "green",
      "cost": 3,
      "power": 3000,
      "counter": 1000,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "gainDon",
          "n": 2
        }
      },
      "art": "GREEN-117",
      "fruit": null,
      "rarity": "S",
      "faction": "strawhat",
      "formation": "skirmish",
      "arc": "wano"
    },
    {
      "id": "GREEN-118",
      "name": "蒙奇·D·卡普",
      "sub": " 缉捕令 ",
      "type": "char",
      "color": "green",
      "cost": 3,
      "power": 3000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onKO",
        "op": {
          "k": "healLP",
          "x": 1000
        }
      },
      "art": "GREEN-118",
      "fruit": null,
      "rarity": "S",
      "faction": "navy",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "GREEN-119",
      "name": "克洛",
      "sub": " 最恶世代 ",
      "type": "char",
      "color": "green",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [
        "banish",
        "banish"
      ],
      "effect": {
        "hook": "onKO",
        "op": {
          "k": "draw",
          "n": 1
        }
      },
      "art": "GREEN-119",
      "fruit": null,
      "rarity": "SS",
      "faction": "supernova",
      "formation": "skirmish",
      "arc": "wano"
    },
    {
      "id": "GREEN-121",
      "name": "夏莉",
      "sub": " 自由之海 ",
      "type": "char",
      "color": "green",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "banish"
      ],
      "effect": null,
      "art": "GREEN-121",
      "fruit": "paramecia",
      "rarity": "B",
      "faction": "strawhat",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "GREEN-122",
      "name": "白星",
      "sub": " 冒险王 ",
      "type": "char",
      "color": "green",
      "cost": 1,
      "power": 1000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "GREEN-122",
      "fruit": null,
      "rarity": "B",
      "faction": "strawhat",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "GREEN-123",
      "name": "萨奇",
      "sub": " 遗志 ",
      "type": "char",
      "color": "green",
      "cost": 2,
      "power": 2000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onKO",
        "op": {
          "k": "restEnemy",
          "target": "strongest"
        }
      },
      "art": "GREEN-123",
      "fruit": null,
      "rarity": "S",
      "faction": "whitebeard",
      "formation": "skirmish",
      "arc": "wano"
    },
    {
      "id": "GREEN-126",
      "name": "娜美",
      "sub": " 草帽一伙 ",
      "type": "char",
      "color": "green",
      "cost": 2,
      "power": 2000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "GREEN-126",
      "fruit": "paramecia",
      "rarity": "B",
      "faction": "strawhat",
      "formation": "skirmish",
      "arc": "wano"
    },
    {
      "id": "GREEN-128",
      "name": "甚平",
      "sub": " 草帽一伙 ",
      "type": "char",
      "color": "green",
      "cost": 4,
      "power": 4000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "GREEN-128",
      "fruit": null,
      "rarity": "B",
      "faction": "strawhat",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "GREEN-129",
      "name": "可可罗",
      "sub": " 伙伴之力 ",
      "type": "char",
      "color": "green",
      "cost": 3,
      "power": 4000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "GREEN-129",
      "fruit": null,
      "rarity": "A",
      "faction": "strawhat",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "GREEN-130",
      "name": "鹤",
      "sub": " 巡逻舰队 ",
      "type": "char",
      "color": "green",
      "cost": 8,
      "power": 8000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "search",
          "n": 1,
          "faction": "navy"
        }
      },
      "art": "GREEN-130",
      "fruit": null,
      "rarity": "SSS",
      "faction": "navy",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "GREEN-132",
      "name": "弗兰奇·新世界",
      "sub": " 伙伴之力 ",
      "type": "char",
      "color": "green",
      "cost": 4,
      "power": 5000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "GREEN-132",
      "fruit": null,
      "rarity": "A",
      "faction": "strawhat",
      "formation": null,
      "arc": null
    },
    {
      "id": "GREEN-133",
      "name": "一笑·大将",
      "sub": " 正义之师 ",
      "type": "char",
      "color": "green",
      "cost": 3,
      "power": 3000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "GREEN-133",
      "fruit": null,
      "rarity": "B",
      "faction": "navy",
      "formation": null,
      "arc": null
    },
    {
      "id": "GREEN-134",
      "name": "贝鲁梅伯",
      "sub": " 海军将校 ",
      "type": "char",
      "color": "green",
      "cost": 2,
      "power": 2000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "GREEN-134",
      "fruit": null,
      "rarity": "B",
      "faction": "navy",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "GREEN-137",
      "name": "提娜",
      "sub": " 海军将校 ",
      "type": "char",
      "color": "green",
      "cost": 4,
      "power": 4000,
      "counter": 1000,
      "keywords": [
        "blocker",
        "blocker"
      ],
      "effect": null,
      "art": "GREEN-137",
      "fruit": "paramecia",
      "rarity": "SS",
      "faction": "navy",
      "formation": "bulwark",
      "arc": "wano"
    },
    {
      "id": "GREEN-139",
      "name": "孔美奥",
      "sub": " 巡逻舰队 ",
      "type": "char",
      "color": "green",
      "cost": 5,
      "power": 5000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "GREEN-139",
      "fruit": null,
      "rarity": "B",
      "faction": "navy",
      "formation": "bulwark",
      "arc": "wano"
    },
    {
      "id": "GREEN-140",
      "name": "诺琪高",
      "sub": " 草帽一伙 ",
      "type": "char",
      "color": "green",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [
        "doubleAttack"
      ],
      "effect": null,
      "art": "GREEN-140",
      "fruit": "zoan",
      "rarity": "S",
      "faction": "strawhat",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "GREEN-141",
      "name": "凯撒·克朗",
      "sub": " 最恶世代 ",
      "type": "char",
      "color": "green",
      "cost": 3,
      "power": 4000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "GREEN-141",
      "fruit": null,
      "rarity": "A",
      "faction": "supernova",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "GREEN-142",
      "name": "犬岚公爵",
      "sub": " 家族之绊 ",
      "type": "char",
      "color": "green",
      "cost": 5,
      "power": 5000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "GREEN-142",
      "fruit": null,
      "rarity": "B",
      "faction": "whitebeard",
      "formation": "skirmish",
      "arc": "wano"
    },
    {
      "id": "GREEN-143",
      "name": "蒙奇·D·路飞",
      "sub": " 草帽一伙 ",
      "type": "char",
      "color": "green",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "search",
          "n": 1,
          "faction": "strawhat"
        }
      },
      "art": "GREEN-143",
      "fruit": "logia",
      "rarity": "S",
      "faction": "strawhat",
      "formation": "bulwark",
      "arc": "wano"
    },
    {
      "id": "GREEN-144",
      "name": "萨卡斯基·大将",
      "sub": " 缉捕令 ",
      "type": "char",
      "color": "green",
      "cost": 8,
      "power": 8000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "GREEN-144",
      "fruit": null,
      "rarity": "SS",
      "faction": "navy",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "GREEN-145",
      "name": "巴斯提雍",
      "sub": " 海军将校 ",
      "type": "char",
      "color": "green",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [
        "rush",
        "blocker"
      ],
      "effect": null,
      "art": "GREEN-145",
      "fruit": null,
      "rarity": "SS",
      "faction": "navy",
      "formation": "bulwark",
      "arc": "wano"
    },
    {
      "id": "GREEN-146",
      "name": "缇娜",
      "sub": " 正义之师 ",
      "type": "char",
      "color": "green",
      "cost": 3,
      "power": 3000,
      "counter": 1000,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "debuffFoeAll",
          "x": 2000
        }
      },
      "art": "GREEN-146",
      "fruit": null,
      "rarity": "S",
      "faction": "navy",
      "formation": "vanguard",
      "arc": "wano"
    },
    {
      "id": "GREEN-148",
      "name": "阿拉马基·大将",
      "sub": " 海军将校 ",
      "type": "char",
      "color": "green",
      "cost": 2,
      "power": 2000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "GREEN-148",
      "fruit": null,
      "rarity": "B",
      "faction": "navy",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "GREEN-149",
      "name": "波尔萨利诺·大将",
      "sub": " 执法者 ",
      "type": "char",
      "color": "green",
      "cost": 1,
      "power": 2000,
      "counter": 2000,
      "keywords": [],
      "effect": null,
      "art": "GREEN-149",
      "fruit": "paramecia",
      "rarity": "A",
      "faction": "navy",
      "formation": null,
      "arc": null
    },
    {
      "id": "GREEN-150",
      "name": "和道一文字",
      "sub": " 草帽一伙 ",
      "type": "char",
      "color": "green",
      "cost": 3,
      "power": 3000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "GREEN-150",
      "fruit": "paramecia",
      "rarity": "B",
      "faction": "strawhat",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "GREEN-151",
      "name": "加布拉",
      "sub": " 巡逻舰队 ",
      "type": "char",
      "color": "green",
      "cost": 8,
      "power": 8000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "GREEN-151",
      "fruit": "logia",
      "rarity": "SS",
      "faction": "navy",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "GREEN-152",
      "name": "基拉",
      "sub": " 出头天 ",
      "type": "char",
      "color": "green",
      "cost": 4,
      "power": 4000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "GREEN-152",
      "fruit": null,
      "rarity": "B",
      "faction": "supernova",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "GREEN-153",
      "name": "阿拉马基",
      "sub": " 巡逻舰队 ",
      "type": "char",
      "color": "green",
      "cost": 4,
      "power": 4000,
      "counter": 1000,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "powerSelf",
          "x": 3000
        }
      },
      "art": "GREEN-153",
      "fruit": null,
      "rarity": "S",
      "faction": "navy",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "GREEN-154",
      "name": "托尼托尼·乔巴·新世界",
      "sub": " 草帽一伙 ",
      "type": "char",
      "color": "green",
      "cost": 6,
      "power": 7000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "GREEN-154",
      "fruit": null,
      "rarity": "A",
      "faction": "strawhat",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "GREEN-155",
      "name": "布拉曼克",
      "sub": " 白胡子一族 ",
      "type": "char",
      "color": "green",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onKO",
        "op": {
          "k": "buffAll",
          "x": 2000
        }
      },
      "art": "GREEN-155",
      "fruit": "zoan",
      "rarity": "S",
      "faction": "whitebeard",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "GREEN-156",
      "name": "妮可·罗宾·新世界",
      "sub": " 冒险者 ",
      "type": "char",
      "color": "green",
      "cost": 4,
      "power": 5000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "GREEN-156",
      "fruit": null,
      "rarity": "A",
      "faction": "strawhat",
      "formation": "bulwark",
      "arc": "wano"
    },
    {
      "id": "GREEN-157",
      "name": "薇薇",
      "sub": " 自由之海 ",
      "type": "char",
      "color": "green",
      "cost": 2,
      "power": 3000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "GREEN-157",
      "fruit": null,
      "rarity": "A",
      "faction": "strawhat",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "GREEN-159",
      "name": "阿健",
      "sub": " 草帽一伙 ",
      "type": "char",
      "color": "green",
      "cost": 4,
      "power": 5000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "GREEN-159",
      "fruit": null,
      "rarity": "A",
      "faction": "strawhat",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "GREEN-161",
      "name": "乔兹",
      "sub": " 白胡子一族 ",
      "type": "char",
      "color": "green",
      "cost": 8,
      "power": 8000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "draw",
          "n": 2
        }
      },
      "art": "GREEN-161",
      "fruit": "paramecia",
      "rarity": "SSS",
      "faction": "whitebeard",
      "formation": "bulwark",
      "arc": "wano"
    },
    {
      "id": "GREEN-162",
      "name": "卡鲁",
      "sub": " 自由之海 ",
      "type": "char",
      "color": "green",
      "cost": 7,
      "power": 8000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "GREEN-162",
      "fruit": null,
      "rarity": "S",
      "faction": "strawhat",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "GREEN-163",
      "name": "拉布",
      "sub": " 扬帆起航 ",
      "type": "char",
      "color": "green",
      "cost": 5,
      "power": 5000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "draw",
          "n": 1
        }
      },
      "art": "GREEN-163",
      "fruit": null,
      "rarity": "S",
      "faction": "strawhat",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "GREEN-164",
      "name": "布鲁克",
      "sub": " 草帽一伙 ",
      "type": "char",
      "color": "green",
      "cost": 2,
      "power": 2000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "restEnemy"
        }
      },
      "art": "GREEN-164",
      "fruit": "zoan",
      "rarity": "S",
      "faction": "strawhat",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "GREEN-165",
      "name": "顿·克利克",
      "sub": " 结盟 ",
      "type": "char",
      "color": "green",
      "cost": 4,
      "power": 4000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "GREEN-165",
      "fruit": null,
      "rarity": "B",
      "faction": "supernova",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "GREEN-166",
      "name": "布鲁克·新世界",
      "sub": " 草帽一伙 ",
      "type": "char",
      "color": "green",
      "cost": 2,
      "power": 3000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "GREEN-166",
      "fruit": null,
      "rarity": "A",
      "faction": "strawhat",
      "formation": null,
      "arc": null
    },
    {
      "id": "GREEN-167",
      "name": "乙姬",
      "sub": " 冒险者 ",
      "type": "char",
      "color": "green",
      "cost": 3,
      "power": 3000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onKO",
        "op": {
          "k": "draw",
          "n": 1
        }
      },
      "art": "GREEN-167",
      "fruit": null,
      "rarity": "S",
      "faction": "strawhat",
      "formation": "bulwark",
      "arc": "wano"
    },
    {
      "id": "GREEN-168",
      "name": "山治",
      "sub": " 冒险王 ",
      "type": "char",
      "color": "green",
      "cost": 2,
      "power": 2000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "GREEN-168",
      "fruit": null,
      "rarity": "B",
      "faction": "strawhat",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "GREEN-169",
      "name": "雪走",
      "sub": " 自由之海 ",
      "type": "char",
      "color": "green",
      "cost": 2,
      "power": 2000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "healLP",
          "x": 2000
        }
      },
      "art": "GREEN-169",
      "fruit": null,
      "rarity": "S",
      "faction": "strawhat",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "GREEN-170",
      "name": "帕帕古",
      "sub": " 草帽一伙 ",
      "type": "char",
      "color": "green",
      "cost": 2,
      "power": 3000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "GREEN-170",
      "fruit": null,
      "rarity": "A",
      "faction": "strawhat",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "GREEN-171",
      "name": "斯潘达姆",
      "sub": " 正义之师 ",
      "type": "char",
      "color": "green",
      "cost": 3,
      "power": 4000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "GREEN-171",
      "fruit": null,
      "rarity": "A",
      "faction": "navy",
      "formation": null,
      "arc": "wano"
    },
    {
      "id": "GREEN-172",
      "name": "波特卡斯·D·艾斯",
      "sub": " 遗志 ",
      "type": "char",
      "color": "green",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "GREEN-172",
      "fruit": "paramecia",
      "rarity": "S",
      "faction": "whitebeard",
      "formation": "skirmish",
      "arc": "wano"
    },
    {
      "id": "GREEN-173",
      "name": "克比",
      "sub": " 海军将校 ",
      "type": "char",
      "color": "green",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "draw",
          "n": 1
        }
      },
      "art": "GREEN-173",
      "fruit": null,
      "rarity": "S",
      "faction": "navy",
      "formation": "bulwark",
      "arc": "wano"
    },
    {
      "id": "GREEN-174",
      "name": "萨卡斯基",
      "sub": " 正义之师 ",
      "type": "char",
      "color": "green",
      "cost": 1,
      "power": 2000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "GREEN-174",
      "fruit": null,
      "rarity": "A",
      "faction": "navy",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "GREEN-175",
      "name": "战国",
      "sub": " 缉捕令 ",
      "type": "char",
      "color": "green",
      "cost": 5,
      "power": 5000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "GREEN-175",
      "fruit": null,
      "rarity": "B",
      "faction": "navy",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "GREEN-177",
      "name": "奇蒙尼",
      "sub": " 草帽一伙 ",
      "type": "char",
      "color": "green",
      "cost": 4,
      "power": 4000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "GREEN-177",
      "fruit": null,
      "rarity": "B",
      "faction": "strawhat",
      "formation": null,
      "arc": null
    },
    {
      "id": "YELLOW-58",
      "name": "卡库",
      "sub": " 铁律 ",
      "type": "char",
      "color": "yellow",
      "cost": 5,
      "power": 5000,
      "counter": 1000,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onKO",
        "op": {
          "k": "damageLP",
          "x": 2000
        }
      },
      "art": "YELLOW-58",
      "fruit": null,
      "rarity": "S",
      "faction": "navy",
      "formation": null,
      "arc": null
    },
    {
      "id": "YELLOW-59",
      "name": "哥尔·D·罗杰·四皇",
      "sub": " 四皇麾下 ",
      "type": "char",
      "color": "yellow",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "damageLP",
          "x": 1000
        }
      },
      "art": "YELLOW-59",
      "fruit": null,
      "rarity": "S",
      "faction": "yonko",
      "formation": null,
      "arc": null
    },
    {
      "id": "YELLOW-60",
      "name": "阿曼德",
      "sub": " 四皇麾下 ",
      "type": "char",
      "color": "yellow",
      "cost": 2,
      "power": 3000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "YELLOW-60",
      "fruit": null,
      "rarity": "A",
      "faction": "yonko",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "YELLOW-61",
      "name": "提娜",
      "sub": " 铁律 ",
      "type": "char",
      "color": "yellow",
      "cost": 8,
      "power": 8000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "YELLOW-61",
      "fruit": null,
      "rarity": "SS",
      "faction": "navy",
      "formation": "vanguard",
      "arc": "whole_cake"
    },
    {
      "id": "YELLOW-62",
      "name": "Mr.2",
      "sub": " 傀儡师 ",
      "type": "char",
      "color": "yellow",
      "cost": 4,
      "power": 4000,
      "counter": null,
      "keywords": [
        "blocker",
        "blocker"
      ],
      "effect": {
        "hook": "onKO",
        "op": {
          "k": "damageLP",
          "x": 2000
        }
      },
      "art": "YELLOW-62",
      "fruit": null,
      "rarity": "SS",
      "faction": "warlord",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "YELLOW-63",
      "name": "月光·莫利亚·王下",
      "sub": " 处刑人 ",
      "type": "char",
      "color": "yellow",
      "cost": 2,
      "power": 2000,
      "counter": 2000,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "YELLOW-63",
      "fruit": null,
      "rarity": "B",
      "faction": "warlord",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "YELLOW-64",
      "name": "巴索罗缪·熊·王下",
      "sub": " 孤高之刃 ",
      "type": "char",
      "color": "yellow",
      "cost": 4,
      "power": 4000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "YELLOW-64",
      "fruit": null,
      "rarity": "B",
      "faction": "warlord",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "YELLOW-65",
      "name": "加列特",
      "sub": " 皇族血脉 ",
      "type": "char",
      "color": "yellow",
      "cost": 6,
      "power": 7000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "YELLOW-65",
      "fruit": null,
      "rarity": "A",
      "faction": "yonko",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "YELLOW-66",
      "name": "坎迪",
      "sub": " 四皇麾下 ",
      "type": "char",
      "color": "yellow",
      "cost": 7,
      "power": 8000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "YELLOW-66",
      "fruit": null,
      "rarity": "S",
      "faction": "yonko",
      "formation": "skirmish",
      "arc": "whole_cake"
    },
    {
      "id": "YELLOW-67",
      "name": "达斯琪",
      "sub": " 正义之师 ",
      "type": "char",
      "color": "yellow",
      "cost": 4,
      "power": 4000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "damageLP",
          "x": 2000
        }
      },
      "art": "YELLOW-67",
      "fruit": null,
      "rarity": "S",
      "faction": "navy",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "YELLOW-68",
      "name": "巴基·王下",
      "sub": " 孤高之刃 ",
      "type": "char",
      "color": "yellow",
      "cost": 4,
      "power": 4000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "debuffFoeAll",
          "x": 2000
        }
      },
      "art": "YELLOW-68",
      "fruit": "logia",
      "rarity": "S",
      "faction": "warlord",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "YELLOW-69",
      "name": "蒙奇·D·卡普·大将",
      "sub": " 缉捕令 ",
      "type": "char",
      "color": "yellow",
      "cost": 3,
      "power": 3000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "YELLOW-69",
      "fruit": "paramecia",
      "rarity": "B",
      "faction": "navy",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "YELLOW-70",
      "name": "洛克之星",
      "sub": " 四皇麾下 ",
      "type": "char",
      "color": "yellow",
      "cost": 1,
      "power": 1000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "healLP",
          "x": 1000
        }
      },
      "art": "YELLOW-70",
      "fruit": null,
      "rarity": "S",
      "faction": "yonko",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "YELLOW-71",
      "name": "洛克之星·四皇",
      "sub": " 亲卫队长 ",
      "type": "char",
      "color": "yellow",
      "cost": 4,
      "power": 4000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "restEnemy",
          "target": "strongest"
        }
      },
      "art": "YELLOW-71",
      "fruit": null,
      "rarity": "S",
      "faction": "yonko",
      "formation": "skirmish",
      "arc": "whole_cake"
    },
    {
      "id": "YELLOW-72",
      "name": "战国",
      "sub": " 执法者 ",
      "type": "char",
      "color": "yellow",
      "cost": 5,
      "power": 5000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "gainDon",
          "n": 1
        }
      },
      "art": "YELLOW-72",
      "fruit": null,
      "rarity": "S",
      "faction": "navy",
      "formation": "skirmish",
      "arc": "whole_cake"
    },
    {
      "id": "YELLOW-73",
      "name": "Mr.3",
      "sub": " 夜行 ",
      "type": "char",
      "color": "yellow",
      "cost": 4,
      "power": 4000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "YELLOW-73",
      "fruit": "paramecia",
      "rarity": "B",
      "faction": "warlord",
      "formation": "skirmish",
      "arc": "whole_cake"
    },
    {
      "id": "YELLOW-74",
      "name": "扎拉逢",
      "sub": " 四皇麾下 ",
      "type": "char",
      "color": "yellow",
      "cost": 3,
      "power": 3000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "draw",
          "n": 1
        }
      },
      "art": "YELLOW-74",
      "fruit": "zoan",
      "rarity": "S",
      "faction": "yonko",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "YELLOW-75",
      "name": "洛克斯·D·吉贝克",
      "sub": " 蛋糕城 ",
      "type": "char",
      "color": "yellow",
      "cost": 8,
      "power": 8000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "buffAll",
          "x": 1000
        }
      },
      "art": "YELLOW-75",
      "fruit": null,
      "rarity": "SSS",
      "faction": "yonko",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "YELLOW-76",
      "name": "本·贝克曼·四皇",
      "sub": " 皇族血脉 ",
      "type": "char",
      "color": "yellow",
      "cost": 8,
      "power": 8000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "gainDon",
          "n": 1
        }
      },
      "art": "YELLOW-76",
      "fruit": "paramecia",
      "rarity": "SSS",
      "faction": "yonko",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "YELLOW-77",
      "name": "巴雷特",
      "sub": " 大海之主 ",
      "type": "char",
      "color": "yellow",
      "cost": 5,
      "power": 5000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "healLP",
          "x": 1000
        }
      },
      "art": "YELLOW-77",
      "fruit": null,
      "rarity": "S",
      "faction": "yonko",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "YELLOW-78",
      "name": "海楼石",
      "sub": " 皇族血脉 ",
      "type": "char",
      "color": "yellow",
      "cost": 2,
      "power": 2000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "draw",
          "n": 1
        }
      },
      "art": "YELLOW-78",
      "fruit": null,
      "rarity": "S",
      "faction": "yonko",
      "formation": null,
      "arc": null
    },
    {
      "id": "YELLOW-79",
      "name": "阿拉马基·大将",
      "sub": " 铁律 ",
      "type": "char",
      "color": "yellow",
      "cost": 8,
      "power": 8000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onKO",
        "op": {
          "k": "draw",
          "n": 2
        }
      },
      "art": "YELLOW-79",
      "fruit": null,
      "rarity": "SSS",
      "faction": "navy",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "YELLOW-80",
      "name": "斯潘达姆",
      "sub": " 执法者 ",
      "type": "char",
      "color": "yellow",
      "cost": 6,
      "power": 7000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "YELLOW-80",
      "fruit": null,
      "rarity": "A",
      "faction": "navy",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "YELLOW-81",
      "name": "波尔萨利诺·大将",
      "sub": " 铁律 ",
      "type": "char",
      "color": "yellow",
      "cost": 6,
      "power": 7000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "YELLOW-81",
      "fruit": null,
      "rarity": "A",
      "faction": "navy",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "YELLOW-82",
      "name": "万国",
      "sub": " 破竹之势 ",
      "type": "char",
      "color": "yellow",
      "cost": 1,
      "power": 2000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "YELLOW-82",
      "fruit": null,
      "rarity": "A",
      "faction": "yonko",
      "formation": "skirmish",
      "arc": "whole_cake"
    },
    {
      "id": "YELLOW-84",
      "name": "蒙奇·D·卡普",
      "sub": " 正义之师 ",
      "type": "char",
      "color": "yellow",
      "cost": 1,
      "power": 2000,
      "counter": 2000,
      "keywords": [],
      "effect": null,
      "art": "YELLOW-84",
      "fruit": null,
      "rarity": "A",
      "faction": "navy",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "YELLOW-85",
      "name": "维奥莱特",
      "sub": " 处刑人 ",
      "type": "char",
      "color": "yellow",
      "cost": 3,
      "power": 3000,
      "counter": 1000,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "YELLOW-85",
      "fruit": null,
      "rarity": "B",
      "faction": "warlord",
      "formation": null,
      "arc": null
    },
    {
      "id": "YELLOW-86",
      "name": "爱德华·威布尔·王下",
      "sub": " 夜行 ",
      "type": "char",
      "color": "yellow",
      "cost": 5,
      "power": 5000,
      "counter": 1000,
      "keywords": [
        "rush",
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "draw",
          "n": 2
        }
      },
      "art": "YELLOW-86",
      "fruit": null,
      "rarity": "SS",
      "faction": "warlord",
      "formation": "vanguard",
      "arc": "whole_cake"
    },
    {
      "id": "YELLOW-87",
      "name": "爱德华·威布尔",
      "sub": " 夜行 ",
      "type": "char",
      "color": "yellow",
      "cost": 1,
      "power": 1000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onKO",
        "op": {
          "k": "damageLP",
          "x": 2000
        }
      },
      "art": "YELLOW-87",
      "fruit": "zoan",
      "rarity": "S",
      "faction": "warlord",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "YELLOW-89",
      "name": "强纳森",
      "sub": " 正义之师 ",
      "type": "char",
      "color": "yellow",
      "cost": 3,
      "power": 3000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "draw",
          "n": 1
        }
      },
      "art": "YELLOW-89",
      "fruit": null,
      "rarity": "S",
      "faction": "navy",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "YELLOW-91",
      "name": "库赞·大将",
      "sub": " 海军将校 ",
      "type": "char",
      "color": "yellow",
      "cost": 3,
      "power": 3000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "healLP",
          "x": 2000
        }
      },
      "art": "YELLOW-91",
      "fruit": null,
      "rarity": "S",
      "faction": "navy",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "YELLOW-92",
      "name": "唐吉诃德·多弗朗明戈",
      "sub": " 孤高之刃 ",
      "type": "char",
      "color": "yellow",
      "cost": 4,
      "power": 4000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "YELLOW-92",
      "fruit": null,
      "rarity": "B",
      "faction": "warlord",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "YELLOW-93",
      "name": "赞高",
      "sub": " 缉捕令 ",
      "type": "char",
      "color": "yellow",
      "cost": 1,
      "power": 2000,
      "counter": 2000,
      "keywords": [],
      "effect": null,
      "art": "YELLOW-93",
      "fruit": null,
      "rarity": "A",
      "faction": "navy",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "YELLOW-94",
      "name": "斯摩格·大将",
      "sub": " 执法者 ",
      "type": "char",
      "color": "yellow",
      "cost": 2,
      "power": 2000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "gainDon",
          "n": 2
        }
      },
      "art": "YELLOW-94",
      "fruit": null,
      "rarity": "S",
      "faction": "navy",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "YELLOW-95",
      "name": "鼯鼠",
      "sub": " 执法者 ",
      "type": "char",
      "color": "yellow",
      "cost": 6,
      "power": 7000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "YELLOW-95",
      "fruit": "paramecia",
      "rarity": "A",
      "faction": "navy",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "YELLOW-96",
      "name": "克比",
      "sub": " 正义之师 ",
      "type": "char",
      "color": "yellow",
      "cost": 3,
      "power": 3000,
      "counter": 1000,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "YELLOW-96",
      "fruit": null,
      "rarity": "B",
      "faction": "navy",
      "formation": "skirmish",
      "arc": "whole_cake"
    },
    {
      "id": "YELLOW-97",
      "name": "沙尔·克洛克达尔·王下",
      "sub": " 孤高之刃 ",
      "type": "char",
      "color": "yellow",
      "cost": 5,
      "power": 5000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "YELLOW-97",
      "fruit": null,
      "rarity": "B",
      "faction": "warlord",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "YELLOW-98",
      "name": "乔拉",
      "sub": " 夜行 ",
      "type": "char",
      "color": "yellow",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "YELLOW-98",
      "fruit": null,
      "rarity": "B",
      "faction": "warlord",
      "formation": "bulwark",
      "arc": "whole_cake"
    },
    {
      "id": "YELLOW-99",
      "name": "鹤·大将",
      "sub": " 正义之师 ",
      "type": "char",
      "color": "yellow",
      "cost": 3,
      "power": 4000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "YELLOW-99",
      "fruit": null,
      "rarity": "A",
      "faction": "navy",
      "formation": "bulwark",
      "arc": "whole_cake"
    },
    {
      "id": "YELLOW-100",
      "name": "巴基",
      "sub": " 夜行 ",
      "type": "char",
      "color": "yellow",
      "cost": 4,
      "power": 5000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "YELLOW-100",
      "fruit": null,
      "rarity": "A",
      "faction": "warlord",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "YELLOW-101",
      "name": "席尔巴斯·雷利",
      "sub": " 亲卫队长 ",
      "type": "char",
      "color": "yellow",
      "cost": 4,
      "power": 4000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "YELLOW-101",
      "fruit": null,
      "rarity": "B",
      "faction": "yonko",
      "formation": "skirmish",
      "arc": "whole_cake"
    },
    {
      "id": "YELLOW-102",
      "name": "斯纳克",
      "sub": " 亲卫队长 ",
      "type": "char",
      "color": "yellow",
      "cost": 2,
      "power": 2000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "YELLOW-102",
      "fruit": null,
      "rarity": "B",
      "faction": "yonko",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "YELLOW-103",
      "name": "卡莉法",
      "sub": " 巡逻舰队 ",
      "type": "char",
      "color": "yellow",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "YELLOW-103",
      "fruit": null,
      "rarity": "B",
      "faction": "navy",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "YELLOW-104",
      "name": "夏奇·四皇",
      "sub": " 亲卫队长 ",
      "type": "char",
      "color": "yellow",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "YELLOW-104",
      "fruit": null,
      "rarity": "B",
      "faction": "yonko",
      "formation": "skirmish",
      "arc": "whole_cake"
    },
    {
      "id": "YELLOW-105",
      "name": "萨卡斯基",
      "sub": " 执法者 ",
      "type": "char",
      "color": "yellow",
      "cost": 3,
      "power": 3000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "YELLOW-105",
      "fruit": null,
      "rarity": "B",
      "faction": "navy",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "YELLOW-106",
      "name": "文斯莫克·勇治",
      "sub": " 四皇麾下 ",
      "type": "char",
      "color": "yellow",
      "cost": 1,
      "power": 2000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "YELLOW-106",
      "fruit": null,
      "rarity": "A",
      "faction": "yonko",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "YELLOW-107",
      "name": "一笑·大将",
      "sub": " 正义之师 ",
      "type": "char",
      "color": "yellow",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "YELLOW-107",
      "fruit": null,
      "rarity": "B",
      "faction": "navy",
      "formation": "skirmish",
      "arc": "whole_cake"
    },
    {
      "id": "YELLOW-108",
      "name": "路奇",
      "sub": " 巡逻舰队 ",
      "type": "char",
      "color": "yellow",
      "cost": 3,
      "power": 4000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "YELLOW-108",
      "fruit": null,
      "rarity": "A",
      "faction": "navy",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "YELLOW-109",
      "name": "纳尔逊",
      "sub": " 铁律 ",
      "type": "char",
      "color": "yellow",
      "cost": 2,
      "power": 2000,
      "counter": 1000,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "YELLOW-109",
      "fruit": null,
      "rarity": "B",
      "faction": "navy",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "YELLOW-110",
      "name": "火烧山",
      "sub": " 铁律 ",
      "type": "char",
      "color": "yellow",
      "cost": 1,
      "power": 1000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "YELLOW-110",
      "fruit": "zoan",
      "rarity": "B",
      "faction": "navy",
      "formation": "bulwark",
      "arc": "whole_cake"
    },
    {
      "id": "YELLOW-111",
      "name": "库赞",
      "sub": " 执法者 ",
      "type": "char",
      "color": "yellow",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "gainDon",
          "n": 2
        }
      },
      "art": "YELLOW-111",
      "fruit": null,
      "rarity": "S",
      "faction": "navy",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "YELLOW-112",
      "name": "沙尔·克洛克达尔",
      "sub": " 夜行 ",
      "type": "char",
      "color": "yellow",
      "cost": 3,
      "power": 4000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "YELLOW-112",
      "fruit": "paramecia",
      "rarity": "A",
      "faction": "warlord",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "YELLOW-113",
      "name": "波尔萨利诺",
      "sub": " 海军将校 ",
      "type": "char",
      "color": "yellow",
      "cost": 3,
      "power": 3000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "damageLP",
          "x": 2000
        }
      },
      "art": "YELLOW-113",
      "fruit": null,
      "rarity": "S",
      "faction": "navy",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "YELLOW-114",
      "name": "歌姬",
      "sub": " 破竹之势 ",
      "type": "char",
      "color": "yellow",
      "cost": 2,
      "power": 3000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "YELLOW-114",
      "fruit": null,
      "rarity": "A",
      "faction": "yonko",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "YELLOW-115",
      "name": "波雅·汉库珂·王下",
      "sub": " 处刑人 ",
      "type": "char",
      "color": "yellow",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "doubleAttack"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "gainDon",
          "n": 1
        }
      },
      "art": "YELLOW-115",
      "fruit": null,
      "rarity": "S",
      "faction": "warlord",
      "formation": "skirmish",
      "arc": "whole_cake"
    },
    {
      "id": "YELLOW-116",
      "name": "Miss圣诞快乐",
      "sub": " 孤高之刃 ",
      "type": "char",
      "color": "yellow",
      "cost": 1,
      "power": 2000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "YELLOW-116",
      "fruit": null,
      "rarity": "A",
      "faction": "warlord",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "YELLOW-117",
      "name": "夏奇",
      "sub": " 大海之主 ",
      "type": "char",
      "color": "yellow",
      "cost": 4,
      "power": 4000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "draw",
          "n": 2
        }
      },
      "art": "YELLOW-117",
      "fruit": null,
      "rarity": "S",
      "faction": "yonko",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "YELLOW-118",
      "name": "霍格巴克",
      "sub": " 夜行 ",
      "type": "char",
      "color": "yellow",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "YELLOW-118",
      "fruit": null,
      "rarity": "B",
      "faction": "warlord",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "YELLOW-119",
      "name": "一笑",
      "sub": " 正义之师 ",
      "type": "char",
      "color": "yellow",
      "cost": 5,
      "power": 5000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "YELLOW-119",
      "fruit": null,
      "rarity": "B",
      "faction": "navy",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "YELLOW-120",
      "name": "香克斯",
      "sub": " 四皇麾下 ",
      "type": "char",
      "color": "yellow",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "draw",
          "n": 1
        }
      },
      "art": "YELLOW-120",
      "fruit": null,
      "rarity": "S",
      "faction": "yonko",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "YELLOW-121",
      "name": "康佩什",
      "sub": " 破竹之势 ",
      "type": "char",
      "color": "yellow",
      "cost": 7,
      "power": 8000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "YELLOW-121",
      "fruit": null,
      "rarity": "S",
      "faction": "yonko",
      "formation": "bulwark",
      "arc": "whole_cake"
    },
    {
      "id": "YELLOW-122",
      "name": "本·贝克曼",
      "sub": " 蛋糕城 ",
      "type": "char",
      "color": "yellow",
      "cost": 4,
      "power": 4000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onKO",
        "op": {
          "k": "healLP",
          "x": 2000
        }
      },
      "art": "YELLOW-122",
      "fruit": null,
      "rarity": "S",
      "faction": "yonko",
      "formation": "bulwark",
      "arc": "whole_cake"
    },
    {
      "id": "YELLOW-123",
      "name": "布鲁诺",
      "sub": " 正义之师 ",
      "type": "char",
      "color": "yellow",
      "cost": 8,
      "power": 8000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "gainDon",
          "n": 1
        }
      },
      "art": "YELLOW-123",
      "fruit": "paramecia",
      "rarity": "SSS",
      "faction": "navy",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "YELLOW-124",
      "name": "特拉法尔加·罗",
      "sub": " 孤高之刃 ",
      "type": "char",
      "color": "yellow",
      "cost": 3,
      "power": 3000,
      "counter": 1000,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "YELLOW-124",
      "fruit": null,
      "rarity": "B",
      "faction": "warlord",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "YELLOW-125",
      "name": "乔拉可尔·米霍克·王下",
      "sub": " 王下七武海 ",
      "type": "char",
      "color": "yellow",
      "cost": 8,
      "power": 8000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "draw",
          "n": 1
        }
      },
      "art": "YELLOW-125",
      "fruit": null,
      "rarity": "SSS",
      "faction": "warlord",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "YELLOW-126",
      "name": "席尔巴斯·雷利·四皇",
      "sub": " 四皇麾下 ",
      "type": "char",
      "color": "yellow",
      "cost": 2,
      "power": 2000,
      "counter": 1000,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "YELLOW-126",
      "fruit": "paramecia",
      "rarity": "B",
      "faction": "yonko",
      "formation": "vanguard",
      "arc": "whole_cake"
    },
    {
      "id": "YELLOW-127",
      "name": "迪亚玛蒂",
      "sub": " 暗流涌动 ",
      "type": "char",
      "color": "yellow",
      "cost": 3,
      "power": 3000,
      "counter": 1000,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "YELLOW-127",
      "fruit": null,
      "rarity": "B",
      "faction": "warlord",
      "formation": null,
      "arc": null
    },
    {
      "id": "YELLOW-128",
      "name": "Miss黄金周",
      "sub": " 王下七武海 ",
      "type": "char",
      "color": "yellow",
      "cost": 2,
      "power": 2000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "damageLP",
          "x": 1000
        }
      },
      "art": "YELLOW-128",
      "fruit": null,
      "rarity": "S",
      "faction": "warlord",
      "formation": null,
      "arc": "whole_cake"
    },
    {
      "id": "YELLOW-130",
      "name": "托雷波尔",
      "sub": " 暗流涌动 ",
      "type": "char",
      "color": "yellow",
      "cost": 3,
      "power": 4000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "YELLOW-130",
      "fruit": null,
      "rarity": "A",
      "faction": "warlord",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "YELLOW-131",
      "name": "拉基·路",
      "sub": " 蛋糕城 ",
      "type": "char",
      "color": "yellow",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "rush",
        "doubleAttack"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "search",
          "n": 1
        }
      },
      "art": "YELLOW-131",
      "fruit": null,
      "rarity": "SS",
      "faction": "yonko",
      "formation": "skirmish",
      "arc": "whole_cake"
    },
    {
      "id": "YELLOW-132",
      "name": "T·彭",
      "sub": " 缉捕令 ",
      "type": "char",
      "color": "yellow",
      "cost": 5,
      "power": 5000,
      "counter": 1000,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "YELLOW-132",
      "fruit": null,
      "rarity": "B",
      "faction": "navy",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "YELLOW-133",
      "name": "耶稣布·四皇",
      "sub": " 破竹之势 ",
      "type": "char",
      "color": "yellow",
      "cost": 2,
      "power": 3000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "YELLOW-133",
      "fruit": null,
      "rarity": "A",
      "faction": "yonko",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "YELLOW-134",
      "name": "鬼蜘蛛",
      "sub": " 铁律 ",
      "type": "char",
      "color": "yellow",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "restEnemy"
        }
      },
      "art": "YELLOW-134",
      "fruit": "paramecia",
      "rarity": "S",
      "faction": "navy",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "YELLOW-135",
      "name": "贝鲁梅特尔",
      "sub": " 巡逻舰队 ",
      "type": "char",
      "color": "yellow",
      "cost": 8,
      "power": 8000,
      "counter": null,
      "keywords": [
        "doubleAttack"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "restEnemy",
          "target": "last"
        }
      },
      "art": "YELLOW-135",
      "fruit": "logia",
      "rarity": "SSS",
      "faction": "navy",
      "formation": "skirmish",
      "arc": "whole_cake"
    },
    {
      "id": "YELLOW-137",
      "name": "唐吉诃德·多弗朗明戈·王下",
      "sub": " 孤高之刃 ",
      "type": "char",
      "color": "yellow",
      "cost": 7,
      "power": 8000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "YELLOW-137",
      "fruit": null,
      "rarity": "S",
      "faction": "warlord",
      "formation": "skirmish",
      "arc": "whole_cake"
    },
    {
      "id": "YELLOW-138",
      "name": "Mr.1",
      "sub": " 孤高之刃 ",
      "type": "char",
      "color": "yellow",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "draw",
          "n": 1
        }
      },
      "art": "YELLOW-138",
      "fruit": null,
      "rarity": "S",
      "faction": "warlord",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "YELLOW-139",
      "name": "巴斯提雍",
      "sub": " 缉捕令 ",
      "type": "char",
      "color": "yellow",
      "cost": 4,
      "power": 4000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "YELLOW-139",
      "fruit": null,
      "rarity": "B",
      "faction": "navy",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "YELLOW-141",
      "name": "拉基·路·四皇",
      "sub": " 皇族血脉 ",
      "type": "char",
      "color": "yellow",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [
        "doubleAttack",
        "rush"
      ],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "debuffFoeAll",
          "x": 1000
        }
      },
      "art": "YELLOW-141",
      "fruit": null,
      "rarity": "SS",
      "faction": "yonko",
      "formation": "bulwark",
      "arc": "whole_cake"
    },
    {
      "id": "YELLOW-142",
      "name": "加布拉",
      "sub": " 铁律 ",
      "type": "char",
      "color": "yellow",
      "cost": 4,
      "power": 5000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "YELLOW-142",
      "fruit": null,
      "rarity": "A",
      "faction": "navy",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "YELLOW-143",
      "name": "鹤",
      "sub": " 执法者 ",
      "type": "char",
      "color": "yellow",
      "cost": 1,
      "power": 1000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "YELLOW-143",
      "fruit": null,
      "rarity": "B",
      "faction": "navy",
      "formation": "vanguard",
      "arc": "whole_cake"
    },
    {
      "id": "YELLOW-144",
      "name": "塔马戈",
      "sub": " 蛋糕城 ",
      "type": "char",
      "color": "yellow",
      "cost": 8,
      "power": 8000,
      "counter": null,
      "keywords": [
        "doubleAttack"
      ],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "search",
          "n": 1
        }
      },
      "art": "YELLOW-144",
      "fruit": null,
      "rarity": "SSS",
      "faction": "yonko",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "YELLOW-145",
      "name": "贝鲁梅伯",
      "sub": " 巡逻舰队 ",
      "type": "char",
      "color": "yellow",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "YELLOW-145",
      "fruit": null,
      "rarity": "B",
      "faction": "navy",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "YELLOW-147",
      "name": "Miss双手指",
      "sub": " 处刑人 ",
      "type": "char",
      "color": "yellow",
      "cost": 5,
      "power": 5000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "search",
          "n": 1
        }
      },
      "art": "YELLOW-147",
      "fruit": null,
      "rarity": "S",
      "faction": "warlord",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "YELLOW-149",
      "name": "孔美奥",
      "sub": " 铁律 ",
      "type": "char",
      "color": "yellow",
      "cost": 5,
      "power": 5000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "restEnemy",
          "target": "strongest"
        }
      },
      "art": "YELLOW-149",
      "fruit": null,
      "rarity": "S",
      "faction": "navy",
      "formation": null,
      "arc": null
    },
    {
      "id": "YELLOW-150",
      "name": "战国·大将",
      "sub": " 铁律 ",
      "type": "char",
      "color": "yellow",
      "cost": 2,
      "power": 2000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "YELLOW-150",
      "fruit": "zoan",
      "rarity": "B",
      "faction": "navy",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "YELLOW-151",
      "name": "金狮子史基",
      "sub": " 大海之主 ",
      "type": "char",
      "color": "yellow",
      "cost": 4,
      "power": 5000,
      "counter": 2000,
      "keywords": [],
      "effect": null,
      "art": "YELLOW-151",
      "fruit": "paramecia",
      "rarity": "A",
      "faction": "yonko",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "YELLOW-152",
      "name": "文斯莫克·伊治",
      "sub": " 四皇麾下 ",
      "type": "char",
      "color": "yellow",
      "cost": 3,
      "power": 3000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "YELLOW-152",
      "fruit": null,
      "rarity": "B",
      "faction": "yonko",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "YELLOW-154",
      "name": "香克斯·四皇",
      "sub": " 破竹之势 ",
      "type": "char",
      "color": "yellow",
      "cost": 3,
      "power": 4000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "YELLOW-154",
      "fruit": null,
      "rarity": "A",
      "faction": "yonko",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "YELLOW-155",
      "name": "卡塔库栗",
      "sub": " 皇族血脉 ",
      "type": "char",
      "color": "yellow",
      "cost": 2,
      "power": 2000,
      "counter": 1000,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onKO",
        "op": {
          "k": "healLP",
          "x": 2000
        }
      },
      "art": "YELLOW-155",
      "fruit": null,
      "rarity": "S",
      "faction": "yonko",
      "formation": "bulwark",
      "arc": "whole_cake"
    },
    {
      "id": "YELLOW-156",
      "name": "缇娜",
      "sub": " 缉捕令 ",
      "type": "char",
      "color": "yellow",
      "cost": 2,
      "power": 2000,
      "counter": 1000,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "healLP",
          "x": 2000
        }
      },
      "art": "YELLOW-156",
      "fruit": null,
      "rarity": "S",
      "faction": "navy",
      "formation": "skirmish",
      "arc": "whole_cake"
    },
    {
      "id": "YELLOW-157",
      "name": "佩罗斯佩罗",
      "sub": " 破竹之势 ",
      "type": "char",
      "color": "yellow",
      "cost": 3,
      "power": 3000,
      "counter": 2000,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "draw",
          "n": 1
        }
      },
      "art": "YELLOW-157",
      "fruit": null,
      "rarity": "S",
      "faction": "yonko",
      "formation": "vanguard",
      "arc": "whole_cake"
    },
    {
      "id": "YELLOW-158",
      "name": "萨卡斯基·大将",
      "sub": " 正义之师 ",
      "type": "char",
      "color": "yellow",
      "cost": 4,
      "power": 4000,
      "counter": 1000,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "gainDon",
          "n": 1
        }
      },
      "art": "YELLOW-158",
      "fruit": null,
      "rarity": "S",
      "faction": "navy",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "YELLOW-159",
      "name": "乔拉可尔·米霍克",
      "sub": " 傀儡师 ",
      "type": "char",
      "color": "yellow",
      "cost": 4,
      "power": 4000,
      "counter": 1000,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "YELLOW-159",
      "fruit": null,
      "rarity": "B",
      "faction": "warlord",
      "formation": "skirmish",
      "arc": "whole_cake"
    },
    {
      "id": "YELLOW-160",
      "name": "庞蒂",
      "sub": " 皇族血脉 ",
      "type": "char",
      "color": "yellow",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "YELLOW-160",
      "fruit": null,
      "rarity": "S",
      "faction": "yonko",
      "formation": "bulwark",
      "arc": "whole_cake"
    },
    {
      "id": "YELLOW-161",
      "name": "Mr.4",
      "sub": " 王下七武海 ",
      "type": "char",
      "color": "yellow",
      "cost": 4,
      "power": 5000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "YELLOW-161",
      "fruit": "zoan",
      "rarity": "A",
      "faction": "warlord",
      "formation": null,
      "arc": "whole_cake"
    },
    {
      "id": "YELLOW-162",
      "name": "蒙多尔",
      "sub": " 大海之主 ",
      "type": "char",
      "color": "yellow",
      "cost": 3,
      "power": 3000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "YELLOW-162",
      "fruit": null,
      "rarity": "B",
      "faction": "yonko",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "YELLOW-163",
      "name": "文斯莫克·伽治",
      "sub": " 四皇麾下 ",
      "type": "char",
      "color": "yellow",
      "cost": 8,
      "power": 8000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "draw",
          "n": 1
        }
      },
      "art": "YELLOW-163",
      "fruit": null,
      "rarity": "SSS",
      "faction": "yonko",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "YELLOW-164",
      "name": "夏洛特·布蕾",
      "sub": " 皇族血脉 ",
      "type": "char",
      "color": "yellow",
      "cost": 3,
      "power": 3000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "YELLOW-164",
      "fruit": null,
      "rarity": "B",
      "faction": "yonko",
      "formation": "skirmish",
      "arc": "whole_cake"
    },
    {
      "id": "YELLOW-165",
      "name": "Mr.5",
      "sub": " 处刑人 ",
      "type": "char",
      "color": "yellow",
      "cost": 8,
      "power": 8000,
      "counter": null,
      "keywords": [
        "doubleAttack"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "restEnemy",
          "target": "last"
        }
      },
      "art": "YELLOW-165",
      "fruit": null,
      "rarity": "SSS",
      "faction": "warlord",
      "formation": "vanguard",
      "arc": "whole_cake"
    },
    {
      "id": "YELLOW-166",
      "name": "文斯莫克·蕾玖",
      "sub": " 皇族血脉 ",
      "type": "char",
      "color": "yellow",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "YELLOW-166",
      "fruit": "paramecia",
      "rarity": "S",
      "faction": "yonko",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "YELLOW-167",
      "name": "文斯莫克·尼治",
      "sub": " 破竹之势 ",
      "type": "char",
      "color": "yellow",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "YELLOW-167",
      "fruit": null,
      "rarity": "S",
      "faction": "yonko",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "YELLOW-168",
      "name": "阿拉马基",
      "sub": " 正义之师 ",
      "type": "char",
      "color": "yellow",
      "cost": 3,
      "power": 3000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "draw",
          "n": 2
        }
      },
      "art": "YELLOW-168",
      "fruit": null,
      "rarity": "S",
      "faction": "navy",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "YELLOW-170",
      "name": "斯摩格",
      "sub": " 正义之师 ",
      "type": "char",
      "color": "yellow",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "YELLOW-170",
      "fruit": null,
      "rarity": "B",
      "faction": "navy",
      "formation": "bulwark",
      "arc": "whole_cake"
    },
    {
      "id": "YELLOW-171",
      "name": "卡彭·贝基",
      "sub": " 皇族血脉 ",
      "type": "char",
      "color": "yellow",
      "cost": 3,
      "power": 4000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "YELLOW-171",
      "fruit": "logia",
      "rarity": "A",
      "faction": "yonko",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "YELLOW-173",
      "name": "哥尔·D·罗杰",
      "sub": " 亲卫队长 ",
      "type": "char",
      "color": "yellow",
      "cost": 5,
      "power": 5000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "debuffFoeAll",
          "x": 1000
        }
      },
      "art": "YELLOW-173",
      "fruit": null,
      "rarity": "S",
      "faction": "yonko",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "YELLOW-175",
      "name": "道格拉斯",
      "sub": " 破竹之势 ",
      "type": "char",
      "color": "yellow",
      "cost": 3,
      "power": 3000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "healLP",
          "x": 1000
        }
      },
      "art": "YELLOW-175",
      "fruit": null,
      "rarity": "S",
      "faction": "yonko",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "YELLOW-176",
      "name": "波宾",
      "sub": " 破竹之势 ",
      "type": "char",
      "color": "yellow",
      "cost": 4,
      "power": 4000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "YELLOW-176",
      "fruit": "paramecia",
      "rarity": "B",
      "faction": "yonko",
      "formation": "skirmish",
      "arc": "whole_cake"
    },
    {
      "id": "YELLOW-177",
      "name": "耶稣布",
      "sub": " 四皇麾下 ",
      "type": "char",
      "color": "yellow",
      "cost": 3,
      "power": 3000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "healLP",
          "x": 1000
        }
      },
      "art": "YELLOW-177",
      "fruit": null,
      "rarity": "S",
      "faction": "yonko",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "YELLOW-178",
      "name": "贝比5",
      "sub": " 孤高之刃 ",
      "type": "char",
      "color": "yellow",
      "cost": 2,
      "power": 2000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "YELLOW-178",
      "fruit": null,
      "rarity": "B",
      "faction": "warlord",
      "formation": "vanguard",
      "arc": "whole_cake"
    },
    {
      "id": "PURPLE-65",
      "name": "乌尔蒂",
      "sub": " 大看板 ",
      "type": "char",
      "color": "purple",
      "cost": 3,
      "power": 4000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "PURPLE-65",
      "fruit": "zoan",
      "rarity": "A",
      "faction": "beast",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "PURPLE-66",
      "name": "提娜",
      "sub": " 海军将校 ",
      "type": "char",
      "color": "purple",
      "cost": 1,
      "power": 1000,
      "counter": 1000,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "PURPLE-66",
      "fruit": null,
      "rarity": "B",
      "faction": "navy",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "PURPLE-67",
      "name": "霍尔德姆",
      "sub": " 百兽军团 ",
      "type": "char",
      "color": "purple",
      "cost": 8,
      "power": 8000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "damageLP",
          "x": 2000
        }
      },
      "art": "PURPLE-67",
      "fruit": null,
      "rarity": "SSS",
      "faction": "beast",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "PURPLE-69",
      "name": "多古拉",
      "sub": " 狩猎者 ",
      "type": "char",
      "color": "purple",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "damageLP",
          "x": 2000
        }
      },
      "art": "PURPLE-69",
      "fruit": "paramecia",
      "rarity": "S",
      "faction": "beast",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "PURPLE-70",
      "name": "佩罗娜·王下",
      "sub": " 孤高之刃 ",
      "type": "char",
      "color": "purple",
      "cost": 2,
      "power": 2000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "damageLP",
          "x": 1000
        }
      },
      "art": "PURPLE-70",
      "fruit": null,
      "rarity": "S",
      "faction": "warlord",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "PURPLE-71",
      "name": "多彭",
      "sub": " 狩猎者 ",
      "type": "char",
      "color": "purple",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [
        "doubleAttack"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "draw",
          "n": 2
        }
      },
      "art": "PURPLE-71",
      "fruit": "paramecia",
      "rarity": "S",
      "faction": "beast",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "PURPLE-72",
      "name": "克比",
      "sub": " 海军将校 ",
      "type": "char",
      "color": "purple",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [
        "blocker",
        "rush"
      ],
      "effect": null,
      "art": "PURPLE-72",
      "fruit": "zoan",
      "rarity": "SS",
      "faction": "navy",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "PURPLE-73",
      "name": "奎因·百兽海贼团",
      "sub": " 百兽军团 ",
      "type": "char",
      "color": "purple",
      "cost": 4,
      "power": 5000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "PURPLE-73",
      "fruit": "logia",
      "rarity": "A",
      "faction": "beast",
      "formation": null,
      "arc": null
    },
    {
      "id": "PURPLE-74",
      "name": "乔拉可尔·米霍克·王下",
      "sub": " 孤高之刃 ",
      "type": "char",
      "color": "purple",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [
        "blocker",
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "draw",
          "n": 2
        }
      },
      "art": "PURPLE-74",
      "fruit": "paramecia",
      "rarity": "SS",
      "faction": "warlord",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "PURPLE-76",
      "name": "斯摩格",
      "sub": " 执法者 ",
      "type": "char",
      "color": "purple",
      "cost": 2,
      "power": 2000,
      "counter": 1000,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "damageLP",
          "x": 1000
        }
      },
      "art": "PURPLE-76",
      "fruit": null,
      "rarity": "S",
      "faction": "navy",
      "formation": "skirmish",
      "arc": "dressrosa"
    },
    {
      "id": "PURPLE-77",
      "name": "库赞",
      "sub": " 缉捕令 ",
      "type": "char",
      "color": "purple",
      "cost": 4,
      "power": 5000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "PURPLE-77",
      "fruit": null,
      "rarity": "A",
      "faction": "navy",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "PURPLE-79",
      "name": "波雅·汉库珂",
      "sub": " 处刑人 ",
      "type": "char",
      "color": "purple",
      "cost": 2,
      "power": 2000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "draw",
          "n": 1
        }
      },
      "art": "PURPLE-79",
      "fruit": "paramecia",
      "rarity": "S",
      "faction": "warlord",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "PURPLE-80",
      "name": "蒙奇·D·卡普",
      "sub": " 执法者 ",
      "type": "char",
      "color": "purple",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "buffAll",
          "x": 2000
        }
      },
      "art": "PURPLE-80",
      "fruit": null,
      "rarity": "S",
      "faction": "navy",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "PURPLE-81",
      "name": "Mr.1",
      "sub": " 处刑人 ",
      "type": "char",
      "color": "purple",
      "cost": 5,
      "power": 6000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "PURPLE-81",
      "fruit": null,
      "rarity": "A",
      "faction": "warlord",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "PURPLE-83",
      "name": "克隆",
      "sub": " SMILE能力者 ",
      "type": "char",
      "color": "purple",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "draw",
          "n": 1
        }
      },
      "art": "PURPLE-83",
      "fruit": "paramecia",
      "rarity": "S",
      "faction": "beast",
      "formation": "skirmish",
      "arc": "dressrosa"
    },
    {
      "id": "PURPLE-84",
      "name": "蝙蝠",
      "sub": " 狩猎者 ",
      "type": "char",
      "color": "purple",
      "cost": 2,
      "power": 2000,
      "counter": 1000,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "PURPLE-84",
      "fruit": "logia",
      "rarity": "B",
      "faction": "beast",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "PURPLE-85",
      "name": "腕龙",
      "sub": " 百兽军团 ",
      "type": "char",
      "color": "purple",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "draw",
          "n": 1
        }
      },
      "art": "PURPLE-85",
      "fruit": null,
      "rarity": "S",
      "faction": "beast",
      "formation": null,
      "arc": "dressrosa"
    },
    {
      "id": "PURPLE-88",
      "name": "火烧山",
      "sub": " 执法者 ",
      "type": "char",
      "color": "purple",
      "cost": 2,
      "power": 2000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "PURPLE-88",
      "fruit": null,
      "rarity": "B",
      "faction": "navy",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "PURPLE-90",
      "name": "特雷波尔",
      "sub": " 夜行 ",
      "type": "char",
      "color": "purple",
      "cost": 3,
      "power": 3000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "PURPLE-90",
      "fruit": "zoan",
      "rarity": "B",
      "faction": "warlord",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "PURPLE-91",
      "name": "猛犸",
      "sub": " SMILE能力者 ",
      "type": "char",
      "color": "purple",
      "cost": 7,
      "power": 8000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "PURPLE-91",
      "fruit": null,
      "rarity": "S",
      "faction": "beast",
      "formation": "vanguard",
      "arc": "dressrosa"
    },
    {
      "id": "PURPLE-92",
      "name": "巴索罗缪·熊",
      "sub": " 夜行 ",
      "type": "char",
      "color": "purple",
      "cost": 5,
      "power": 6000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "PURPLE-92",
      "fruit": null,
      "rarity": "A",
      "faction": "warlord",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "PURPLE-94",
      "name": "大和",
      "sub": " 鬼岛之主 ",
      "type": "char",
      "color": "purple",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "draw",
          "n": 1
        }
      },
      "art": "PURPLE-94",
      "fruit": null,
      "rarity": "S",
      "faction": "beast",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "PURPLE-95",
      "name": "范德戴肯",
      "sub": " 百兽军团 ",
      "type": "char",
      "color": "purple",
      "cost": 3,
      "power": 3000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "PURPLE-95",
      "fruit": null,
      "rarity": "B",
      "faction": "beast",
      "formation": null,
      "arc": null
    },
    {
      "id": "PURPLE-97",
      "name": "凯多·百兽海贼团",
      "sub": " 狩猎者 ",
      "type": "char",
      "color": "purple",
      "cost": 3,
      "power": 3000,
      "counter": 1000,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "PURPLE-97",
      "fruit": null,
      "rarity": "B",
      "faction": "beast",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "PURPLE-98",
      "name": "波雅·汉库珂·王下",
      "sub": " 夜行 ",
      "type": "char",
      "color": "purple",
      "cost": 4,
      "power": 5000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "PURPLE-98",
      "fruit": "paramecia",
      "rarity": "A",
      "faction": "warlord",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "PURPLE-99",
      "name": "波尔萨利诺",
      "sub": " 缉捕令 ",
      "type": "char",
      "color": "purple",
      "cost": 2,
      "power": 3000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "PURPLE-99",
      "fruit": null,
      "rarity": "A",
      "faction": "navy",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "PURPLE-100",
      "name": "鼯鼠",
      "sub": " 正义之师 ",
      "type": "char",
      "color": "purple",
      "cost": 3,
      "power": 3000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "gainDon",
          "n": 1
        }
      },
      "art": "PURPLE-100",
      "fruit": null,
      "rarity": "S",
      "faction": "navy",
      "formation": null,
      "arc": null
    },
    {
      "id": "PURPLE-101",
      "name": "润媞",
      "sub": " 真打 ",
      "type": "char",
      "color": "purple",
      "cost": 4,
      "power": 5000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "PURPLE-101",
      "fruit": null,
      "rarity": "A",
      "faction": "beast",
      "formation": "vanguard",
      "arc": "dressrosa"
    },
    {
      "id": "PURPLE-102",
      "name": "夏洛特·布琳",
      "sub": " 夜行 ",
      "type": "char",
      "color": "purple",
      "cost": 2,
      "power": 3000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "PURPLE-102",
      "fruit": null,
      "rarity": "A",
      "faction": "warlord",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "PURPLE-103",
      "name": "巴基",
      "sub": " 夜行 ",
      "type": "char",
      "color": "purple",
      "cost": 6,
      "power": 7000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "PURPLE-103",
      "fruit": null,
      "rarity": "A",
      "faction": "warlord",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "PURPLE-107",
      "name": "迪亚玛蒂",
      "sub": " 处刑人 ",
      "type": "char",
      "color": "purple",
      "cost": 3,
      "power": 4000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "PURPLE-107",
      "fruit": "paramecia",
      "rarity": "A",
      "faction": "warlord",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "PURPLE-108",
      "name": "奎因",
      "sub": " 狩猎者 ",
      "type": "char",
      "color": "purple",
      "cost": 4,
      "power": 4000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "damageLP",
          "x": 2000
        }
      },
      "art": "PURPLE-108",
      "fruit": "zoan",
      "rarity": "S",
      "faction": "beast",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "PURPLE-109",
      "name": "卡特琳娜·蝶美",
      "sub": " 真打 ",
      "type": "char",
      "color": "purple",
      "cost": 3,
      "power": 3000,
      "counter": 1000,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "PURPLE-109",
      "fruit": "paramecia",
      "rarity": "B",
      "faction": "beast",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "PURPLE-111",
      "name": "沙尔·克洛克达尔·王下",
      "sub": " 暗流涌动 ",
      "type": "char",
      "color": "purple",
      "cost": 5,
      "power": 5000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "draw",
          "n": 1
        }
      },
      "art": "PURPLE-111",
      "fruit": null,
      "rarity": "S",
      "faction": "warlord",
      "formation": "vanguard",
      "arc": "dressrosa"
    },
    {
      "id": "PURPLE-112",
      "name": "范·奥卡",
      "sub": " SMILE能力者 ",
      "type": "char",
      "color": "purple",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "draw",
          "n": 1
        }
      },
      "art": "PURPLE-112",
      "fruit": null,
      "rarity": "S",
      "faction": "beast",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "PURPLE-114",
      "name": "马尔科",
      "sub": " 真打 ",
      "type": "char",
      "color": "purple",
      "cost": 1,
      "power": 1000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "PURPLE-114",
      "fruit": null,
      "rarity": "B",
      "faction": "beast",
      "formation": "skirmish",
      "arc": "dressrosa"
    },
    {
      "id": "PURPLE-115",
      "name": "布鲁诺",
      "sub": " 缉捕令 ",
      "type": "char",
      "color": "purple",
      "cost": 4,
      "power": 5000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "PURPLE-115",
      "fruit": null,
      "rarity": "A",
      "faction": "navy",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "PURPLE-116",
      "name": "金·百兽海贼团",
      "sub": " 鬼岛之主 ",
      "type": "char",
      "color": "purple",
      "cost": 4,
      "power": 4000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "PURPLE-116",
      "fruit": null,
      "rarity": "B",
      "faction": "beast",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "PURPLE-117",
      "name": "福兹弗",
      "sub": " 大看板 ",
      "type": "char",
      "color": "purple",
      "cost": 8,
      "power": 8000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "PURPLE-117",
      "fruit": "logia",
      "rarity": "SS",
      "faction": "beast",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "PURPLE-118",
      "name": "阿布萨罗姆",
      "sub": " 夜行 ",
      "type": "char",
      "color": "purple",
      "cost": 4,
      "power": 4000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "draw",
          "n": 2
        }
      },
      "art": "PURPLE-118",
      "fruit": null,
      "rarity": "S",
      "faction": "warlord",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "PURPLE-119",
      "name": "贝鲁梅特尔",
      "sub": " 执法者 ",
      "type": "char",
      "color": "purple",
      "cost": 1,
      "power": 1000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "PURPLE-119",
      "fruit": null,
      "rarity": "B",
      "faction": "navy",
      "formation": null,
      "arc": "dressrosa"
    },
    {
      "id": "PURPLE-120",
      "name": "巴基·王下",
      "sub": " 暗流涌动 ",
      "type": "char",
      "color": "purple",
      "cost": 4,
      "power": 4000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "PURPLE-120",
      "fruit": null,
      "rarity": "B",
      "faction": "warlord",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "PURPLE-122",
      "name": "强纳森",
      "sub": " 正义之师 ",
      "type": "char",
      "color": "purple",
      "cost": 7,
      "power": 8000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "PURPLE-122",
      "fruit": null,
      "rarity": "S",
      "faction": "navy",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "PURPLE-123",
      "name": "霍格巴克",
      "sub": " 处刑人 ",
      "type": "char",
      "color": "purple",
      "cost": 2,
      "power": 3000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "PURPLE-123",
      "fruit": null,
      "rarity": "A",
      "faction": "warlord",
      "formation": "skirmish",
      "arc": "dressrosa"
    },
    {
      "id": "PURPLE-124",
      "name": "玛丽哥德",
      "sub": " 王下七武海 ",
      "type": "char",
      "color": "purple",
      "cost": 5,
      "power": 5000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "buffAll",
          "x": 2000
        }
      },
      "art": "PURPLE-124",
      "fruit": "paramecia",
      "rarity": "S",
      "faction": "warlord",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "PURPLE-125",
      "name": "戴夫戈",
      "sub": " 真打 ",
      "type": "char",
      "color": "purple",
      "cost": 3,
      "power": 4000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "PURPLE-125",
      "fruit": null,
      "rarity": "A",
      "faction": "beast",
      "formation": "skirmish",
      "arc": "dressrosa"
    },
    {
      "id": "PURPLE-126",
      "name": "月光·莫利亚·王下",
      "sub": " 孤高之刃 ",
      "type": "char",
      "color": "purple",
      "cost": 5,
      "power": 5000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "buffAll",
          "x": 1000
        }
      },
      "art": "PURPLE-126",
      "fruit": null,
      "rarity": "S",
      "faction": "warlord",
      "formation": null,
      "arc": null
    },
    {
      "id": "PURPLE-127",
      "name": "霍迪·琼斯",
      "sub": " 鬼岛之主 ",
      "type": "char",
      "color": "purple",
      "cost": 4,
      "power": 4000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "PURPLE-127",
      "fruit": null,
      "rarity": "B",
      "faction": "beast",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "PURPLE-128",
      "name": "金",
      "sub": " 狩猎者 ",
      "type": "char",
      "color": "purple",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "PURPLE-128",
      "fruit": null,
      "rarity": "B",
      "faction": "beast",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "PURPLE-129",
      "name": "赞高",
      "sub": " 巡逻舰队 ",
      "type": "char",
      "color": "purple",
      "cost": 4,
      "power": 5000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "PURPLE-129",
      "fruit": "paramecia",
      "rarity": "A",
      "faction": "navy",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "PURPLE-130",
      "name": "达鲁马",
      "sub": " 狩猎者 ",
      "type": "char",
      "color": "purple",
      "cost": 5,
      "power": 5000,
      "counter": null,
      "keywords": [
        "rush",
        "blocker"
      ],
      "effect": null,
      "art": "PURPLE-130",
      "fruit": null,
      "rarity": "SS",
      "faction": "beast",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "PURPLE-131",
      "name": "Mr.5",
      "sub": " 王下七武海 ",
      "type": "char",
      "color": "purple",
      "cost": 4,
      "power": 4000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "PURPLE-131",
      "fruit": null,
      "rarity": "B",
      "faction": "warlord",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "PURPLE-132",
      "name": "德林杰",
      "sub": " 傀儡师 ",
      "type": "char",
      "color": "purple",
      "cost": 2,
      "power": 2000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "PURPLE-132",
      "fruit": null,
      "rarity": "B",
      "faction": "warlord",
      "formation": "skirmish",
      "arc": "dressrosa"
    },
    {
      "id": "PURPLE-133",
      "name": "路奇",
      "sub": " 海军将校 ",
      "type": "char",
      "color": "purple",
      "cost": 3,
      "power": 3000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "PURPLE-133",
      "fruit": null,
      "rarity": "B",
      "faction": "navy",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "PURPLE-134",
      "name": "巴加斯",
      "sub": " 百兽军团 ",
      "type": "char",
      "color": "purple",
      "cost": 4,
      "power": 4000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "PURPLE-134",
      "fruit": null,
      "rarity": "B",
      "faction": "beast",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "PURPLE-135",
      "name": "佩罗娜",
      "sub": " 处刑人 ",
      "type": "char",
      "color": "purple",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "PURPLE-135",
      "fruit": "zoan",
      "rarity": "B",
      "faction": "warlord",
      "formation": null,
      "arc": "dressrosa"
    },
    {
      "id": "PURPLE-138",
      "name": "黑炭大蛇",
      "sub": " 大看板 ",
      "type": "char",
      "color": "purple",
      "cost": 5,
      "power": 5000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "gainDon",
          "n": 1
        }
      },
      "art": "PURPLE-138",
      "fruit": null,
      "rarity": "S",
      "faction": "beast",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "PURPLE-139",
      "name": "塞尼奥尔·皮克",
      "sub": " 傀儡师 ",
      "type": "char",
      "color": "purple",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "draw",
          "n": 1
        }
      },
      "art": "PURPLE-139",
      "fruit": null,
      "rarity": "S",
      "faction": "warlord",
      "formation": "skirmish",
      "arc": "dressrosa"
    },
    {
      "id": "PURPLE-140",
      "name": "圣胡安·恶狼",
      "sub": " SMILE能力者 ",
      "type": "char",
      "color": "purple",
      "cost": 4,
      "power": 4000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "draw",
          "n": 1
        }
      },
      "art": "PURPLE-140",
      "fruit": null,
      "rarity": "S",
      "faction": "beast",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "PURPLE-141",
      "name": "豹藏",
      "sub": " 鬼岛之主 ",
      "type": "char",
      "color": "purple",
      "cost": 3,
      "power": 4000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "PURPLE-141",
      "fruit": null,
      "rarity": "A",
      "faction": "beast",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "PURPLE-142",
      "name": "伊卡洛斯",
      "sub": " SMILE能力者 ",
      "type": "char",
      "color": "purple",
      "cost": 2,
      "power": 3000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "PURPLE-142",
      "fruit": null,
      "rarity": "A",
      "faction": "beast",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "PURPLE-143",
      "name": "阿拉马基",
      "sub": " 执法者 ",
      "type": "char",
      "color": "purple",
      "cost": 5,
      "power": 5000,
      "counter": 2000,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "PURPLE-143",
      "fruit": null,
      "rarity": "B",
      "faction": "navy",
      "formation": "vanguard",
      "arc": "dressrosa"
    },
    {
      "id": "PURPLE-144",
      "name": "卡莉法",
      "sub": " 正义之师 ",
      "type": "char",
      "color": "purple",
      "cost": 4,
      "power": 4000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "PURPLE-144",
      "fruit": "zoan",
      "rarity": "B",
      "faction": "navy",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "PURPLE-145",
      "name": "月光·莫利亚",
      "sub": " 处刑人 ",
      "type": "char",
      "color": "purple",
      "cost": 8,
      "power": 8000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "buffAll",
          "x": 2000
        }
      },
      "art": "PURPLE-145",
      "fruit": null,
      "rarity": "SSS",
      "faction": "warlord",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "PURPLE-146",
      "name": "格拉迪乌斯",
      "sub": " 处刑人 ",
      "type": "char",
      "color": "purple",
      "cost": 2,
      "power": 3000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "PURPLE-146",
      "fruit": null,
      "rarity": "A",
      "faction": "warlord",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "PURPLE-147",
      "name": "斯皮德",
      "sub": " 百兽军团 ",
      "type": "char",
      "color": "purple",
      "cost": 4,
      "power": 4000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "draw",
          "n": 1
        }
      },
      "art": "PURPLE-147",
      "fruit": null,
      "rarity": "S",
      "faction": "beast",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "PURPLE-148",
      "name": "阿巴罗·比萨罗",
      "sub": " 真打 ",
      "type": "char",
      "color": "purple",
      "cost": 2,
      "power": 3000,
      "counter": 2000,
      "keywords": [],
      "effect": null,
      "art": "PURPLE-148",
      "fruit": null,
      "rarity": "A",
      "faction": "beast",
      "formation": "skirmish",
      "arc": "dressrosa"
    },
    {
      "id": "PURPLE-150",
      "name": "Mr.4",
      "sub": " 处刑人 ",
      "type": "char",
      "color": "purple",
      "cost": 3,
      "power": 3000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "PURPLE-150",
      "fruit": null,
      "rarity": "B",
      "faction": "warlord",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "PURPLE-151",
      "name": "鬼蜘蛛",
      "sub": " 正义之师 ",
      "type": "char",
      "color": "purple",
      "cost": 5,
      "power": 5000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "PURPLE-151",
      "fruit": null,
      "rarity": "B",
      "faction": "navy",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "PURPLE-152",
      "name": "毒Q",
      "sub": " 大看板 ",
      "type": "char",
      "color": "purple",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "draw",
          "n": 2
        }
      },
      "art": "PURPLE-152",
      "fruit": null,
      "rarity": "S",
      "faction": "beast",
      "formation": "skirmish",
      "arc": "dressrosa"
    },
    {
      "id": "PURPLE-153",
      "name": "贝鲁梅伯",
      "sub": " 铁律 ",
      "type": "char",
      "color": "purple",
      "cost": 2,
      "power": 2000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "PURPLE-153",
      "fruit": null,
      "rarity": "B",
      "faction": "navy",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "PURPLE-154",
      "name": "爱德华·威布尔",
      "sub": " 王下七武海 ",
      "type": "char",
      "color": "purple",
      "cost": 3,
      "power": 4000,
      "counter": 2000,
      "keywords": [],
      "effect": null,
      "art": "PURPLE-154",
      "fruit": "paramecia",
      "rarity": "A",
      "faction": "warlord",
      "formation": "bulwark",
      "arc": "dressrosa"
    },
    {
      "id": "PURPLE-156",
      "name": "Miss圣诞快乐",
      "sub": " 孤高之刃 ",
      "type": "char",
      "color": "purple",
      "cost": 5,
      "power": 5000,
      "counter": 1000,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "PURPLE-156",
      "fruit": null,
      "rarity": "B",
      "faction": "warlord",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "PURPLE-157",
      "name": "桑达索尼亚",
      "sub": " 王下七武海 ",
      "type": "char",
      "color": "purple",
      "cost": 2,
      "power": 3000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "PURPLE-157",
      "fruit": "paramecia",
      "rarity": "A",
      "faction": "warlord",
      "formation": null,
      "arc": null
    },
    {
      "id": "PURPLE-158",
      "name": "沙尔·克洛克达尔",
      "sub": " 王下七武海 ",
      "type": "char",
      "color": "purple",
      "cost": 3,
      "power": 4000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "PURPLE-158",
      "fruit": null,
      "rarity": "A",
      "faction": "warlord",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "PURPLE-159",
      "name": "巴索罗缪·熊·王下",
      "sub": " 傀儡师 ",
      "type": "char",
      "color": "purple",
      "cost": 2,
      "power": 3000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "PURPLE-159",
      "fruit": null,
      "rarity": "A",
      "faction": "warlord",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "PURPLE-160",
      "name": "唐吉诃德·多弗朗明戈·王下",
      "sub": " 傀儡师 ",
      "type": "char",
      "color": "purple",
      "cost": 2,
      "power": 2000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "debuffFoeAll",
          "x": 2000
        }
      },
      "art": "PURPLE-160",
      "fruit": null,
      "rarity": "S",
      "faction": "warlord",
      "formation": "bulwark",
      "arc": "dressrosa"
    },
    {
      "id": "PURPLE-161",
      "name": "凯多",
      "sub": " 鬼岛之主 ",
      "type": "char",
      "color": "purple",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onKO",
        "op": {
          "k": "debuffFoeAll",
          "x": 2000
        }
      },
      "art": "PURPLE-161",
      "fruit": null,
      "rarity": "S",
      "faction": "beast",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "PURPLE-162",
      "name": "卡库",
      "sub": " 缉捕令 ",
      "type": "char",
      "color": "purple",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "PURPLE-162",
      "fruit": "zoan",
      "rarity": "S",
      "faction": "navy",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "PURPLE-163",
      "name": "贝比5",
      "sub": " 夜行 ",
      "type": "char",
      "color": "purple",
      "cost": 2,
      "power": 2000,
      "counter": 2000,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "PURPLE-163",
      "fruit": "paramecia",
      "rarity": "B",
      "faction": "warlord",
      "formation": "vanguard",
      "arc": "dressrosa"
    },
    {
      "id": "PURPLE-164",
      "name": "玛格丽特",
      "sub": " 孤高之刃 ",
      "type": "char",
      "color": "purple",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "draw",
          "n": 1
        }
      },
      "art": "PURPLE-164",
      "fruit": "zoan",
      "rarity": "S",
      "faction": "warlord",
      "formation": null,
      "arc": "dressrosa"
    },
    {
      "id": "PURPLE-165",
      "name": "拉菲特",
      "sub": " 狩猎者 ",
      "type": "char",
      "color": "purple",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [
        "blocker",
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "debuffFoeAll",
          "x": 2000
        }
      },
      "art": "PURPLE-165",
      "fruit": null,
      "rarity": "SS",
      "faction": "beast",
      "formation": null,
      "arc": "dressrosa"
    },
    {
      "id": "PURPLE-166",
      "name": "巴巴努基",
      "sub": " 大看板 ",
      "type": "char",
      "color": "purple",
      "cost": 3,
      "power": 4000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "PURPLE-166",
      "fruit": null,
      "rarity": "A",
      "faction": "beast",
      "formation": "vanguard",
      "arc": "dressrosa"
    },
    {
      "id": "PURPLE-167",
      "name": "雨之希留",
      "sub": " SMILE能力者 ",
      "type": "char",
      "color": "purple",
      "cost": 7,
      "power": 8000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "PURPLE-167",
      "fruit": null,
      "rarity": "S",
      "faction": "beast",
      "formation": "skirmish",
      "arc": "dressrosa"
    },
    {
      "id": "PURPLE-168",
      "name": "Mr.2",
      "sub": " 孤高之刃 ",
      "type": "char",
      "color": "purple",
      "cost": 7,
      "power": 8000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "PURPLE-168",
      "fruit": "paramecia",
      "rarity": "S",
      "faction": "warlord",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "PURPLE-169",
      "name": "拉奥G",
      "sub": " 孤高之刃 ",
      "type": "char",
      "color": "purple",
      "cost": 2,
      "power": 2000,
      "counter": 1000,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "PURPLE-169",
      "fruit": "paramecia",
      "rarity": "B",
      "faction": "warlord",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "PURPLE-170",
      "name": "爱德华·威布尔·王下",
      "sub": " 王下七武海 ",
      "type": "char",
      "color": "purple",
      "cost": 5,
      "power": 5000,
      "counter": 2000,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "PURPLE-170",
      "fruit": null,
      "rarity": "B",
      "faction": "warlord",
      "formation": null,
      "arc": "dressrosa"
    },
    {
      "id": "PURPLE-171",
      "name": "特拉法尔加·罗·王下",
      "sub": " 傀儡师 ",
      "type": "char",
      "color": "purple",
      "cost": 2,
      "power": 2000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "PURPLE-171",
      "fruit": null,
      "rarity": "B",
      "faction": "warlord",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "PURPLE-172",
      "name": "莫奈",
      "sub": " 处刑人 ",
      "type": "char",
      "color": "purple",
      "cost": 1,
      "power": 1000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "PURPLE-172",
      "fruit": "zoan",
      "rarity": "B",
      "faction": "warlord",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "PURPLE-173",
      "name": "佐佐木",
      "sub": " SMILE能力者 ",
      "type": "char",
      "color": "purple",
      "cost": 2,
      "power": 2000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "PURPLE-173",
      "fruit": "zoan",
      "rarity": "B",
      "faction": "beast",
      "formation": "skirmish",
      "arc": "dressrosa"
    },
    {
      "id": "PURPLE-174",
      "name": "托雷波尔",
      "sub": " 傀儡师 ",
      "type": "char",
      "color": "purple",
      "cost": 5,
      "power": 5000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "PURPLE-174",
      "fruit": null,
      "rarity": "B",
      "faction": "warlord",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "PURPLE-175",
      "name": "砂糖",
      "sub": " 王下七武海 ",
      "type": "char",
      "color": "purple",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "PURPLE-175",
      "fruit": "paramecia",
      "rarity": "B",
      "faction": "warlord",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "PURPLE-176",
      "name": "维尔戈",
      "sub": " 处刑人 ",
      "type": "char",
      "color": "purple",
      "cost": 4,
      "power": 4000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "PURPLE-176",
      "fruit": null,
      "rarity": "B",
      "faction": "warlord",
      "formation": "vanguard",
      "arc": "dressrosa"
    },
    {
      "id": "PURPLE-177",
      "name": "皮卡",
      "sub": " 暗流涌动 ",
      "type": "char",
      "color": "purple",
      "cost": 8,
      "power": 8000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onKO",
        "op": {
          "k": "draw",
          "n": 1
        }
      },
      "art": "PURPLE-177",
      "fruit": null,
      "rarity": "SSS",
      "faction": "warlord",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "PURPLE-178",
      "name": "Miss双手指",
      "sub": " 孤高之刃 ",
      "type": "char",
      "color": "purple",
      "cost": 3,
      "power": 4000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "PURPLE-178",
      "fruit": "paramecia",
      "rarity": "A",
      "faction": "warlord",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "BLACK-65",
      "name": "贝洛·贝蒂·革命军",
      "sub": " 革命军干部 ",
      "type": "char",
      "color": "black",
      "cost": 8,
      "power": 8000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "BLACK-65",
      "fruit": "paramecia",
      "rarity": "SS",
      "faction": "revolutionary",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "BLACK-67",
      "name": "达斯琪",
      "sub": " 执法者 ",
      "type": "char",
      "color": "black",
      "cost": 3,
      "power": 4000,
      "counter": 2000,
      "keywords": [],
      "effect": null,
      "art": "BLACK-67",
      "fruit": null,
      "rarity": "A",
      "faction": "navy",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLACK-69",
      "name": "波尔萨利诺·大将",
      "sub": " 巡逻舰队 ",
      "type": "char",
      "color": "black",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "damageLP",
          "x": 2000
        }
      },
      "art": "BLACK-69",
      "fruit": null,
      "rarity": "S",
      "faction": "navy",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "BLACK-70",
      "name": "强纳森",
      "sub": " 正义之师 ",
      "type": "char",
      "color": "black",
      "cost": 2,
      "power": 3000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "BLACK-70",
      "fruit": null,
      "rarity": "A",
      "faction": "navy",
      "formation": "skirmish",
      "arc": "marineford"
    },
    {
      "id": "BLACK-71",
      "name": "斯潘达姆",
      "sub": " 缉捕令 ",
      "type": "char",
      "color": "black",
      "cost": 1,
      "power": 1000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "BLACK-71",
      "fruit": null,
      "rarity": "B",
      "faction": "navy",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLACK-72",
      "name": "佐佐木",
      "sub": " 大看板 ",
      "type": "char",
      "color": "black",
      "cost": 4,
      "power": 4000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "damageLP",
          "x": 1000
        }
      },
      "art": "BLACK-72",
      "fruit": null,
      "rarity": "S",
      "faction": "beast",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "BLACK-73",
      "name": "萨奇",
      "sub": " 家族之绊 ",
      "type": "char",
      "color": "black",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "damageLP",
          "x": 1000
        }
      },
      "art": "BLACK-73",
      "fruit": "paramecia",
      "rarity": "S",
      "faction": "whitebeard",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "BLACK-74",
      "name": "哈雷达斯",
      "sub": " 疾风 ",
      "type": "char",
      "color": "black",
      "cost": 5,
      "power": 5000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "BLACK-74",
      "fruit": null,
      "rarity": "B",
      "faction": "revolutionary",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLACK-77",
      "name": "小八",
      "sub": " 革命军干部 ",
      "type": "char",
      "color": "black",
      "cost": 3,
      "power": 4000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "BLACK-77",
      "fruit": null,
      "rarity": "A",
      "faction": "revolutionary",
      "formation": "bulwark",
      "arc": "marineford"
    },
    {
      "id": "BLACK-78",
      "name": "哈库·革命军",
      "sub": " 隐密行动 ",
      "type": "char",
      "color": "black",
      "cost": 5,
      "power": 5000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "buffAll",
          "x": 2000
        }
      },
      "art": "BLACK-78",
      "fruit": "paramecia",
      "rarity": "S",
      "faction": "revolutionary",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "BLACK-81",
      "name": "阿托摩斯",
      "sub": " 家族之绊 ",
      "type": "char",
      "color": "black",
      "cost": 3,
      "power": 3000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "BLACK-81",
      "fruit": null,
      "rarity": "B",
      "faction": "whitebeard",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "BLACK-83",
      "name": "贝鲁梅伯",
      "sub": " 铁律 ",
      "type": "char",
      "color": "black",
      "cost": 3,
      "power": 4000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "BLACK-83",
      "fruit": null,
      "rarity": "A",
      "faction": "navy",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "BLACK-85",
      "name": "布拉曼克",
      "sub": " 遗志 ",
      "type": "char",
      "color": "black",
      "cost": 4,
      "power": 4000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "BLACK-85",
      "fruit": null,
      "rarity": "B",
      "faction": "whitebeard",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLACK-86",
      "name": "莫莉·革命军",
      "sub": " 起义者 ",
      "type": "char",
      "color": "black",
      "cost": 5,
      "power": 5000,
      "counter": 2000,
      "keywords": [
        "banish"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "koWeakest"
        }
      },
      "art": "BLACK-86",
      "fruit": null,
      "rarity": "S",
      "faction": "revolutionary",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "BLACK-87",
      "name": "蒙奇·D·卡普·大将",
      "sub": " 铁律 ",
      "type": "char",
      "color": "black",
      "cost": 3,
      "power": 4000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "BLACK-87",
      "fruit": "logia",
      "rarity": "A",
      "faction": "navy",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "BLACK-88",
      "name": "蒙奇·D·龙",
      "sub": " 革命军干部 ",
      "type": "char",
      "color": "black",
      "cost": 4,
      "power": 4000,
      "counter": 2000,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "gainDon",
          "n": 1
        }
      },
      "art": "BLACK-88",
      "fruit": null,
      "rarity": "S",
      "faction": "revolutionary",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "BLACK-89",
      "name": "卡莉法",
      "sub": " 执法者 ",
      "type": "char",
      "color": "black",
      "cost": 5,
      "power": 6000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "BLACK-89",
      "fruit": null,
      "rarity": "A",
      "faction": "navy",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "BLACK-90",
      "name": "阿拉马基",
      "sub": " 巡逻舰队 ",
      "type": "char",
      "color": "black",
      "cost": 2,
      "power": 2000,
      "counter": 1000,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onKO",
        "op": {
          "k": "draw",
          "n": 1
        }
      },
      "art": "BLACK-90",
      "fruit": null,
      "rarity": "S",
      "faction": "navy",
      "formation": "bulwark",
      "arc": "marineford"
    },
    {
      "id": "BLACK-91",
      "name": "卡库",
      "sub": " 海军将校 ",
      "type": "char",
      "color": "black",
      "cost": 3,
      "power": 3000,
      "counter": 1000,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "BLACK-91",
      "fruit": null,
      "rarity": "B",
      "faction": "navy",
      "formation": "skirmish",
      "arc": "marineford"
    },
    {
      "id": "BLACK-93",
      "name": "林德伯格·革命军",
      "sub": " 革命军干部 ",
      "type": "char",
      "color": "black",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [
        "blocker",
        "rush"
      ],
      "effect": null,
      "art": "BLACK-93",
      "fruit": null,
      "rarity": "SS",
      "faction": "revolutionary",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "BLACK-95",
      "name": "萨卡斯基",
      "sub": " 巡逻舰队 ",
      "type": "char",
      "color": "black",
      "cost": 4,
      "power": 5000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "BLACK-95",
      "fruit": null,
      "rarity": "A",
      "faction": "navy",
      "formation": "bulwark",
      "arc": "marineford"
    },
    {
      "id": "BLACK-96",
      "name": "菊之丞",
      "sub": " 残火 ",
      "type": "char",
      "color": "black",
      "cost": 5,
      "power": 6000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "BLACK-96",
      "fruit": null,
      "rarity": "A",
      "faction": "whitebeard",
      "formation": "bulwark",
      "arc": "marineford"
    },
    {
      "id": "BLACK-97",
      "name": "纳尔逊",
      "sub": " 海军将校 ",
      "type": "char",
      "color": "black",
      "cost": 3,
      "power": 4000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "BLACK-97",
      "fruit": null,
      "rarity": "A",
      "faction": "navy",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "BLACK-98",
      "name": "库赞·大将",
      "sub": " 缉捕令 ",
      "type": "char",
      "color": "black",
      "cost": 1,
      "power": 1000,
      "counter": 1000,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "BLACK-98",
      "fruit": null,
      "rarity": "B",
      "faction": "navy",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "BLACK-99",
      "name": "路奇",
      "sub": " 正义之师 ",
      "type": "char",
      "color": "black",
      "cost": 5,
      "power": 5000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "BLACK-99",
      "fruit": null,
      "rarity": "B",
      "faction": "navy",
      "formation": "bulwark",
      "arc": "marineford"
    },
    {
      "id": "BLACK-100",
      "name": "提娜",
      "sub": " 缉捕令 ",
      "type": "char",
      "color": "black",
      "cost": 8,
      "power": 8000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "draw",
          "n": 1
        }
      },
      "art": "BLACK-100",
      "fruit": null,
      "rarity": "SSS",
      "faction": "navy",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "BLACK-103",
      "name": "贝鲁梅特尔",
      "sub": " 巡逻舰队 ",
      "type": "char",
      "color": "black",
      "cost": 1,
      "power": 1000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "BLACK-103",
      "fruit": null,
      "rarity": "B",
      "faction": "navy",
      "formation": "skirmish",
      "arc": "marineford"
    },
    {
      "id": "BLACK-104",
      "name": "安布里奥·伊万科夫",
      "sub": " 隐密行动 ",
      "type": "char",
      "color": "black",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "buffAll",
          "x": 1000
        }
      },
      "art": "BLACK-104",
      "fruit": null,
      "rarity": "S",
      "faction": "revolutionary",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "BLACK-108",
      "name": "安布里奥·伊万科夫·革命军",
      "sub": " 疾风 ",
      "type": "char",
      "color": "black",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "BLACK-108",
      "fruit": "paramecia",
      "rarity": "B",
      "faction": "revolutionary",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "BLACK-109",
      "name": "波特卡斯·D·艾斯",
      "sub": " 残火 ",
      "type": "char",
      "color": "black",
      "cost": 5,
      "power": 5000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "BLACK-109",
      "fruit": null,
      "rarity": "B",
      "faction": "whitebeard",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "BLACK-110",
      "name": "摩兹",
      "sub": " 解放战线 ",
      "type": "char",
      "color": "black",
      "cost": 3,
      "power": 3000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "gainDon",
          "n": 1
        }
      },
      "art": "BLACK-110",
      "fruit": null,
      "rarity": "S",
      "faction": "revolutionary",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLACK-111",
      "name": "天羽羽斩",
      "sub": " 赤鞘武士 ",
      "type": "char",
      "color": "black",
      "cost": 3,
      "power": 3000,
      "counter": 1000,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "BLACK-111",
      "fruit": null,
      "rarity": "B",
      "faction": "whitebeard",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "BLACK-112",
      "name": "霜月康家",
      "sub": " 白胡子一族 ",
      "type": "char",
      "color": "black",
      "cost": 3,
      "power": 3000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "BLACK-112",
      "fruit": null,
      "rarity": "B",
      "faction": "whitebeard",
      "formation": "vanguard",
      "arc": "marineford"
    },
    {
      "id": "BLACK-113",
      "name": "赞高",
      "sub": " 海军将校 ",
      "type": "char",
      "color": "black",
      "cost": 2,
      "power": 2000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "gainDon",
          "n": 1
        }
      },
      "art": "BLACK-113",
      "fruit": "logia",
      "rarity": "S",
      "faction": "navy",
      "formation": "skirmish",
      "arc": "marineford"
    },
    {
      "id": "BLACK-114",
      "name": "猫蝮蛇",
      "sub": " 家族之绊 ",
      "type": "char",
      "color": "black",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [
        "doubleAttack"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "draw",
          "n": 2
        }
      },
      "art": "BLACK-114",
      "fruit": null,
      "rarity": "S",
      "faction": "whitebeard",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "BLACK-116",
      "name": "豹藏",
      "sub": " 鬼岛之主 ",
      "type": "char",
      "color": "black",
      "cost": 4,
      "power": 4000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "draw",
          "n": 1
        }
      },
      "art": "BLACK-116",
      "fruit": null,
      "rarity": "S",
      "faction": "beast",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "BLACK-117",
      "name": "小玉",
      "sub": " 家族之绊 ",
      "type": "char",
      "color": "black",
      "cost": 3,
      "power": 3000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "BLACK-117",
      "fruit": null,
      "rarity": "B",
      "faction": "whitebeard",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "BLACK-118",
      "name": "拉库约",
      "sub": " 残火 ",
      "type": "char",
      "color": "black",
      "cost": 4,
      "power": 4000,
      "counter": 2000,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "draw",
          "n": 2
        }
      },
      "art": "BLACK-118",
      "fruit": "paramecia",
      "rarity": "S",
      "faction": "whitebeard",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "BLACK-119",
      "name": "一笑·大将",
      "sub": " 执法者 ",
      "type": "char",
      "color": "black",
      "cost": 7,
      "power": 8000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "BLACK-119",
      "fruit": null,
      "rarity": "S",
      "faction": "navy",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "BLACK-120",
      "name": "克比",
      "sub": " 缉捕令 ",
      "type": "char",
      "color": "black",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "banish"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "draw",
          "n": 1
        }
      },
      "art": "BLACK-120",
      "fruit": null,
      "rarity": "S",
      "faction": "navy",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "BLACK-121",
      "name": "奎因",
      "sub": " 狩猎者 ",
      "type": "char",
      "color": "black",
      "cost": 3,
      "power": 3000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "BLACK-121",
      "fruit": null,
      "rarity": "B",
      "faction": "beast",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "BLACK-123",
      "name": "萨博",
      "sub": " 解放战线 ",
      "type": "char",
      "color": "black",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "banish"
      ],
      "effect": null,
      "art": "BLACK-123",
      "fruit": null,
      "rarity": "B",
      "faction": "revolutionary",
      "formation": "skirmish",
      "arc": "marineford"
    },
    {
      "id": "BLACK-124",
      "name": "金",
      "sub": " SMILE能力者 ",
      "type": "char",
      "color": "black",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "BLACK-124",
      "fruit": null,
      "rarity": "B",
      "faction": "beast",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "BLACK-126",
      "name": "加布拉",
      "sub": " 执法者 ",
      "type": "char",
      "color": "black",
      "cost": 2,
      "power": 3000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "BLACK-126",
      "fruit": "zoan",
      "rarity": "A",
      "faction": "navy",
      "formation": "bulwark",
      "arc": "marineford"
    },
    {
      "id": "BLACK-127",
      "name": "一笑",
      "sub": " 缉捕令 ",
      "type": "char",
      "color": "black",
      "cost": 4,
      "power": 4000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "BLACK-127",
      "fruit": null,
      "rarity": "B",
      "faction": "navy",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "BLACK-128",
      "name": "维尔戈",
      "sub": " 海军将校 ",
      "type": "char",
      "color": "black",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [
        "banish",
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "search",
          "n": 1,
          "faction": "navy"
        }
      },
      "art": "BLACK-128",
      "fruit": null,
      "rarity": "SS",
      "faction": "navy",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "BLACK-129",
      "name": "战国·大将",
      "sub": " 缉捕令 ",
      "type": "char",
      "color": "black",
      "cost": 8,
      "power": 8000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "gainDon",
          "n": 2
        }
      },
      "art": "BLACK-129",
      "fruit": null,
      "rarity": "SSS",
      "faction": "navy",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "BLACK-130",
      "name": "杰克",
      "sub": " 遗志 ",
      "type": "char",
      "color": "black",
      "cost": 4,
      "power": 4000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "BLACK-130",
      "fruit": null,
      "rarity": "B",
      "faction": "whitebeard",
      "formation": "bulwark",
      "arc": "marineford"
    },
    {
      "id": "BLACK-131",
      "name": "闪电·革命军",
      "sub": " 革命军干部 ",
      "type": "char",
      "color": "black",
      "cost": 5,
      "power": 5000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "BLACK-131",
      "fruit": "paramecia",
      "rarity": "B",
      "faction": "revolutionary",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "BLACK-133",
      "name": "凯多·百兽海贼团",
      "sub": " 鬼岛之主 ",
      "type": "char",
      "color": "black",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "debuffFoeAll",
          "x": 2000
        }
      },
      "art": "BLACK-133",
      "fruit": null,
      "rarity": "S",
      "faction": "beast",
      "formation": null,
      "arc": "marineford"
    },
    {
      "id": "BLACK-134",
      "name": "缇娜",
      "sub": " 正义之师 ",
      "type": "char",
      "color": "black",
      "cost": 8,
      "power": 8000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "search",
          "n": 1,
          "faction": "navy"
        }
      },
      "art": "BLACK-134",
      "fruit": "paramecia",
      "rarity": "SSS",
      "faction": "navy",
      "formation": "vanguard",
      "arc": "marineford"
    },
    {
      "id": "BLACK-137",
      "name": "克尔拉",
      "sub": " 风之军 ",
      "type": "char",
      "color": "black",
      "cost": 4,
      "power": 4000,
      "counter": 2000,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "BLACK-137",
      "fruit": null,
      "rarity": "B",
      "faction": "revolutionary",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "BLACK-138",
      "name": "乔兹",
      "sub": " 赤鞘武士 ",
      "type": "char",
      "color": "black",
      "cost": 4,
      "power": 4000,
      "counter": 1000,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "BLACK-138",
      "fruit": null,
      "rarity": "B",
      "faction": "whitebeard",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "BLACK-139",
      "name": "萨博·革命军",
      "sub": " 风之军 ",
      "type": "char",
      "color": "black",
      "cost": 3,
      "power": 3000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "draw",
          "n": 2
        }
      },
      "art": "BLACK-139",
      "fruit": "paramecia",
      "rarity": "S",
      "faction": "revolutionary",
      "formation": "bulwark",
      "arc": "marineford"
    },
    {
      "id": "BLACK-140",
      "name": "斯库亚德",
      "sub": " 家族之绊 ",
      "type": "char",
      "color": "black",
      "cost": 2,
      "power": 2000,
      "counter": 1000,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "BLACK-140",
      "fruit": "logia",
      "rarity": "B",
      "faction": "whitebeard",
      "formation": "skirmish",
      "arc": "marineford"
    },
    {
      "id": "BLACK-141",
      "name": "卡拉斯",
      "sub": " 解放战线 ",
      "type": "char",
      "color": "black",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "BLACK-141",
      "fruit": "logia",
      "rarity": "B",
      "faction": "revolutionary",
      "formation": "skirmish",
      "arc": "marineford"
    },
    {
      "id": "BLACK-142",
      "name": "阿拉马基·大将",
      "sub": " 铁律 ",
      "type": "char",
      "color": "black",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "BLACK-142",
      "fruit": null,
      "rarity": "S",
      "faction": "navy",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "BLACK-143",
      "name": "佩吉万",
      "sub": " 百兽军团 ",
      "type": "char",
      "color": "black",
      "cost": 3,
      "power": 3000,
      "counter": 2000,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "BLACK-143",
      "fruit": null,
      "rarity": "B",
      "faction": "beast",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "BLACK-144",
      "name": "霍迪·琼斯",
      "sub": " 百兽军团 ",
      "type": "char",
      "color": "black",
      "cost": 5,
      "power": 6000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "BLACK-144",
      "fruit": null,
      "rarity": "A",
      "faction": "beast",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "BLACK-145",
      "name": "夏姆洛克",
      "sub": " 疾风 ",
      "type": "char",
      "color": "black",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "BLACK-145",
      "fruit": null,
      "rarity": "B",
      "faction": "revolutionary",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "BLACK-147",
      "name": "猛犸",
      "sub": " 狩猎者 ",
      "type": "char",
      "color": "black",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "BLACK-147",
      "fruit": null,
      "rarity": "S",
      "faction": "beast",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "BLACK-149",
      "name": "战国",
      "sub": " 铁律 ",
      "type": "char",
      "color": "black",
      "cost": 8,
      "power": 8000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onKO",
        "op": {
          "k": "gainDon",
          "n": 1
        }
      },
      "art": "BLACK-149",
      "fruit": null,
      "rarity": "SSS",
      "faction": "navy",
      "formation": "skirmish",
      "arc": "marineford"
    },
    {
      "id": "BLACK-150",
      "name": "莫莉",
      "sub": " 解放战线 ",
      "type": "char",
      "color": "black",
      "cost": 3,
      "power": 3000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "BLACK-150",
      "fruit": null,
      "rarity": "B",
      "faction": "revolutionary",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "BLACK-151",
      "name": "酒天丸",
      "sub": " 家族之绊 ",
      "type": "char",
      "color": "black",
      "cost": 5,
      "power": 5000,
      "counter": null,
      "keywords": [
        "rush",
        "blocker"
      ],
      "effect": {
        "hook": "onKO",
        "op": {
          "k": "search",
          "n": 1,
          "faction": "whitebeard"
        }
      },
      "art": "BLACK-151",
      "fruit": "paramecia",
      "rarity": "SS",
      "faction": "whitebeard",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "BLACK-153",
      "name": "达鲁马",
      "sub": " 鬼岛之主 ",
      "type": "char",
      "color": "black",
      "cost": 1,
      "power": 1000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "BLACK-153",
      "fruit": null,
      "rarity": "B",
      "faction": "beast",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "BLACK-155",
      "name": "犬岚公爵",
      "sub": " 赤鞘武士 ",
      "type": "char",
      "color": "black",
      "cost": 3,
      "power": 3000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "BLACK-155",
      "fruit": null,
      "rarity": "B",
      "faction": "whitebeard",
      "formation": "bulwark",
      "arc": "marineford"
    },
    {
      "id": "BLACK-159",
      "name": "卡拉斯·革命军",
      "sub": " 革命军干部 ",
      "type": "char",
      "color": "black",
      "cost": 4,
      "power": 4000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "BLACK-159",
      "fruit": null,
      "rarity": "B",
      "faction": "revolutionary",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "BLACK-160",
      "name": "鹤·大将",
      "sub": " 执法者 ",
      "type": "char",
      "color": "black",
      "cost": 7,
      "power": 8000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "BLACK-160",
      "fruit": null,
      "rarity": "S",
      "faction": "navy",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "BLACK-161",
      "name": "耕四郎",
      "sub": " 残火 ",
      "type": "char",
      "color": "black",
      "cost": 5,
      "power": 5000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "buffAll",
          "x": 2000
        }
      },
      "art": "BLACK-161",
      "fruit": null,
      "rarity": "S",
      "faction": "whitebeard",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "BLACK-162",
      "name": "库赞",
      "sub": " 缉捕令 ",
      "type": "char",
      "color": "black",
      "cost": 5,
      "power": 5000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "BLACK-162",
      "fruit": null,
      "rarity": "B",
      "faction": "navy",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "BLACK-163",
      "name": "爱德华·纽盖特",
      "sub": " 遗志 ",
      "type": "char",
      "color": "black",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "BLACK-163",
      "fruit": null,
      "rarity": "S",
      "faction": "whitebeard",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLACK-164",
      "name": "巴斯提雍",
      "sub": " 海军将校 ",
      "type": "char",
      "color": "black",
      "cost": 4,
      "power": 4000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "BLACK-164",
      "fruit": "logia",
      "rarity": "B",
      "faction": "navy",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "BLACK-165",
      "name": "林德伯格",
      "sub": " 疾风 ",
      "type": "char",
      "color": "black",
      "cost": 3,
      "power": 3000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "BLACK-165",
      "fruit": null,
      "rarity": "B",
      "faction": "revolutionary",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "BLACK-166",
      "name": "达斯琪·大将",
      "sub": " 缉捕令 ",
      "type": "char",
      "color": "black",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "BLACK-166",
      "fruit": null,
      "rarity": "B",
      "faction": "navy",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "BLACK-167",
      "name": "蒙奇·D·卡普",
      "sub": " 执法者 ",
      "type": "char",
      "color": "black",
      "cost": 3,
      "power": 3000,
      "counter": 2000,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "BLACK-167",
      "fruit": null,
      "rarity": "B",
      "faction": "navy",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "BLACK-170",
      "name": "闪电",
      "sub": " 疾风 ",
      "type": "char",
      "color": "black",
      "cost": 3,
      "power": 4000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "BLACK-170",
      "fruit": null,
      "rarity": "A",
      "faction": "revolutionary",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "BLACK-171",
      "name": "萨卡斯基·大将",
      "sub": " 缉捕令 ",
      "type": "char",
      "color": "black",
      "cost": 4,
      "power": 5000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "BLACK-171",
      "fruit": null,
      "rarity": "A",
      "faction": "navy",
      "formation": "skirmish",
      "arc": "marineford"
    },
    {
      "id": "BLACK-172",
      "name": "哈库",
      "sub": " 革命军干部 ",
      "type": "char",
      "color": "black",
      "cost": 2,
      "power": 2000,
      "counter": null,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "BLACK-172",
      "fruit": null,
      "rarity": "B",
      "faction": "revolutionary",
      "formation": "skirmish",
      "arc": "marineford"
    },
    {
      "id": "BLACK-174",
      "name": "传次郎",
      "sub": " 遗志 ",
      "type": "char",
      "color": "black",
      "cost": 1,
      "power": 1000,
      "counter": 1000,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "BLACK-174",
      "fruit": null,
      "rarity": "B",
      "faction": "whitebeard",
      "formation": "skirmish",
      "arc": "marineford"
    },
    {
      "id": "BLACK-175",
      "name": "蝙蝠",
      "sub": " 百兽军团 ",
      "type": "char",
      "color": "black",
      "cost": 6,
      "power": 7000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "BLACK-175",
      "fruit": "paramecia",
      "rarity": "A",
      "faction": "beast",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "BLACK-176",
      "name": "腕龙",
      "sub": " 大看板 ",
      "type": "char",
      "color": "black",
      "cost": 3,
      "power": 3000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "BLACK-176",
      "fruit": null,
      "rarity": "B",
      "faction": "beast",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "BLACK-177",
      "name": "蒙奇·D·龙·革命军",
      "sub": " 隐密行动 ",
      "type": "char",
      "color": "black",
      "cost": 1,
      "power": 1000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "BLACK-177",
      "fruit": null,
      "rarity": "B",
      "faction": "revolutionary",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLACK-178",
      "name": "克隆",
      "sub": " 鬼岛之主 ",
      "type": "char",
      "color": "black",
      "cost": 2,
      "power": 2000,
      "counter": 2000,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "BLACK-178",
      "fruit": null,
      "rarity": "B",
      "faction": "beast",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "BLACK-179",
      "name": "波尔萨利诺",
      "sub": " 正义之师 ",
      "type": "char",
      "color": "black",
      "cost": 2,
      "power": 3000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "BLACK-179",
      "fruit": "logia",
      "rarity": "A",
      "faction": "navy",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLACK-182",
      "name": "孔美奥",
      "sub": " 执法者 ",
      "type": "char",
      "color": "black",
      "cost": 1,
      "power": 2000,
      "counter": 2000,
      "keywords": [],
      "effect": null,
      "art": "BLACK-182",
      "fruit": null,
      "rarity": "A",
      "faction": "navy",
      "formation": "vanguard",
      "arc": "marineford"
    },
    {
      "id": "BLACK-183",
      "name": "巴巴努基",
      "sub": " 真打 ",
      "type": "char",
      "color": "black",
      "cost": 3,
      "power": 4000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "BLACK-183",
      "fruit": "zoan",
      "rarity": "A",
      "faction": "beast",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "BLACK-187",
      "name": "马尔科",
      "sub": " 真打 ",
      "type": "char",
      "color": "black",
      "cost": 3,
      "power": 3000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "draw",
          "n": 2
        }
      },
      "art": "BLACK-187",
      "fruit": null,
      "rarity": "S",
      "faction": "beast",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "BLACK-189",
      "name": "戴夫戈",
      "sub": " 百兽军团 ",
      "type": "char",
      "color": "black",
      "cost": 4,
      "power": 4000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onKO",
        "op": {
          "k": "gainDon",
          "n": 2
        }
      },
      "art": "BLACK-189",
      "fruit": null,
      "rarity": "S",
      "faction": "beast",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLACK-190",
      "name": "克尔拉·革命军",
      "sub": " 风之军 ",
      "type": "char",
      "color": "black",
      "cost": 3,
      "power": 3000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "draw",
          "n": 1
        }
      },
      "art": "BLACK-190",
      "fruit": null,
      "rarity": "S",
      "faction": "revolutionary",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "BLACK-191",
      "name": "东塔克斯三世",
      "sub": " 疾风 ",
      "type": "char",
      "color": "black",
      "cost": 2,
      "power": 3000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "BLACK-191",
      "fruit": null,
      "rarity": "A",
      "faction": "revolutionary",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLACK-192",
      "name": "鹤",
      "sub": " 缉捕令 ",
      "type": "char",
      "color": "black",
      "cost": 7,
      "power": 8000,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "BLACK-192",
      "fruit": null,
      "rarity": "S",
      "faction": "navy",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLACK-193",
      "name": "斯摩格·大将",
      "sub": " 海军将校 ",
      "type": "char",
      "color": "black",
      "cost": 3,
      "power": 4000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "BLACK-193",
      "fruit": "zoan",
      "rarity": "A",
      "faction": "navy",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "RED-G7",
      "name": "烈焰短刀",
      "sub": " 结盟 ",
      "type": "gear",
      "color": "red",
      "cost": 2,
      "power": null,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "RED-G7",
      "fruit": null,
      "rarity": "A",
      "gear": {
        "atk": 1000
      },
      "faction": "supernova",
      "formation": null,
      "arc": null
    },
    {
      "id": "RED-G8",
      "name": "赤犬军刀",
      "sub": " 结盟 ",
      "type": "gear",
      "color": "red",
      "cost": 2,
      "power": null,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "RED-G8",
      "fruit": null,
      "rarity": "B",
      "gear": {
        "atk": 2000
      },
      "faction": "supernova",
      "formation": null,
      "arc": null
    },
    {
      "id": "RED-G9",
      "name": "革命火铳",
      "sub": " 新星 ",
      "type": "gear",
      "color": "red",
      "cost": 3,
      "power": null,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "RED-G9",
      "fruit": null,
      "rarity": "S",
      "gear": {
        "atk": 3000
      },
      "faction": "supernova",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLUE-G6",
      "name": "苍波刃",
      "sub": " 缉捕令 ",
      "type": "gear",
      "color": "blue",
      "cost": 2,
      "power": null,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "BLUE-G6",
      "fruit": null,
      "rarity": "A",
      "gear": {
        "atk": 1000
      },
      "faction": "navy",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLUE-G7",
      "name": "海军制式铳",
      "sub": " 正义之师 ",
      "type": "gear",
      "color": "blue",
      "cost": 2,
      "power": null,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "BLUE-G7",
      "fruit": null,
      "rarity": "B",
      "gear": {
        "atk": 2000
      },
      "faction": "navy",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLUE-G8",
      "name": "冰河佩剑",
      "sub": " 缉捕令 ",
      "type": "gear",
      "color": "blue",
      "cost": 3,
      "power": null,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "BLUE-G8",
      "fruit": null,
      "rarity": "S",
      "gear": {
        "atk": 3000
      },
      "faction": "navy",
      "formation": null,
      "arc": null
    },
    {
      "id": "GREEN-G7",
      "name": "翠林弓",
      "sub": " 草帽一伙 ",
      "type": "gear",
      "color": "green",
      "cost": 2,
      "power": null,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "GREEN-G7",
      "fruit": null,
      "rarity": "A",
      "gear": {
        "atk": 1000
      },
      "faction": "strawhat",
      "formation": null,
      "arc": null
    },
    {
      "id": "GREEN-G8",
      "name": "兽王爪",
      "sub": " 自由之海 ",
      "type": "gear",
      "color": "green",
      "cost": 2,
      "power": null,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "GREEN-G8",
      "fruit": null,
      "rarity": "B",
      "gear": {
        "atk": 2000
      },
      "faction": "strawhat",
      "formation": null,
      "arc": null
    },
    {
      "id": "GREEN-G9",
      "name": "巨树大盾",
      "sub": " 冒险者 ",
      "type": "gear",
      "color": "green",
      "cost": 3,
      "power": null,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "GREEN-G9",
      "fruit": null,
      "rarity": "S",
      "gear": {
        "atk": 3000
      },
      "faction": "strawhat",
      "formation": null,
      "arc": null
    },
    {
      "id": "YELLOW-G7",
      "name": "雷光三节棍",
      "sub": " 四皇麾下 ",
      "type": "gear",
      "color": "yellow",
      "cost": 2,
      "power": null,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "YELLOW-G7",
      "fruit": null,
      "rarity": "A",
      "gear": {
        "atk": 1000
      },
      "faction": "yonko",
      "formation": null,
      "arc": null
    },
    {
      "id": "YELLOW-G8",
      "name": "天候棒·改",
      "sub": " 四皇麾下 ",
      "type": "gear",
      "color": "yellow",
      "cost": 2,
      "power": null,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "YELLOW-G8",
      "fruit": null,
      "rarity": "B",
      "gear": {
        "atk": 2000
      },
      "faction": "yonko",
      "formation": null,
      "arc": null
    },
    {
      "id": "YELLOW-G9",
      "name": "琥珀长弓",
      "sub": " 亲卫队长 ",
      "type": "gear",
      "color": "yellow",
      "cost": 3,
      "power": null,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "YELLOW-G9",
      "fruit": null,
      "rarity": "S",
      "gear": {
        "atk": 3000
      },
      "faction": "yonko",
      "formation": null,
      "arc": null
    },
    {
      "id": "PURPLE-G6",
      "name": "紫电细剑",
      "sub": " 暗流涌动 ",
      "type": "gear",
      "color": "purple",
      "cost": 2,
      "power": null,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "PURPLE-G6",
      "fruit": null,
      "rarity": "A",
      "gear": {
        "atk": 1000
      },
      "faction": "warlord",
      "formation": null,
      "arc": null
    },
    {
      "id": "PURPLE-G7",
      "name": "傀儡丝线",
      "sub": " 王下七武海 ",
      "type": "gear",
      "color": "purple",
      "cost": 2,
      "power": null,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "PURPLE-G7",
      "fruit": null,
      "rarity": "B",
      "gear": {
        "atk": 2000
      },
      "faction": "warlord",
      "formation": null,
      "arc": null
    },
    {
      "id": "PURPLE-G8",
      "name": "暗影匕首",
      "sub": " 王下七武海 ",
      "type": "gear",
      "color": "purple",
      "cost": 3,
      "power": null,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "PURPLE-G8",
      "fruit": null,
      "rarity": "S",
      "gear": {
        "atk": 3000
      },
      "faction": "warlord",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLACK-G7",
      "name": "黑翼斩舰刀",
      "sub": " 真打 ",
      "type": "gear",
      "color": "black",
      "cost": 2,
      "power": null,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "BLACK-G7",
      "fruit": null,
      "rarity": "A",
      "gear": {
        "atk": 1000
      },
      "faction": "beast",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLACK-G8",
      "name": "铁狱锁链",
      "sub": " 大看板 ",
      "type": "gear",
      "color": "black",
      "cost": 2,
      "power": null,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "BLACK-G8",
      "fruit": null,
      "rarity": "B",
      "gear": {
        "atk": 2000
      },
      "faction": "beast",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLACK-G9",
      "name": "夜枭钩爪",
      "sub": " SMILE能力者 ",
      "type": "gear",
      "color": "black",
      "cost": 3,
      "power": null,
      "counter": null,
      "keywords": [],
      "effect": null,
      "art": "BLACK-G9",
      "fruit": null,
      "rarity": "S",
      "gear": {
        "atk": 3000
      },
      "faction": "beast",
      "formation": null,
      "arc": null
    },
    {
      "id": "BLUE-174",
      "name": "霍格巴克",
      "sub": " 孤高之刃 ",
      "type": "char",
      "color": "blue",
      "cost": 5,
      "power": 5000,
      "counter": 2000,
      "keywords": [
        "rush"
      ],
      "effect": {
        "hook": "onPlay",
        "op": {
          "k": "buffAll",
          "x": 1000
        }
      },
      "art": "BLUE-174",
      "fruit": "paramecia",
      "rarity": "S",
      "faction": "warlord",
      "formation": "skirmish",
      "arc": "enies_lobby"
    },
    {
      "id": "BLUE-175",
      "name": "乔拉",
      "sub": " 傀儡师 ",
      "type": "char",
      "color": "blue",
      "cost": 1,
      "power": 2000,
      "counter": 2000,
      "keywords": [],
      "effect": null,
      "art": "BLUE-175",
      "fruit": null,
      "rarity": "A",
      "faction": "warlord",
      "formation": "bulwark",
      "arc": null
    },
    {
      "id": "BLUE-176",
      "name": "托雷波尔",
      "sub": " 处刑人 ",
      "type": "char",
      "color": "blue",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "BLUE-176",
      "fruit": null,
      "rarity": "B",
      "faction": "warlord",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "PURPLE-179",
      "name": "战国",
      "sub": " 缉捕令 ",
      "type": "char",
      "color": "purple",
      "cost": 3,
      "power": 3000,
      "counter": 1000,
      "keywords": [
        "rush"
      ],
      "effect": null,
      "art": "PURPLE-179",
      "fruit": null,
      "rarity": "B",
      "faction": "navy",
      "formation": null,
      "arc": "dressrosa"
    },
    {
      "id": "PURPLE-180",
      "name": "达斯琪",
      "sub": " 缉捕令 ",
      "type": "char",
      "color": "purple",
      "cost": 1,
      "power": 1000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "PURPLE-180",
      "fruit": null,
      "rarity": "B",
      "faction": "navy",
      "formation": "vanguard",
      "arc": "dressrosa"
    },
    {
      "id": "PURPLE-181",
      "name": "斯潘达姆",
      "sub": " 正义之师 ",
      "type": "char",
      "color": "purple",
      "cost": 7,
      "power": 7000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": {
        "hook": "whenAttacking",
        "op": {
          "k": "draw",
          "n": 2
        }
      },
      "art": "PURPLE-181",
      "fruit": null,
      "rarity": "S",
      "faction": "navy",
      "formation": "skirmish",
      "arc": null
    },
    {
      "id": "PURPLE-182",
      "name": "T·彭",
      "sub": " 缉捕令 ",
      "type": "char",
      "color": "purple",
      "cost": 3,
      "power": 4000,
      "counter": 1000,
      "keywords": [],
      "effect": null,
      "art": "PURPLE-182",
      "fruit": null,
      "rarity": "A",
      "faction": "navy",
      "formation": "vanguard",
      "arc": null
    },
    {
      "id": "PURPLE-183",
      "name": "孔美奥",
      "sub": " 海军将校 ",
      "type": "char",
      "color": "purple",
      "cost": 6,
      "power": 6000,
      "counter": null,
      "keywords": [
        "blocker"
      ],
      "effect": null,
      "art": "PURPLE-183",
      "fruit": null,
      "rarity": "B",
      "faction": "navy",
      "formation": "vanguard",
      "arc": null
    }
  ]
}
;

// ===== public API =====
global.OPTCG = {
  newGame, applyAction, cloneGame, validateDeck,
  powerOfUnit, leaderPower, usableDons, hasKeyword,
  fuse, fuseLockReason,
  listActions, createAI, evaluate, makeRng, shuffle,
  deckOf, POOL,
};
})(globalThis);
