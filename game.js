...
(function () {
  ...
  function cardEl(def, opts = {}) {
    ...
    el.__cardDef = def; // 悬停信息卡兜底数据源：Boss 合成首领（皮壳分离）不在 POOL，tipHtml 优先读元素自带定义
  ...
  function renderHints() {
    ...
    const myTurn = G.active === MY && !G.pending;
    let text;
    if (G.winner !== null) text = G.winner === MY ? '胜利！' : (G.winner === -1 ? '平局' : '战败…');
  ...
  async function playOne(ev) {
    switch (ev.t) {
      ...
      case 'win': {
        if (ev.winner === -1) { // v2 平局（deckout-draw）：专属横幅与音效
          sfx('lose');
          await sleep(250);
          showBanner('平局', 'defeat');
          break;
        }
  ...
  function setupUI() {
    ...
    $('btnMenu').onclick = async () => {
      ...
      // 同时补记本地战绩（v0.9.4 前 settle 走了、stats 没记=云端账本与本地战绩对不上）
      if (G && G.winner === null) { recordResult(false, false); }
  ...
  // v0.9.5 对局桌面主题：color=null 撤桌（退出对局），否则挂 scene-table+theme-{color}
  // （六色=对手/关卡色；CSS 见 style.css .bg-scene-table 段）
  const THEME_COLORS = ['red', 'blue', 'green', 'yellow', 'purple', 'black'];
  function applyTableScene(color) {
    document.body.classList.remove('scene-table');
    for (const c of THEME_COLORS) document.body.classList.remove('theme-' + c);
    if (color && THEME_COLORS.includes(color)) {
      document.body.classList.add('scene-table');
  ...
  function showModeSelect() {
    ...
    applyTableScene(null);                          // v0.9.5 回港口撤对局桌面
  ...
  function startGame(opts = {}) {
    ...
    // 故事之旅大关成长：主角攻击力加成挂接到玩家船长 power（模式层传入，引擎零改动）
    const myLeaderOut = (opts.myLeaderAtk > 0 && myLeader && Number.isFinite(myLeader.power))
      ? { ...myLeader, power: myLeader.power + Math.round(opts.myLeaderAtk) }
      : myLeader;
    ...
    G = O.newGame({
      ...
      rules: opts.rules || null, // v2 可选规则集（deckoutByLp：牌库打空按 LP 裁定）——模式层透传，缺省=旧行为
    });
    ...
    // 故事之旅不录：Boss LP/战力覆写与主角 atk 成长只存 id 不可还原，录了回放必中途「回放中断」
    replayCtx = (opts.ctx && opts.ctx.mode === 'story') ? null : {
    ...
    // v0.9.5 对局桌面主题（背景换）：故事局=关卡色（story.js ctx.themeColor），
    // 其余模式=对手船长色；写回 gameCtx 供断档恢复还原桌面
    const themeColor = (gameCtx && gameCtx.themeColor) || foeLeader.color || 'red';
    if (gameCtx) gameCtx.themeColor = themeColor;
    document.body.classList.remove('scene-select', 'scene-story'); // 航海日志纸只在选关面板，对局一律主题桌面
    applyTableScene(themeColor);
    clearNextStageTimer();
    ended = false;
  ...
  function mountNextStageButton(win) {
    ...
    const next = (stage >= 1 && window.OPTCG_STORY.STAGES && stage < window.OPTCG_STORY.STAGES.length) ? stage + 1 : 0; // 终关通关后=重返关卡列表（不倒计时）
  ...
  // v0.9.4：抽出公共入口——投降局（btnMenu 判负路径）此前绕过 showEndPanel 漏 record，双账本不一致
  function recordResult(win, withReplay) {
    try {
      if (window.OPTCG_STATS && !replayMode) {
        window.OPTCG_STATS.record({
          ...
          win, turns: G.turn,
          replay: (withReplay && replayCtx && replayCtx.actions.length >= 4) ? { ...replayCtx } : null, // 投降局不记半程回放（无终局，回放器打不完）
        });
      }
    } catch (e) { /* 统计异常不阻断结算 */ }
  }
  ...
  function showEndPanel() {
    ...
    recordResult(win, true);
    const draw = G.winner === -1; // v2 平局三态：win=false 不再唯一对应败北
    const byDeckout = G.winReason === 'deckout-lp' || G.winReason === 'deckout-draw';
    let detail = `历时 ${G.turn} 回合 · 我方剩余 LP ${Math.max(0, G.players[MY].lp)} · 对方剩余 LP ${Math.max(0, G.players[FOE].lp)}`
      + (byDeckout ? '<br>牌库打空——按积分（LP）裁定' : '');
    if (gameCtx && window.OPTCG_MODES) {
      const s = window.OPTCG_MODES.settle(gameCtx, win, draw);
      if (s) detail = s + '<br>' + detail;
    }
    $('endTitle').innerHTML = (draw ? CAP().icon('flag') + ' 平局' : win ? CAP().icon('trophy') + ' 胜利！' : CAP().icon('skull') + ' 战败');
  ...
  function restoreFromSnapshot(snap) {
    try {
      ...
      document.body.classList.remove('scene-select', 'scene-story'); // 断档恢复也按模式落场景（G8）
      applyTableScene(gameCtx && gameCtx.themeColor); // v0.9.5 恢复对局桌面主题（旧档无 themeColor=默认深海）
  ...
  async function startReplay(rp) {
    ...
    replayMode = true;
    applyTableScene(lb.color); // v0.9.5 回放同享对局桌面主题（退出经 backToMenu 撤桌）
  ...
  function tipHtml(el) {
    ...
    if (!id && !el.__cardDef) return null;
    // 元素自带定义优先（v0.9.2 皮壳分离）：故事 Boss 首领的 cardId 是技能壳 id，
    // 直接查 POOL 会显示壳真身（立绘/名字）——mkFoe 产物才是玩家看到的 Boss
    const def = el.__cardDef || O.POOL.cards.find((c) => c.id === id) || O.POOL.leaders.find((l) => l.id === id);
    if (!def) return null;
    const parts = [
