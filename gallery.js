...
(function () {
  ...
  function renderBody() {
    ...
    // 口径统一（v0.9.4）：头部总数含船长组——此前只数 cards（861）与分区合计（+船长12=873）对不上
    total += OPTCG.POOL.leaders.length;
  ...
  function openViewer(def) {
    ...
    viewer.innerHTML = '<div class="codex-view-card">'
      + '<div class="codex-view-art"><img src="art/' + (def.art || def.id) + '.webp" alt="' + def.name + '"'
      + ' onerror="this.style.display=\'none\';this.parentNode.classList.add(\'no-art\')">'
      + '<div class="cva-fb">' + (() => { try { const C = window.OPTCG_CAPTAINS; return C.icon((C.FACTION[def.color] || {}).emblem, 'glyph-emblem'); } catch (e) { return '🃏'; } })() + '</div></div>'
