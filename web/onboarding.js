...
(function () {
  ...
  const STEPS = [
    ...
    },
    {
      ...
      text: '对局开始后：屏幕下方是你的手牌；左下角的圆点是 DON!! 费用区，出牌要消耗它；行动完毕点「结束回合」交给对方。',
      map: ['hand', 'don', 'end'],
    },
    {
      icon: 'swords', title: '第 4 步 · 攻击方式',
      text: '点己方未横置的角色或船长，再点对方船长或已横置的角色即可发起攻击。战力不低于对方即击沉；打中船长则扣 1 张生命。',
    },
    {
      icon: 'trophy', title: '第 5 步 · 胜利目标',
      text: '把对方船长的生命扣到 0 即获胜。手牌右下角带 C 标记的牌，可在被攻击时打出反击、临时加战力。',
    },
  ];
  const MAP_META = {
    hand: { icon: 'layers', label: '手牌：点卡出牌' },
    don: { icon: 'star', label: 'DON!! 费用区：出牌的燃料' },
